...
| **بوابات الدفع الإلكتروني المتكاملة (Tap GCC & Stripe & Paymob & XPay & Mock)** | 🟢 | 100% | `storefront-payment.service.ts`, `StorefrontPaymentGatewaysManager.tsx`, `StorefrontOnlinePaymentModal.tsx`, `storefront-public.controller.ts` | دعم متكامل وشامل لـ 5 بوابات دفع رئيسية: **تاب (Tap Payments الخليجية)** لدعم بطاقات مدى السعودية، شبكة كي نت الكويتية، بطاقات ناباس القطرية، وبطاقات بنفت، و **Stripe العالمية** لدعم كافة البطاقات الدولية و Apple Pay و Google Pay، بالإضافة إلى **Paymob** و **XPay** للمدفوعات المصرية و **Sandbox Mock**، مع تأكيد سداد آلي عبر الـ Webhooks وإشعارات واتساب فورية. **تحديث (بند O2):** تأكيد السداد الوارد صار يفشل مغلقاً ولا يصل إلى أي طلب دون إثبات ملكيته للمنشأة (الثابت WH-1). **ملاحظة مفتوحة (O25):** التحقق من توقيع الويب هوك نفسه لم يُنفَّذ بعد لـXPay وTap وStripe، وتحقق Paymob يفشل آمناً معكوساً عند غياب السر — مُجدوَل في البند 9. |
...
| **سداد الأقساط والربط المحاسبي الكامل (Installments Payments & GL Sync)** | 🟢 | 100% | `customer-installments.service.ts`, `sales-finance.service.ts`, `accounting-posting.service.ts` | سداد الأقساط بقفل تشاؤمي `forUpdate()` لمنع التحصيل الزائد، مع خصم رصيد العميل في الأستاذ المساعد، إثبات حركة الخزينة، وترحيل قيد متوازن للأستاذ العام برقم إيصال موحد `REC-YYMMDD-XXXX`. |
