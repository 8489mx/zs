...
* **مسارات الكود والوحدات المقسمة:**
  ...
  23. **تطهير وحذف المنشآت القديمة المكررة وقصر شارة المنصة المركزية حصراً على `zs` (Legacy Platform Tenants Cleanup & Strict SaaS Isolation):**
      ...
      - **حصر شارة المنصة المركزية والتحقق البرمجي:** تنظيف وتحديث فحص `isPlatformTenantRow` في الواجهة (`TenantActionsMenu.tsx`) والباك إند (`saas-admin.service.ts`) ليقتصر حصراً على المعرف الرسمي للمنصة `zs` أو `PLATFORM_TENANT_ID`، وإلغاء كافة الشروط الفضفاضة السابقة التي كانت تمنح شارة المنصة وحساب المؤسس الدائم تلقائياً لأي منشأة تحمل ID باسم `default` أو slug باسم `zsystems`، مما أنهى التضارب وحصر السوبر أدمن وإدارة الساس في المنشأة الأم الفريدة.
