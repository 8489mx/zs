...
## 📐 ملف مصاحب إلزامي: دستور الثوابت المعمارية

> هذا الملف يجيب على سؤال **«ماذا يفعل النظام؟»**.
> أما أسئلة **«ما الذي يُمنع كتابته؟ وأين المصدر الوحيد للحقيقة؟ وما الثوابت المفروضة؟ وماذا دُقِّق؟»**
> فإجابتها في **[`ARCHITECTURE_INVARIANTS.md`](./ARCHITECTURE_INVARIANTS.md)**.
>
> **قبل كتابة أي كود مالي أو محاسبي أو يمسّ المخزون، اقرأ القسمين 2 و3 من ذلك الملف.**
> يحتوي على: المحركات التي يجب المرور بها حصراً (`applyStockDelta`، `insertPostedJournal`، محركات
> الحساب النقية)، والأنماط المحظورة مع الخطأ الحقيقي الذي سبَّبه كل نمط، وسجل الثوابت
> (I1–I12 · G1 · G1-C · MATCH-1–3 · C1–C3)، وسجل الهجرات البنيوية، وحالة مراحل التدقيق، والبنود المفتوحة.
>
> **وملف حماية الأداء:** [`PERFORMANCE_CONSTITUTION.md`](./PERFORMANCE_CONSTITUTION.md) — الثوابت PERF-1..PERF-12
> (سرعة كتالوج نقطة البيع، كتالوج المتجر العام، تقسيم حزم الواجهة، الفهارس الساخنة). اقرأه قبل لمس
> `vite.config.ts` أو `main.ts` أو استيرادات الـShell أو الكتالوجات.

### قدرة: أداء وسرعة المنظومة (Performance) — ✅ مفعّلة ومحروسة (21 سبتمبر 2026)
- **نقطة البيع:** مزامنة الكتالوج الكامل (حتى 25 ألف صنف) خطية: حساب أرصدة المواقع لـ20 ألف صنف ~25ms (كان ~13 ثانية تحجب السيرفر). الكتالوج مخزَّن محلياً في IndexedDB (للبحث والباركود والبيع عند انقطاع الإنترنت، والفواتير تنتظر في طابور وتُرحَّل عند عودته)، ويُعاد تحميله كاملاً **فقط** لو تغيّر سعر/اسم/باركود/وحدة/عرض أو أُضيف/أُلغي صنف، أو مرة كل ساعة لتحديث المخزون المحلي — لا مع كل بيعة كما كان (PERF-9، هجرة 137).
- **صفحة الأصناف:** تعمل مع أي حجم كتالوج (معامل مصفوفة واحد بدل معامل لكل صنف — كان الحد 65,535 صنفاً).
- **الردود:** بلا مُسلسِل عالمي؛ الردود الكبيرة أسرع ~100ms لكل 5000 صف.
- **المتجر العام:** كتالوج بكاش ذاكرة 60 ثانية + Stale-While-Revalidate حتى 5 دقائق (نسخة فورية وتحديث بالخلفية)، وإعادة البناء بخمس قراءات متوازية. الأسعار تُعاد على السيرفر عند الطلب.
- **الواجهة:** jsPDF وhtml2canvas وxlsx وrecharts وماسح الباركود بالكاميرا وSentry وpdfjs **لا تُحمَّل عند فتح أي صفحة** — كلها عند أول استخدام.
- **قاعدة البيانات:** فهارس المسارات الساخنة (الفواتير المعلقة، الخزينة بالتاريخ، أصناف المشتريات، مدفوعات الموردين، التحويلات، الاشتراكات) — هجرة 136.
...
* **حالة الوحدة العامة:** 🟢 مكتمل ومراجَع — مراجعة المتجر بمراحلها الثلاث أُغلقت في 21 سبتمبر 2026 (الوصول العام للطلبات والتحصيل، الخصم وطلب الطاولة والمقاسات، الصلاحيات والأسرار والترقيم والمخزون وحد الطلبات). بقي بندان موثقان ليسا أخطاء: O62 (قد يُطلب من الكاشير إذن خصم/سعر عند ترحيل طلب فيه كوبون أو مقاس من السلة) وO63 (المقاسات بلا مخزون مستقل، ومقاس واحد لكل صنف في الطلب)
...
| :--- | :---: | :---: | :--- | :--- |
| **واجهة الكتالوج العام للمتجر** | 🟢 | 100% | `PublicStorefrontPage.tsx`, `storefront.service.ts` | تصفح الأصناف، الأقسام، شريط البحث الفوري، فرز الأسعار، والمفضلة.<br/>**تحصين سبتمبر 2026 (O52):** الأصناف المحذوفة منطقياً (`is_active = false`) لم تعد تظهر في الكتالوج والبحث وصفحة الصنف، ولا يمكن طلبها أو تقييمها (مع مهلة كاش الكتالوج حتى 5 دقائق للعرض فقط؛ الطلب يُرفض فوراً). |
| **سلة المشتريات التفاعلية** | 🟢 | 100% | `StorefrontCartDrawer.tsx`, `StorefrontProductCard.tsx`, `storefront-variant-pricing.ts`, `online-order-pricing.engine.ts` | درج سلة جانبي، تعديل الكميات (+ و - بنظام RTL صحيح)، وحساب المجاميع فورياً.<br/>**المقاسات (سبتمبر 2026 — SF-4):** المقاس المختار يُرسل مع الطلب ويُسعَّر في السيرفر بنفس قاعدة العرض (سعر مطلق أو الأساس + فرق)، ويظهر اسمه للتاجر وفي الفاتورة وتذكرة المطبخ. **مقاس واحد لكل صنف في الطلب**: اختيار مقاس آخر لنفس الصنف يستبدل السطر مع تنبيه في نافذة المعاينة. الإضافة السريعة لصنف له مقاسات تفتح المعاينة. كمية المعاينة صارت تُحترم عند الإضافة. |
| **نافذة إتمام الطلب المحكمة** | 🟢 | 100% | `StorefrontCheckoutModal.tsx` | تحقق صارم برقم الموبايل المصري (11 رقم: 010/011/012/015)، الاسم، العنوان، وتثبيت زر الطلب Sticky CTA.<br/>**الدفع السريع للعميل الراجع (سبتمبر 2026):** لو بيانات العميل محفوظة على الجهاز وكلها صالحة، تظهر في سطر ملخص واحد («التوصيل إلى: الاسم · الهاتف · العنوان») مع زر «تعديل البيانات» بدل الحقول الثلاثة، فيصبح الطلب ضغطة واحدة. |
...
| **تتبع الطلبات المباشر «طلباتي»** | 🟢 | 100% | `StorefrontCustomerOrderCard.tsx`, `StorefrontMyOrdersModal.tsx`, `customer-order-refs.ts`, `online-order-access.engine.ts` | شريط مراحل تتبع حي (تم الاستلام ➔ جاري التجهيز ➔ مع المندوب ➔ تم التسليم) مع إمكانية إلغاء وتعديل الطلب وهو pending وغير مسدد.<br/>**تحصين سبتمبر 2026 (SF-1):** كانت القائمة والإلغاء والتعديل تعتمد على رقم الطلب التسلسلي أو البحث برقم الهاتف، فكان أي زائر يقرأ بيانات كل العملاء ويلغي طلباتهم. صار لكل طلب **توكن سري** يُحفظ على جهاز العميل عند الطلب (السيرفر يخزن هاشه فقط) ويُرسل في الرأس `x-order-token`. **أُزيل البحث برقم الهاتف**: القائمة تعرض طلبات هذا الجهاز فقط، والطلبات الأقدم من التحديث لا تظهر للعميل (التاجر يديرها كالمعتاد). متابعة الطلب من جهاز آخر تحتاج دخولاً بـOTP (غير مبني بعد).<br/>**قفل الطلب عند استلامه (SF-6):** بمجرد أن ينزّل الكاشير الطلب في السلة أو يحوّله لفاتورة ينتقل إلى «جاري التجهيز» ولا يستطيع العميل تعديله أو إلغاءه. لو فتح الكاشير الطلب ثم تراجع، يعيده التاجر لـ«قيد الانتظار» من قائمة الحالة.<br/>**رابط التتبع و«اطلب تاني» (سبتمبر 2026):** بعد الطلب يصل العميل رابط متابعة في رسالة واتساب التأكيد (ويظهر في نافذة النجاح مع زر نسخ وزر «إرسال لنفسي على واتساب»)؛ فتحه من أي جهاز يضيف الطلب لـ«طلباتي» على ذلك الجهاز — بلا حساب ولا تسجيل دخول. والطلبات المكتملة أو الملغاة فيها زر «اطلب تاني» يملأ السلة بنفس الأصناف والمقاسات بأسعار اليوم، ويبلغ العميل بأي صنف لم يعد متاحاً. |
...
| **كوبونات الخصم والعروض الترويجية** | 🟢 | 100% | `StorefrontCouponsManager.tsx`, `coupon.dto.ts`, `storefront.service.ts` | كوبونات خصم (نسبة مئوية مع سقف، مبلغ ثابت، أو شحن مجاني)، حدود استخدام، وتطبيق لحظي في الدفع.<br/>**المرحلة 3 (سبتمبر 2026 — SF-8):** حجز الاستخدام صار داخل معاملة الطلب (الطلب الفاشل لا يستهلك كوبوناً)، وإلغاء الطلب أو تعديله بدون الكوبون يعيد الاستخدام، وتعديل الطلب يعيد تقييم الكوبون بدل إسقاطه. وتخمين الأكواد محدود بـ20 محاولة/10 دقائق لكل IP.<br/>**تحصين سبتمبر 2026 (المرحلة 7):** كان عدّاد الاستخدام يُحدَّث بنمط قراءة-ثم-كتابة (`times_used = القيمة المقروءة + 1`) وفحص `usage_limit` يجري على قراءة غير مقفلة — أي **تحديث ضائع**: طلبان متزامنان يقرآن العدد نفسه ويكتبان الزيادة نفسها، فكوبون محدود بـ100 استخدام كان يمكن استهلاكه أضعاف حدّه تحت الضغط. صار الحجز **ذرياً في عبارة واحدة**: `times_used = times_used + 1` بشرط `(usage_limit IS NULL OR times_used < usage_limit)`، فتفرض قاعدة البيانات الحد على الصف الحي، ويُرفض الطلب برسالة واضحة عند صفر صفوف متأثرة (أي أن طلباً آخر التقط آخر استخدام). |
...
| **تسعير التوصيل الذكي حسب المنطقة/المحافظة** | 🟢 | 100% | `StorefrontDeliveryZonesManager.tsx`, `delivery-zone.dto.ts` | مصفوفة مناطق وشحن، قوالب جاهزة بضغطة زر (محلي وشحن محافظات)، اختيار المنطقة بدقة، وحساب الفاتورة فورياً.<br/>**تصحيح سبتمبر 2026 (O56):** تعديل العميل لطلبه كان يعيد رسوم التوصيل الثابتة ويسقط المنطقة والشحن المجاني وإعفاء الاستلام. صار الإنشاء والتعديل يستخدمان حساباً واحداً. |
...
| **استوديو وضبط صور الأصناف** | 🟢 | 100% | `StorefrontProductStudio.tsx`, `image-compressor.ts`, `storefront-media.service.ts` | ضغط الصور، قص، ورفع ومعاينة مباشرة لصور الكتالوج والتصنيفات.<br/>**تخزين الصور (سبتمبر 2026 — SF-9):** الصور (الأصناف والأقسام والبانرات) تُحفظ مرة واحدة في جدول `storefront_media` ويُرسل للمتجر رابط قصير بدل الصورة نفسها، والمتصفح يخزّن كل صورة سنة كاملة. الكتالوج صار صغيراً جداً فيفتح المتجر أسرع، وبدل شاشة التحميل تظهر كروت رمادية بشكل الصفحة. وصارت صورة الصنف تظهر في بطاقة مشاركة واتساب/فيسبوك. نُقلت الصور القديمة تلقائياً (هجرة 135). |
| **بوابات الدفع الإلكتروني المتكاملة (Tap GCC & Stripe & Paymob & XPay & Mock)** | 🟢 | 100% | `storefront-payment.service.ts`, `StorefrontPaymentGatewaysManager.tsx`, `StorefrontOnlinePaymentModal.tsx`, `storefront-public.controller.ts` | دعم متكامل وشامل لـ 5 بوابات دفع رئيسية: **تاب (Tap Payments الخليجية)** لدعم بطاقات مدى السعودية، شبكة كي نت الكويتية، بطاقات ناباس القطرية، وبطاقات بنفت، و **Stripe العالمية** لدعم كافة البطاقات الدولية و Apple Pay و Google Pay، بالإضافة إلى **Paymob** و **XPay** للمدفوعات المصرية و **Sandbox Mock**، مع تأكيد سداد آلي عبر الـ Webhooks وإشعارات واتساب فورية.<br/>**تحصين سبتمبر 2026 (البند 9 و O25):** تحصين كافة نقاط الويب هوك بالفشل المغلق التام (`UnauthorizedException`)، التحقق المشفر من التواقيع الرقمية بـ `crypto.timingSafeEqual`، حماية إعادة الإرسال (300s Replay Protection) لـ Stripe، وحماية الإدمبوتنسية لمنع ازدواجية المعالجة. كما تم تصحيح تحويل طلبات الأونلاين المسددة إلكترونياً إلى مبيعات بحيث تُسجل فوراً بحالة `collected` وتمنع مطالبة مندوب التوصيل للعميل بالدفع نقداً (COD) مرة ثانية. جناح اختبار حرج `storefront-payment-webhooks.spec.ts` بنجاح 100%.<br/>**تحصين 21 سبتمبر 2026 (SF-2 / SF-3):** مسار الدفع التجريبي `mock-pay` كان عاماً بلا أي فحص، وإصلاح «`collected` بمجرد `paid`» جعله طريقاً لتعليم أي طلب مدفوعاً واستلامه بلا تحصيل. صار المحاكي متاحاً **فقط** حين يضع التاجر البوابة في وضع الاختبار، ويتطلب توكن الطلب، ولا يُحتسب تحصيلاً أبداً (`gateway_provider='mock'`). ومع بيانات بوابة حية، فشل الاتصال بالبوابة يُظهر خطأً للعميل بدلاً من شاشة دفع وهمية. جناح `storefront-order-access.spec.ts`.<br/>**حماية مفاتيح البوابات (سبتمبر 2026 — SF-7):** شاشة الإعدادات لم تعد تعرض المفاتيح السرية كاملة؛ تظهر مقنّعة بآخر 4 أحرف فقط، وحفظ الشاشة دون تغييرها لا يمس المفتاح المخزَّن (لتغييره: امسح الحقل والصق المفتاح الجديد). والشاشة تتطلب صلاحية المتجر أو الإعدادات. |
| **لوحة إدارة الطلبات للتاجر** | 🟢 | 100% | `MerchantOnlineOrdersPage.tsx`, `ConvertDeliveryModal.tsx`, `MerchantOrderDetailModal.tsx`, `storefront-pos-loader.ts`, `storefront.service.ts` | متابعة الطلبات، الفلترة حسب الحالة، وتنزيل الطلب بنقرة واحدة في سلة الـ POS أو تحويله مباشرة لفاتورة دليفري مع مندوب.<br/>**تحصين وتطوير سبتمبر 2026:**<br/>1. **إلغاء جماعي محمي بكلمة مرور الأدمن (Bulk Cancel with Admin Gate):** زر مخصص لإلغاء كافة الطلبات المعلقة دفعة واحدة مع نافذة تأكيد عبر `StandardDialog` تطلب إدخال كلمة مرور مسؤول النظام (`role in ('admin', 'super_admin')`)، وتمنع استخدام كلمات مرور الكاشير لمنع التلاعب، مع فك حجز المخزون فورياً لكل صنف (`stock_reserved`) وإعادة تفعيل الكوبونات.<br/>2. **استعلام الأعداد التجميعي السريع (Order Counts Aggregation):** إضافة مسار `/api/storefront/admin/orders/counts` يُرجع أعداد الحالات في ~2ms واستبدال عمليات السحب الدوري في `AppShell` و`PosWorkspaceHeader` به لمنع استنزاف الذاكرة وسحب آلاف الطلبات بالكامل دورياً، مما حل بطء الشاشة الرئيسية وشريط الإشعارات نهائياً.<br/>3. **التقسيم إلى صفحات (Pagination 50/page):** دعم التصفح عبر صفحات (50 طلباً لكل صفحة) مع أزرار التنقل الذكية لتسريع فتح صفحة إدارة الطلبات مهما بلغ عدد الطلبات التاريخية.<br/>**تحصين سبتمبر 2026 (SF-3):** شارة «حالة السداد» في تفاصيل الطلب (مسدد إلكترونياً / تحويل مؤكد / دفع تجريبي غير محصّل / بانتظار تأكيد التحويل / يُحصَّل عند الاستلام)، وزر **«تأكيد استلام التحويل»** لطلبات إنستاباي والمحفظة (`POST admin/orders/:id/confirm-payment`، بصلاحية `storefront` أو `sales`). اختيار العميل لإنستاباي لم يعد يجعل الفاتورة محصّلة: بدون تأكيد التاجر يطالب المندوب بالمبلغ.<br/>**تصحيحات المرحلة 2 (سبتمبر 2026):** خصم الكوبون صار يصل فاتورة الدليفري وسلة الكاشير (كان يضيع فيحصّل المندوب الإجمالي كاملاً)، والطلبات المدفوعة بالبطاقة تُقيَّد على قناة البطاقة/البنك بعد أن كانت تُقيَّد نقداً في درج الكاشير. ملاحظة: التنزيل في سلة الكاشير قد يطلب إذن خصم أو تعديل سعر من الكاشير (O62). |
| **فلترة وتوجيه خيارات الشحن حسب وجهة الطلب (Order Shipping Destination Smart Routing)** | 🟢 | 100% | `order-shipping-destination.ts`, `MerchantOrdersTable.tsx`, `MerchantOrderDetailModal.tsx`, `storefront.service.ts` | توجيه ذكي وخاص لشركات الشحن حسب وجهة العميل: إخفاء شركات الشحن الخليجية (أرامكس وسمسا) عن الطلبات والعملاء داخل مصر مع عرض بوسطة والدليفري الداخلي حصراً، وإتاحة شركات الشحن الخليجية حصراً للطلبات المتجهة للسعودية ودول الخليج. |
...
| **الورديات والجلسات النقدية (Shifts)** | 🟢 | 100% | `sessions.service.ts`, `CashDrawerPage.tsx`, `CashDrawerFormsPanel.tsx`, `ShiftModal.tsx` | فتح الوردية برصيد افتتاحي، إغلاق الوردية، جرد الكاش الفعلي، ربط زر إغلاق الوردية في الكاشير بالبوب اب المباشر فورياً عبر `?action=close`، وحساب العجز والزيادة تلقائياً.<br/>**تحصين سبتمبر 2026:** العد الأعمى مفروض على **حمولة الـAPI** لا على الواجهة (الخادم يُصفّر الحقول المالية لدور الكاشير، وإغلاق الكاشير يُحوَّل لحالة `pending_review` تتطلب مراجعة مدير). فرق الوردية يُقيَّد فوراً بقيد مزدوج متزن داخل نفس المعاملة، ويُمنع تصافي العجز مع الزيادة (حسابان مختلفان: عجز 7200 / زيادة 7100). أُضيف مسار تسوية حقيقي: العجز **فوق حد التسامح** (`settings.cashierShortageTolerance`، افتراضه صفر) يُحمَّل كذمة مدينة على أمين الصندوق (حساب 1135) باسمه في الأستاذ المساعد بدل شطبه كمصروف، مع تسجيل `variance_resolution` و`variance_journal_entry_id` والمعتمِد (هجرة 108). كما صُحِّح تحديث ضائع في حركات الخزينة (الإدراج وإعادة الحساب والتحديث كانت خارج أي معاملة). |
...
| **البيع بدون إنترنت والمزامنة الذكية (Offline POS & Multi-Cashier Smart Caching)** | 🟢 | 100% | `usePosOfflineSync.ts`, `pos-offline-sync.ts`, `pos-catalog-storage.ts`, `usePosCatalogWarmup.ts`, `usePosCatalog.ts`, `PosOfflineQueueModal.tsx`, `lazy-route.tsx`, `CashDrawerFormsPanel.tsx` | منظومة بيع أوفلاين متكاملة: ترقيم وثائقي موحد متعدد الكاشيرات وفق الدستور مع كود طرفية مميز لكل جهاز (`INV-${terminal}-${dateStr}-${seq}`) لمنع أي تعارض، تخزين محلي دائم في IndexedDB يستوعب أكثر من 20,000 صنف، تحمية ذكية بالخلفية (Background Warmup) تسحب الأصناف تلقائياً عند الفتح وتفحص تحديثات الأسعار والأصناف عبر نبضات خفيفة للغاية (<50 بايت) بدون استهلاك رامات وبزمن قراءة 1-2ms للباركود، حماية التنقل بين الشاشات بدون إنترنت، استعادة الجلسة بأمان، نافذة إدارة ومراجعة الفواتير المعلقة، تنبيه ومنع إغلاق الوردية قبل ترحيل الفواتير المعلقة، ومزامنة ذكية تلقائية فور عودة الاتصال مع معالجة القيود المحاسبية الآمنة بتصميم مؤسسي نظيف.<br/>**تصحيح حرج سبتمبر 2026:** كان النص السابق يصف «تجديد مفاتيح الـ Idempotency» عند إعادة المحاولة كأنه ميزة — **وهو في الحقيقة كان يُنتج فواتير مكررة**: عند رد الخادم بـ409 (الطلب الأصلي ما زال قيد التنفيذ) أو 422 (حجز قديم قد يكون رُحِّل فعلاً) كانت الواجهة تولّد مفتاحاً عشوائياً جديداً وتُعيد الإرسال، فتُرحَّل الفاتورة مرتين ويُخصم المخزون مرتين ويتضاعف الإيراد. **القاعدة الآن: `sale.id` هو مفتاح الإدمبوتنسي المستقر ولا يُستبدل أبداً**؛ عند 409 تبقى الفاتورة في الطابور ويُعاد الإرسال لاحقاً **بنفس المفتاح**، وعند 422 يُستعلم الخادم عن الحالة الفعلية (`GET /api/operations/:type/:key/status`) فإن كانت مُرحَّلة تُشطب من الطابور، وإن تعذر التأكد تُعلَّم `failed` للمراجعة اليدوية بدل التخمين. |
...
| **التحويلات المخزنية بين الفروع والمستودعات** | 🟢 | 100% | `stock_transfers`, `inventory-transfer.service.ts`, `TransfersPage.tsx` | نقل بضاعة بين المخازن عبر مخزن ترانزيت حقيقي (`in_transit`) مع دورة اعتماد وتحديث الأرصدة تلقائياً.<br/>**تحصين سبتمبر 2026 (المرحلة 3):** كان `stock_transfer_items` يحمل كمية واحدة (`qty`) فقط، فكانت الوجهة **مضطرة لاستلام ما أُرسل بالضبط** ولا سبيل لتسجيل فاقد النقل — إرسال 100 واستلام 98 كان يترك الوحدتين عالقتين في مخزن الترانزيت إلى الأبد كرصيد وهمي لا يجرده أحد. أُضيفت `dispatched_qty`/`received_qty`/`variance_qty` (هجرة 114) بقيد `CHECK` يضمن `received ≤ dispatched` وأن الفرق مطابق؛ وعند الاستلام تخرج الكمية المُرسلة **كاملة** من الترانزيت، فيدخل المستلَم للوجهة ويُشطب العجز كحركة `transfer_transit_loss` **تُنقص الرصيد العام** (البضاعة غادرت المصدر ولم تصل فعلاً). |
...
| **الجرد الفعلي الدوري والمفاجئ** | 🟢 | 100% | `StockCountPage.tsx`, `inventory-count.service.ts` | جرد المخزون الفعلي ومقارنته بالدفتري واستخراج فروق الجرد وتوليد تسوية فورية. يلتقط لقطة `expected_qty` عند فتح الجلسة ويُعيد التحقق منها عند الترحيل فيرفض الترحيل إن تحرك الرصيد أثناء العد (حماية تزامن صحيحة). |
| **جرد تسوية عداد المخزون العام (Global Stock Counter Reconciliation)** | 🟢 | 100% | `2040000000115_stock_global_counter_reconciliation.ts`, `stock_reconciliation_log` | **عملية لمرة واحدة** لإصلاح الانحراف التاريخي بين `products.stock_qty` ومجموع `product_location_stock`. سببه أن مسارات سابقة (بيع السيارات، استلام أوامر الشراء) كانت تكتب أرصدة المواقع دون المساس بالعداد العام؛ المسارات أُصلحت لكن **الانحراف المتراكم لا يُشفى ذاتياً**. مصدر الحقيقة هو مجموع أرصدة المواقع شاملاً الرصيد غير المخصص — وهو نفس تعريف `trueGlobalQty` داخل `applyStockDelta` فيتطابق الطرفان بالبناء بعدها. **ضمانات الأمان:** كل تصحيح يُسجَّل في `stock_reconciliation_log` قبل تطبيقه · الأصناف التي لها رصيد عام بلا أي صف موقع **تُحفَظ على الرصيد غير المخصص ولا تُصفَّر** (الحذف كان سيكون إتلافاً) · تأكيد بعدي يرمي استثناءً إن بقي أي انحراف · `down()` يستعيد الأرصدة السابقة من السجل · وإعادة التشغيل لا تُصحِّح شيئاً إضافياً بعد التطابق. |
...
| **توليد وطباعة ملصقات الباركود والرف التموينية وبطاقة بيان السلع (Barcode & Regulatory Shelf Label Printing Engine)** | 🟢 | 100% | `BarcodePrintDialog.tsx`, `barcode-labels.ts`, `internal-detail-review.css`, `ProductsWorkspace.tsx`, `ProductsTableCard.tsx`, `PurchaseDetailCard.tsx` | نافذة توليد وطباعة ملصقات الباركود والرفوف المتطورة المتوافقة مع معيار ملاءمة الشاشات (Zero-Scroll Standard)؛ دعم متكامل لكافة طابعات الرول الحراري وشيتات الـ A4 اللاصقة، إدراج استيكرات الرفوف التموينية القانونية (60×30 مم و 70×35 مم) بخطوط عريضة للسعر وعبارة "شامل الضريبة المضافة" وفق القرار الوزاري 330 لسنة 2017، بطاقة بيان السلع الغذائية والمعبأة المكتملة قانونياً لحماية المتاجر والسوبرماركت ومحامص العطارة من محاضر التموين والسلع مجهولة المصدر: (تاريخ الإنتاج، تاريخ التعبئة، فترة الصلاحية بالشهور أو تاريخ انتهاء صريح، بيان الوزن الصافي الرسمي "صافي: X" وفق المواصفة القياسية المصرية 2613 لسنة 2008، وشروط وتعليمات التخزين بقوالب سريعة للأغذية الجافة والمبردة والمجمدة)، ميزة الطباعة المجمعة للأصناف المحددة من جدول المنتجات بضغطة زر مع ضبط كميات النسخ، والطباعة المباشرة لملصقات باركود كافة بنود فواتير المشتريات بالكميات المستلمة أو المطلوبة فورياً. |
...
* **الجداول في قاعدة البيانات:** `purchases`, `purchase_items`, `purchase_returns`, `suppliers`, `supplier_transactions`, `goods_receipt_notes`, `goods_receipt_lines`, `purchase_three_way_matches`
...
| **أذون الاستلام المخزني والمطابقة الثلاثية وحساب وسيط التوريدات (Goods Receipt Notes, 3-Way Matching & GRNI Settlement Engine)** | 🟢 | 100% | `goods-receipt.service.ts`, `goods-receipt.controller.ts`, `three-way-match.engine.ts`, `GoodsReceiptModal.tsx`, `ThreeWayMatchModal.tsx`, `GoodsReceiptsListModal.tsx`, `2040000000106_goods_receipt_notes_and_three_way_matching.ts` | منظومة استلام مخزني ومطابقة ثلاثية قياسية مطابقة لمعايير SAP ERP و Oracle NetSuite؛ إنشاء أذون استلام مخزني رسمية بترقيم موحد (`GRN-YYMMDD-XXXX`)، فحص الجودة وفصل الكميات المقبولة عن المرفوضة مع تسجيل أسباب الرفض والتشغيلات (Batch #) وتواريخ الصلاحية، توليد قيد وسيط التوريد غير المفوتر GRNI آلياً (من حـ/ المخزون 1140 إلى حـ/ بضاعة مستلمة غير مفوترة 2125)، محرك المطابقة الثلاثية الصارم (Three-Way Match) بين أمر الشراء (PO) وإذن الاستلام (GRN) وفاتورة المورد، فرض ثوابت الحظر (MATCH-1: حظر الفوترة الزائدة عن المستلم، MATCH-2: سقف التسامح السعري 2% وترحيل الفروق لحساب PPV 5190، MATCH-3: استثناء البنود الخدمية بشرط شهادة الإنجاز)، وتسوية قيد الفاتورة التراكمي وتصفير الفارق الحسابي بدقة 0.000 ج.م مع دعم التجاوز الإداري المبرر من المدير المالي.<br/>**تحصين سبتمبر 2026 (المرحلة 2 من التدقيق):** كانت الثوابت معلنة وغير فاعلة فعلياً — صُحِّح الآتي: (1) `historicalInvoicedLines` لم يكن يُمرَّر للمحرك فكان MATCH-1 معطلاً وأمكن فوترة نفس الشحنة مراراً؛ (2) أعمدة الربط `po_id`/`po_item_id` لم تكن موجودة في المخطط فكان ضلع أمر الشراء (MATCH-2) ميتاً بنيوياً وأُضيفت في هجرة 113؛ (3) دفتر استهلاك داخل الفاتورة الواحدة يمنع تجاوز الحد بتقسيم البند لسطرين؛ (4) التجاوز صار **مقيَّد النطاق** (`price_only` لا يُسقط حجب الكمية، و`full` يتطلب صلاحية إدارة وسبباً ≥ 10 أحرف) بدلاً من مفتاح واحد يُسقط كل الحواجز بنص حر؛ (5) المطابقة أصبحت **بوابة تحجب سداد الموردين** فعلياً بعد أن كانت وسماً استشارياً لا يمنع ترحيلاً ولا صرفاً؛ (6) فارق التسوية صار يحجب الترحيل بعد أن كان يُحسب ويُهمل؛ (7) ضريبة القيمة المضافة تُحتسب لكل سطر بمعدله لدعم الفواتير مختلطة المعدلات؛ (8) حجب الزيادة السعرية فقط دون الفرق المواتي للمشتري. **الحارس:** `test/critical/purchases-three-way-match.spec.ts`. |
...
| **خطط وجداول أقساط العملاء (Customer Installment Plans)** | 🟢 | 100% | `customer-installments.service.ts`, `customer_installment_plans`, `customer_installments` | إنشاء خطط الأقساط الشهرية، احتساب الفائدة، امتصاص كسور السنتات في القسط الأخير لضمان مطابقة الإجمالي بدقة 100%، وتوليد ترقيم موحد `INST-YYMMDD-XXXX`. |
| **سداد الأقساط والربط المحاسبي الكامل (Installments Payments & GL Sync)** | 🟢 | 100% | `customer-installments.service.ts`, `customer-installments.controller.ts`, `sales-finance.service.ts`, `accounting-posting.service.ts` | سداد الأقساط بقفل تشاؤمي `forUpdate()` لمنع التحصيل الزائد، مع خصم رصيد العميل في الأستاذ المساعد، إثبات حركة الخزينة، وترحيل قيد متوازن للأستاذ العام برقم إيصال موحد `REC-YYMMDD-XXXX`.<br/>**تحصين سبتمبر 2026 (البند 10 و O21 — نفذه Antigravity):** تحصين `CustomerInstallmentsController` بـ`PermissionsGuard` وفرض صلاحية `@RequireAnyPermission('sales', 'customers')` على كافة مسارات خطط وجداول ودفعات الأقساط لمنع أي مستخدم غير مصرح له من إدارة الأقساط أو تسجيل تحصيلات نقدية. |
| **إدارة علاقات العملاء والفرص البيعية (CRM Deals & Pipeline)** | 🟢 | 100% | `crm.service.ts`, `crm_deals`, `crm_activities` | متابعة مراحل الفرص البيعية وحساب القيمة الوزنية ومعدل التحويل بدقة، مع تحويل ذري آمن للفرصة إلى عميل مسجل بالدفاتر. |
...
| **بيع السيارات المتنقلة ومخزون المندوب (Van Sales & Mobile Warehouse)** | 🟢 | 100% | `van-sales.service.ts`, `location-stock-ledger.ts`, `accounting-posting.service.ts` | تحميل سيارة المندوب من المستودع، البيع الميداني (نقدي وآجل)، التحصيل الميداني من عملاء مدينين، مرتجعات العملاء، وتصفية الرحلة بإعادة غير المُباع وتسوية عجز/زيادة الكاش.<br/>**تحصين سبتمبر 2026 (المرحلة 3):** كانت الوحدة تكتب `product_location_stock` بعبارات `qty + n` / `qty - n` مباشرة **ولا تمس `products.stock_qty` ولا `stock_movements` إطلاقاً**. حُوِّلت كل حركات المخزون (7 مواضع) للمرور عبر `applyStockDelta` بدالة موحدة `moveVanStock`.<br/>**تصحيح لما سبق (26 سبتمبر 2026):** المراجعة أعلاه لم تُشغّل الوحدة فعلياً ضد قاعدة بيانات حقيقية، فلم تكتشف أن **الميزة كانت معطوبة بالكامل منذ إنشائها ولم تُنفَّذ عملية بيع ميداني واحدة بنجاح قط**: `executeFieldSale` كان يكتب لعمودين غير موجودين أصلاً (`sales.payment_method`/`sale_items.total_price`/`sales.notes` — الأسماء الحقيقية `payment_type`/`line_total`/`note`)، و`getOrCreateVanLocation` كان يدرج `location_type='van_stock'` وهي قيمة يرفضها قيد `stock_locations_type_valid` (لم يُحدَّث عند بناء الميزة) — أي أن أول إجراء يقوم به أي مندوب (فتح رحلة) كان يفشل قبل الوصول لعطل البيع أصلاً. اكتُشف هذا فقط بكتابة اختبار تكاملي حقيقي (`test/e2e/van-sales-field-flow.e2e.ts`) ينفّذ التدفق كاملاً ضد Postgres فعلي، بعدما تبيّن أن حارس الوحدة الوحيد (`phase8-logistics.spec.ts`) يعيد كتابة نفس الحسابات داخل ملف الاختبار ولا ينادي الخدمة الحقيقية إطلاقاً. **صُلح كل ذلك** (هجرتان 151 و152)، **وربطت المبيعات الميدانية بالمحرك المحاسبي لأول مرة**: `executeFieldSale` يستدعي `postSale` (بنفس مقاومة الفشل المؤجَّل لـ`SalesWriteService.createSale`) ويسجّل `status='posted'` بدل `'completed'` الوهمية التي كانت تُخفي كل إيراد المناديب عن كل تقرير مالي وضريبي يُرشِّح بـ`status='posted'`؛ ودالتان جديدتان `postVanFieldCollection`/`postVanTripSettlement` يعاملان نقدية المندوب كعهدة (ذمة "بلا شريك") لا تتحول لخزينة حقيقية إلا عند تصفية الرحلة، مع تسجيل العجز/الزيادة صراحة بدل ابتلاعه. **واكتُشف خلال البناء عطل ثالث أعمق**: التحميل والتفريغ (نقل بين موقعين بـ`skipGlobalUpdate:true` على الساقين) كانا يُضخِّمان رصيد الوجهة بمقدار الكمية المنقولة في كل رحلة — لأن `applyStockDelta` يعيد حساب الرصيد العام من مجموع المواقع الحقيقي في كل نداء، فتجاهل الكتابة في الساق الأولى يترك الرصيد العام غير متزامن مع مجموع المواقع وقت الساق الثانية، ومنطق "الرصيد غير المخصص" الذاتي الإصلاح يُفسِّر الفارق العابر خطأً كمخزون حقيقي غير مُتتبَّع ويضخّه في أي موقع يُكتب بعده. صار الآن كلا الساقين يكتبان الرصيد العام فعلياً (`skipGlobalUpdate:false`) فيتصالحان لنفس الإجمالي الصحيح. كما رُتِّبت الأصناف تصاعدياً بمعرّف الصنف قبل حلقات المخزون الثلاث (تحميل/بيع/تفريغ) اتساقاً مع بقية النظام لمنع تعليق متبادل بين رحلتين متزامنتين. |
...
| الميزة التفصيلية | الحالة | نسبة الإنجاز | ملفات التنفيذ الأساسية | الشرح وملاحظات العمل |
| :--- | :---: | :---: | :--- | :--- |
| **دليل الحسابات الشجري (Chart of Accounts)** | 🟢 | 100% | `accounting.service.ts`, `ChartOfAccountsPage.tsx` | أصول، خصوم، حقوق ملكية، إيرادات، ومصروفات بتسلسل محاسبي معتمد.<br/>**تحصين سبتمبر 2026 (المرحلة 1):** فحص «هل على الحساب قيود محاسبية؟» في `updateAccount` و`deleteAccount` كان يستعلم `journal_entry_lines` **بلا فلتر `tenant_id`** — مخالفة لدستور العزل. أُضيف الفلتر. (يبقى الحارس الصحيح القائم: تغيير `account_type` و`normal_balance` ممنوع بعد وجود حركات.) |
| **عكس وإلغاء القيود اليومية (Journal Entry Reversal)** | 🟢 | 100% | `accounting.service.ts:reverseJournalEntry` | إنشاء قيد عكسي مرآتي (تبديل المدين والدائن) وربطه بالأصل عبر `reversal_of_entry_id`/`reversed_by_entry_id` مع إلزام سبب مكتوب.<br/>**تحصين سبتمبر 2026 (المرحلة 1):** (1) كانت حراس «مُرحَّل» و«سبق عكسه» تُفحص على قراءة **غير مقفلة وخارج المعاملة**، فطلبان متزامنان يُنشئان قيدين عكسيين ويتضاعف أثر العكس على الدفتر — أُضيفت قراءة مقفلة (`forUpdate`) داخل المعاملة مع إعادة فحص الحالتين؛ (2) كان العكس **يتجاوز قفل الفترة المحاسبية تماماً** لأنه يُدرج مباشرة في `journal_entries` بلا أي فحص، فأمكن العكس داخل سنة مقفلة وإعادة كتابة أرصدتها بعد اعتمادها — أُضيف `assertPeriodOpenForReversal` بنفس منطق المقارنة المستخدم في `insertPostedJournal`. |
...
| **محرك تقييم العملات الأجنبية وأرباح وخسائر فروق الصرف IAS 21 (Multi-Currency FX Revaluation Engine)** | 🟢 | 100% | `forex-revaluation.service.ts`, `accounting.controller.ts`, `AccountingForexRevaluationPage.tsx`, `forex.api.ts`, `2040000000073_forex_revaluation_engine.ts` | معيار المحاسبة الدولي IAS 21 لإعادة تقييم الحسابات والأرصدة بالعملات الأجنبية في نهاية كل فترة مالية عبر المسار `/accounting/forex`؛ قراءة فورية لأرصدة الحسابات النقدية والبنكية وحسابات العملاء والموردين بالعملة الأجنبية ومقارنة سعر الصرف الدفتري مع سعر الإقفال الحالي، واحتساب فروق الصرف غير المحققة (Unrealized FX Gain/Loss)، وتوليد وترحيل قيد تسوية آلي متزن في الأستاذ العام (حـ/ أرباح أو خسائر فروق العملة 4920/5920) وحفظ تقرير التدقيق وسجل الدورات.<br/>**⚠️ تصحيح حرج سبتمبر 2026 (المرحلة 1):** كانت الوحدة **معطلة بالكامل وترمي خطأ عند كل استدعاء** — الاستعلام كان يشير إلى عمودَي `name` و`type` على `accounting_accounts` (الأعمدة الحقيقية `name_ar`/`name_en` و`account_type`) وإلى جدول `treasuries` **غير الموجود في المخطط إطلاقاً**، وأخفى `(this.db as any)` الأخطاء الثلاثة عن المترجم. كذلك كان اختيار الحسابات بمطابقة أسماء نصية (`%دولار%`) وهو ما **لا يميّز البند النقدي من غير النقدي** فكان حساب مثل «مخزون بضاعة USD» سيُعاد تقييمه — وهو خطأ جوهري لأن IAS 21 §23 يوجب إعادة الترجمة للبنود النقدية فقط. **الحالة الآن:** أُضيف عمودا `is_monetary` و`revaluation_currency` (هجرة 112) مع تعبئة أولية من `is_cash_bank`/`is_receivable`/`is_payable` واستثناء المخزون صراحةً، وصار الاختيار صريحاً بدل التخمين النصي. **بند مفتوح (O1):** `revaluation_currency` يبدأ فارغاً فلن تلتقط الوحدة أي حساب حتى تُحدَّد عملة كل حساب نقدي أجنبي — سلوك آمن يحتاج شاشة تهيئة. |
| **الفترات المحاسبية الشهرية وإقفال الشهور (Monthly Fiscal Periods & Closing — O6)** | 🟢 | 100% | `fiscal-year.service.ts`, `fiscal-period.dto.ts`, `accounting-posting.service.ts`, `accounting.service.ts`, `FiscalPeriodsModal.tsx`, `AccountingFiscalYearsPage.tsx`, `2040000000143_accounting_fiscal_periods.ts` | نظام الفترات المحاسبية الشهرية المعتمدة مؤسسياً (12 فترة للسنة المالية) لتمكين الإدارة المالية من إقفال وتجميد الدفاتر شهرياً بعد مراجعة الحسابات وتقديم الإقرارات الضريبية، وحظر إضافة أو تعديل أي حركة مالية في فترة مقفلة بأثر رجعي. تقسيم تلقائي لـ 12 شهراً، إقفال زمني متسلسل (لا تُقفل الفترة N إلا بعد إقفال N-1)، حظر الإقفال مع وجود قيود معلقة في المسودة، حظر ترحيل أي قيد يدوي أو تلقائي في فترة مقفلة بكود `ACCOUNTING_PERIOD_LOCKED` (400)، وإعادة فتح عكسية مشروطة بوجود سبب تدقيقي إلزامي، ونافذة منبثقة متكاملة لإدارة الفترات وإقفالها. |
...
| **حركات الحضور والانصراف والغياب والحصانة الزمنية الدولية (Attendance, Anti-Tampering & Multi-Tenant Timezone)** | 🟢 | 100% | `hr.service.ts`, `mobile-attendance.service.ts`, `tenant-timezone.util.ts`, `quick-attendance-shortcut.tsx`, `DailyAttendanceTable.tsx`, `attendance-punch.engine.ts`, `test/critical/tenant-timezone-attendance.spec.ts` | تسجيل ومتابعة الحضور والانصراف والورديات المتعددة واستثناءات التأخير والأوفر تايم؛ **حصانة تامة ضد التلاعب بساعة الأجهزة**: فرض توقيت السيرفر الرسمي (`useServerTime: true` و `punchAction: check_in / check_out`) وتجريد العميل من حق فرض توقيت جهازه في التسجيل السريع وبصمة الموبايل. **تحصين سبتمبر 2026 (المرحلة 15):** الحصانة كانت تشمل **لحظة** البصمة فقط بينما `work_date` — اليوم الذي تُقيَّد عليه البصمة وما يقرؤه مسيّر الرواتب — ظل يأتي من العميل ويأخذ الأولوية، فكان عميل معدَّل يسجّل بصمة بتوقيت سيرفر صحيح على أي يوم يختاره. صار الحسم كله في محرك نقي واحد `attendance-punch.engine.ts` يُسقط أي `workDate` مرسل من العميل متى كانت البصمة مثبّتة بوقت السيرفر، ويمنع تحويل طلب الانصراف إلى حضور جديد، ويجري مسار البصمة كله داخل معاملة واحدة بقفل على صف اليوم لمنع ازدواج البصمة أو محو وردية مكتملة عند النقر المزدوج؛ **محرك المناطق الزمنية الذكي للمستأجرين (Dynamic Tenant Timezone Engine)**: قراءة `timezone` من إعدادات كل منشأة (`settings.timezone` مع كاش سريع 5 دقائق) لتحديد تاريخ العمل `work_date` والوقت المعروض بدقة بلد المشترك (الكويت `Asia/Kuwait`، السعودية `Asia/Riyadh`، الإمارات `Asia/Dubai`، مصر `Africa/Cairo`) مع الحماية من انزلاق تاريخ اليوم عبر حدود منتصف الليل. |
| **مسير الرواتب والسلف والخصومات** | 🟢 | 100% | `payroll_runs`, `hr.service.ts`, `PayrollPage.tsx`, `hr-treasury` | احتساب المرتبات شهرياً، استقطاع السلف والغياب، وصرف الرواتب وترحيلها للخزينة والمصروفات.<br/>**⚠️ تصحيح مالي حرج سبتمبر 2026 (المرحلة 6):** كان قسط القرض يُسجَّل **بكامل قيمته المجدولة** في `loan_deduction_amount` ثم يُقصَّ صافي الأجر إلى صفر عند عدم الكفاية (`Math.max(0, rawNetPay)`) — بينما `settlePayrollLoanDeductions` يُرصِّد أقساط القرض بذلك المبلغ الكامل. النتيجة: **الشركة تُسقط من رصيد القرض مبلغاً لم تستقطعه فعلياً من الموظف** (مثال: إجمالي 5,000 وخصومات 4,500 وقسط 1,000 → الموظف يقبض صفراً، والمتاح للقسط 500 فقط، ومع ذلك كان القرض يُرصَّد 1,000 فتضيع 500 على الشركة في كل حالة). **الحالة الآن:** أولوية خصومات صريحة — القسط يأخذ المتاح فقط `min(المجدول، الإجمالي − الخصومات)`، ويُحفظ المستقطع **الفعلي** لا المجدول، والفرق يُؤجَّل بملاحظة موثقة على بند المسير. طُبِّق على مساري الاحتساب (البناء الكامل وإعادة الاحتساب اليدوي).<br/>**⚠️ تصحيح مالي حرج ثانٍ سبتمبر 2026 (المرحلة 17):** توزيع الخصم على أقساط القرض كان يعيد قراءة المبلغ المسدَّد من قاعدة البيانات في كل دورة بينما يستخدم **رصيد القرض من لقطة الاستعلام الأولى**. فإذا غطّى خصم شهر واحد قسطين من نفس القرض (موظف فاته شهر)، ينتهي المسدَّد والمتبقي غير متصالحَين مع أصل القرض، ويظهر القرض **مديناً بعد سداده بالكامل فيُخصم من راتب الشهر التالي مرة أخرى**، كما يفقد سقف الخصم معناه فيسمح باستقطاع أكثر مما على القرض فعلاً. **الحالة الآن:** التوزيع في محرك نقي `payroll-loan-allocation.engine.ts` يقرأ رصيد القرض حديثاً في كل دورة، وحارس تطابق الأرصدة الذي كان موجوداً أصلاً — وكانت اللقطة القديمة تمتص الفارق فتجعله غير قابل للوصول — صار يعمل فعلاً ويرفض البيانات غير المتصالحة بصوت عالٍ بدل تخزين أرصدة فاسدة. |
...
| **إدارة العهد العينية والنقدية للموظفين (Employee Custody & Assets)** | 🟢 | 100% | `HrAssetsPage.tsx`, `hr.service.ts`, `hr_employee_assets`, `employee-portal.service.ts` | نظام كامل لإدارة العهد العينية (سيارات، لابتوبات، هواتف، أجهزة) والنقدية، وتتبع تسليمها واسترجاعها، مع إثبات تلقائي للخصومات المالية على راتب الموظف عند التلف أو الفقدان، وربطها الصارم بإخلاء الطرف وتصفية مستحقات نهاية الخدمة (`endOfService`) وبوابة الموظف الذاتية (`EmployeePortalPage`). **تحديث (بند O2):** سجلات العهد كانت تُكتب بلا نطاق منشأة فكان حارس إخلاء الطرف لا يراها ولا يحجب أبداً؛ صارت تُكتب بالنطاق، وتعديل حالة العهدة صار مقيّداً بمنشأتها (هجرة 127 للترميم). |
| **مخالصات ومستحقات نهاية الخدمة والربط المحاسبي وحارس العهد (HR End of Service Settlement & Accounting Integration)** | 🟢 | 100% | `end-of-service.service.ts`, `accounting-posting.service.ts`, `EndOfServicePage.tsx`, `2040000000117_hr_end_of_service_uniqueness_and_indexes.ts` | احتساب مكافأة نهاية الخدمة آلياً وفق قانون العمل السعودي (المادتان 84 و85 بدقة الكسور الصريحة لمنع فاقد التقريب) وقانون العمل المصري (المادة 125)، مع احتساب بدل رصيد الإجازات وراتب الشهر المعلق وبدل الإنذار؛ توليد قيد اليومية الآلي المتوازن 100% في الأستاذ العام عبر `postEndOfServiceSettlement` مع تطبيق سقف امتصاص الاستقطاعات (Absorption Invariant)؛ التوجيه السليم لحسابات المصروفات (6200) وسلف الموظفين (1160) وإيرادات استرداد العهد والجزاءات (7100)؛ تحديث فوري وإقفال لأرصدة السلف والأقساط المستحقة في `hr_employee_loans` ودفتر الأستاذ المساعد `hr_employee_ledger`؛ حارس رقابي إلزامي لحجب صرف المخالصة في حال وجود عهد نشطة إلا بوجود إخلاء طرف معتمد أو استقطاع قيمتها (**كان معطلاً فعلياً حتى بند O2 لأن سجلات العهد كانت بلا نطاق منشأة — صار نافذاً الآن، وشاشة المعاينة تقرأ بنفس النطاق الذي يقرأ به الحارس**)؛ ترقيم تسلسلي قياسي حصين عبر المعرف الفريد بدون `COUNT(*)`؛ مؤشر تفرد جزئي يمنع ازدواج القيود برقم هجرة 117. |
...
## 10. الصيانة وأوامر الشغل وتذاكر الأجهزة (Maintenance & Repairs)
* **حالة الوحدة العامة:** 🟢 مكتمل 100%
* **مسارات الكود:** `backend/src/modules/maintenance`, `frontend/src/features/maintenance`
* **الجداول في قاعدة البيانات:** `maintenance_tickets`, `maintenance_ticket_parts`, `maintenance_ticket_history`

| الميزة التفصيلية | الحالة | نسبة الإنجاز | ملفات التنفيذ الأساسية | الشرح وملاحظات العمل |
| :--- | :---: | :---: | :--- | :--- |
| **استلام الأجهزة وتذاكر الصيانة والترقيم الذري** | 🟢 | 100% | `maintenance.service.ts`, `MaintenancePage.tsx` | فتح كارت/تذكرة صيانة، فحص الجهاز، تسجيل المشكلة، وتكلفة تقريبية مع باركود للجهاز.<br/>**تحصين سبتمبر 2026 (البند 12 — نفذه Antigravity):** القضاء التام على سباق تكرار أرقام التذاكر `COUNT(*)+1` عبر التحول للترقيم الذري بإدراج رقم مؤقت `TMP-ZM-...` ثم التسمية بالمعرف الحقيقي عبر المحرك القياسي الموحد `formatDailyDocumentNumber('ZM', ticketId)` داخل المعاملة الذرية، مع حماية المسارات بـ`PermissionsGuard` و`@RequireFeature('maintenance')`. |
| **تتبع مراحل الصيانة واستهلاك قطع الغيار** | 🟢 | 100% | `maintenance_ticket_parts`, `MaintenanceOrderModal.tsx` | ربط قطع الغيار المستخدمة من المخزن وخصمها من الرصيد وحساب أجر اليد. |
...
* **حالة الوحدة العامة:** 🟢 مكتمل 100% · **مُدقَّق بالعمق (البند 3 — سبتمبر 2026):** قراءة كاملة سطراً بسطر لـ`saas-admin` و`tenant-subscription` و`activation`، أُصلحت خلالها **9 ثغرات** أبرزها تراكم فترات الاشتراك عند إعادة إرسال إشعار الدفع، وكلمة مرور لوحة المطوّرين المنشورة في الكود، واجتياز مسار في رفع ملفات التشخيص. التفاصيل في `ARCHITECTURE_INVARIANTS.md` §5-ب (المرحلة 14). **بنود مفتوحة مرتبطة:** O32 (تخفيض الباقة لا يُطفئ الموديولات)، O34 (انتحال الشخصية بلا أثر تدقيق مميَّز)، O37 (ترقية ذاتية عبر مسار المحاكاة)
...
| **هندسة الباقات الأربعة المعتمدة (4-Tier SaaS Matrix)** | 🟢 | 100% | `saas-admin.service.ts`, `plan-feature.service.ts`, `TenantSubscriptionPage.tsx`, `DeveloperActivationPanel.tsx`, `2040000000035_align_plans_matrix_and_modular_tiers.ts` | هيكل تسعير واشتراكات متدرج ومحكم بدقة: <br>1. **الأساسية (Basic):** 4 ميزات فقط (المبيعات ونقاط البيع POS، المنتجات والأصناف، ورديات العمل والكاشير، وصندوق الكاشير والخزينة والمصروفات). <br>2. **الاحترافية (Pro):** 7 ميزات (ما سبق في الأساسية + إدارة المشتريات والموردين، المخزون المتقدم والجرد، والتقارير المتقدمة وسجل النشاط). <br>3. **المتكاملة (Ultimate ERP):** 21 ميزة (ما سبق في الاحترافية + شجرة الحسابات، الأصول الثابتة، التقسيط، الفاتورة الإلكترونية، الإقرار الضريبي، الموارد البشرية، المناديب، نقاط الولاء، الصيانة وسيريال IMEI، الملابس، المطاعم، التصنيع، الاستيراد، والصيدليات - **بدون المتجر الإلكتروني**). <br>4. **التجارة الشاملة (Omnichannel Enterprise):** 22 ميزة بالكامل (كافة ميزات المتكاملة + المتجر الإلكتروني وطلبات الأونلاين وبوابات الدفع الإلكتروني Paymob/XPay/Stripe). |
...
| **النسخ الاحتياطي والاسترجاع الآمن (Backup & Restore)** | 🟢 | 95% | `settings-backup.service.ts`, `SettingsBackupImportSection.tsx`, `BACKUP_RESTORE.md` | أخذ نسخة احتياطية مشفرة بضغطة زر، استرجاع فوري للنسخ من ملفات ZIP/JSON وسجل اللقطات السريعة؛ تم فك القفل في نسختي الإلكترون والدفلبر للمدير المعتمد (`admin` / `super_admin` مع صلاحية `canManageBackups`) مع الحفاظ على الحماية المشددة بطلب كتابة تأكيد الاستعادة `RESTORE BACKUP`.<br/>**تحصين 22 سبتمبر 2026 (O65، RESTORE-1..3):** الاسترجاع كان يكسر ربط القيود بدليل الحسابات وروابط الفروع والأقساط بصمت. صار يعيد كل صف بمعرّفه الأصلي حين يمكن، ويفحص كل المفاتيح الأجنبية قبل الحفظ، ويرفض ويرجع بلا أي تغيير إن بقي سطر يتيم. الصلاحية صارت `canManageBackups` صراحةً (مثل الواجهة). **حد معروف:** نسخة من قاعدة بيانات أخرى تُرفض غالباً بدلاً من أن تُكسر البيانات. جناحا `backup-restore-integrity.spec.ts` و`settings-backup-safety.spec.ts`. |
| **نقل المنشأة بين السحابة والديسكتوب (Cloud ⇄ Desktop Tenant Transfer)** | 🟢 | 90% | `core/tenant-transfer/*`, `tools/zs-backup-tool.ts`, `tenant-transfer.service.ts`, `TenantTransferCard.tsx`, `zsystems-backup.sh`, `zsystems-restore.sh`, هجرة 138 | تصدير كل بيانات المنشأة في ملف واحد (`.zsbak`) واستيرادها في نسخة الديسكتوب لتعمل بدون إنترنت، ثم إعادتها للسحابة كما هي. كل سجل ينتقل برقمه الأصلي، والسجلات الجديدة أوفلاين تاخد نطاق أرقام محجوز للمنشأة. الباك أب الليلي بقى ملف واحد مجمّع ومشفّر (نسخة السيرفر كاملة + حزمة جاهزة لكل منشأة) على Google Drive وأوراكل، والاسترجاع منه مباشرة للسيرفر كله أو لمنشأة واحدة. الشاشة في صفحة النسخ الاحتياطي، وبتقبل حزمة منشأة أو الملف المجمّع وتختار منه المنشأة. أي خلل في سلامة البيانات بيلغي الاستيراد كله من غير أي تغيير. **الناقص:** نسخة احتياطية تلقائية دورية على جهاز المحل وهو أونلاين، وقفل "قراءة بس" على السحابة أثناء العمل أوفلاين (يدوي حالياً: إيقاف المنشأة من لوحة المنصة). جناح `tenant-transfer.spec.ts` (TRANSFER-1..5) |
...
> **الأسعار والتحزيم ليست هنا.** المرجع الوحيد للحقيقة في الأسعار والباقات والمنتجات
> القطاعية هو **[`PRICING_AND_PACKAGING.md`](./PRICING_AND_PACKAGING.md)**.
> التسعير القديم (الباقات الأربعة 3,500 / 7,500 / 15,000 / 24,000 ج.م) **مُبطَل بالكامل**
...
| الميزة التفصيلية | الحالة | نسبة الإنجاز | ملفات التنفيذ الأساسية | الشرح وملاحظات العمل |
| :--- | :---: | :---: | :--- | :--- |
| **بوابة التحقق قبل النشر (CI Gate Integration)** | 🟢 | 100% | `deploy-oracle.yml`, `ci.yml` | ربط سير عمل النشر بانتهاء واجتياز خط الـ CI بنجاح (`workflow_run` مع `conclusion: success`) لمنع نشر أي كود غير مجتاز للفحوصات أو الاختبارات على سيرفر الإنتاج.<br/>**توسعة سبتمبر 2026 — توصيل شبكة الحُرّاس:** كان خط الـCI يشغّل البناء وفحوص الجاهزية والـE2E فقط، بينما **الـ69 جناح اختبار** (`test/infra` + `test/critical` — الثوابت المالية، عزل المستأجرين، رموز البوابات، الإدمبوتنسي، المطابقة الثلاثية، مستخلصات المقاولات) وكل فحوص معمار الواجهة كانت تُشغَّل يدوياً فقط، أي عملياً لا تُشغَّل. صارت الآن مفروضة آلياً على كل Push/PR عبر `npm run test:infra` في وظيفة الباك إند و`qa:functional`/`qa:api`/`qa:contracts`/`qa:routes`/`qa:composition`/`qa:architecture` في وظيفة الواجهة، مع أمر محلي موحّد `npm run guards` مطابق تماماً لما يشغّله الـCI. خمسة من فحوص الواجهة حُوِّلت لنمط **السقّاطة (Ratchet)** (`frontend/scripts/architecture-baseline.mjs` + `scripts/baselines/*.json`) بحيث تفشل على أي مخالفة **جديدة** فقط بدل الفشل الدائم على الدين التاريخي. التفاصيل في `ARCHITECTURE_INVARIANTS.md` §6-ج.<br/>**استكمال (المرحلة الثانية):** أجنحة الواجهة نفسها (Vitest) كانت **18 اختباراً فاشلاً في 9 ملفات** وقت التوصيل، فصُفِّرت بالكامل إلى **246 اختباراً في 55 ملفاً (100% نجاح)** ووُصِّلت هي أيضاً بالـCI عبر `npm run test:run`. التصفير كشف ثلاثة أعطال حقيقية في الإنتاج لا في الاختبارات: انحدار صلاحيات يجعل دور `admin` يتخطى كل القيود في الواجهة (البند O29، أُصلح)، وثلاثة عناوين حقول في شاشة العروض كانت تعرض للمستخدم نص الكود `${getGlobalCurrencySymbol()}` حرفياً بدل رمز العملة (البند O31)، ودالة ميتة كانت تُفشِل بناء الواجهة على الفرع الرئيسي. **المتبقي خارج البوابة:** `frontend npm run lint` وحده (22,451 ملاحظة) — بند مستقل. |
...
| **موجات التسعير الجماعية وقواعد التسعير التلقائية (Bulk Pricing Waves & Rules)** | 🟢 | 100% | `pricing.service.ts`, `pricing_rules`, `price_change_runs`, `price_change_items` | معاينة وتطبيق موجات التسعير بنسب مئوية أو مبالغ ثابتة أو هوامش من التكلفة، مع دعم التقريب (nearest/ending)، وتطبيق الترتيب القانوني للأقفال لمنع الـ Deadlocks مع إمكانية التراجع الذري الآمن بقفل تشاؤمي. |
...
| الميزة التفصيلية | الحالة | نسبة الإنجاز | ملفات التنفيذ الأساسية | الشرح وملاحظات العمل |
| :--- | :---: | :---: | :--- | :--- |
| **رصد وتتبع الأنماط المشبوهة في نقاط البيع (Suspicious POS Telemetry)** | 🟢 | 100% | `sales-write.service.ts`, `cashier-fraud-radar.service.ts` | التقاط فوري لعمليات: (1) حذف الأصناف من السلة بعد مسحها بالباركود (`cart_remove`)، (2) إلغاء الفواتير المسودة قبل الحفظ (`draft_cancel`)، (3) الفواتير الملغاة رسمياً (`sale_cancelled`)، (4) تجاوزات الخصومات اليدوية (`discount_override`)، مع تسجيل اسم الصنف وقيمته وسببه.<br/>**تحصين سبتمبر 2026:** كان الرادار يكشف الأحداث بـ**مطابقة نصوص عربية** على عمود `action` (`LIKE '%خصم%'`, `'%حذف عنصر من السلة%'`) في ثلاثة مواضع (`getSummary`, `getEvents`, `checkAndAlertAnomaly`) — أي أن **إعادة صياغة رسالة سجل تدقيق واحدة كانت تُعمي الرادار بصمت**. أُضيف عمود `audit_logs.event_code` مع ثوابت `AUDIT_EVENT_CODES` ودالة تصنيف موحدة `classifyRadarEvent` (هجرة 109)، وتم **ترحيل السجلات التاريخية** بأكوادها حتى لا تضيع الكشوف السابقة؛ ومطابقة النص بقيت **فقط** للصفوف القديمة التي لا تحمل كوداً (`event_code IS NULL`). القاعدة الآن: أي كشف جديد يُضاف بكود ثابت لا برسالة. |
...
## 36. مبيعات وتوزيع سيارات الفان الميدانية وأسطول الشركة (Van Sales & Fleet Management) 🚚📦
* **حالة الوحدة العامة:** 🟢 مكتمل 100%
* **مسارات الكود:**
  * **الباك إند والخدمات:** `backend/src/modules/delivery-reps/van-sales.service.ts`, `backend/src/modules/delivery-reps/van-sales.controller.ts`, `backend/src/modules/delivery-reps/rep-target.engine.ts`, `backend/src/modules/delivery-reps/delivery-reps.service.ts`, `backend/src/modules/delivery-reps/delivery-reps.controller.ts`, `backend/src/database/migrations/2040000000045_van_sales_and_mobile_stock.ts`, `backend/src/database/migrations/2040000000153_fleet_vehicles_and_shift_assignments.ts`, `backend/src/database/migrations/2040000000155_van_sales_target_returns_and_requisitions.ts`
  * **الاختبارات الحرجة:** `backend/test/critical/rep-target-engine.spec.ts`
  * **الفرونت إند والواجهات:** `frontend/src/features/delivery-reps/api/van-sales.api.ts`, `frontend/src/features/delivery-reps/pages/VanSalesAdminManagementPage.tsx`, `frontend/src/features/delivery-reps/components/VanTripsTab.tsx`, `frontend/src/features/delivery-reps/components/VanLoadRequisitionsAdminTab.tsx`, `frontend/src/features/delivery-reps/components/VanFieldReturnsAdminTab.tsx`, `frontend/src/features/delivery-reps/components/VanRepTargetsAdminTab.tsx`, `frontend/src/features/delivery-reps/components/FleetVehiclesTab.tsx`, `frontend/src/features/delivery-reps/components/DriverLoadRequisitionModal.tsx`, `frontend/src/features/delivery-reps/components/VanInventoryTab.tsx`, `frontend/src/features/delivery-reps/components/VanSaleTab.tsx`, `frontend/src/features/delivery-reps/components/VanCollectionTab.tsx`, `frontend/src/features/delivery-reps/components/UpsertFleetVehicleModal.tsx`, `frontend/src/features/delivery-reps/components/AssignDriverModal.tsx`, `frontend/src/shared/components/delivery-reps/UpsertDeliveryRepModal.tsx`, `frontend/src/shared/components/delivery-reps/DeliveryRepsList.tsx`, `frontend/src/features/delivery-reps/pages/VanSalesMobilePage.tsx`
* **الجداول في قاعدة البيانات:** `fleet_vehicles`, `van_sales_trips`, `van_load_requisitions`, `van_field_returns`, `delivery_rep_targets`, `delivery_representatives`, `stock_locations`, `product_location_stock`, `sales`, `sale_items`, `customer_ledger`, `customer_payments`

| الميزة التفصيلية | الحالة | نسبة الإنجاز | ملفات التنفيذ الأساسية | الشرح وملاحظات العمل |
| :--- | :---: | :---: | :--- | :--- |
| **أسطول سيارات الشركة وربط المستودع المتنقل (Company Fleet Vehicles)** | 🟢 | 100% | `fleet_vehicles`, `van-sales.service.ts`, `UpsertFleetVehicleModal.tsx` | سجل كامل لأسطول سيارات الشركة يشمل: رقم اللوحة، الموديل، النوع، الشاسيه، قراءة العداد، الوقود، وتاريخ انتهاء الرخصة، مع التوليد الآلي لموقع مخزني متنقل (`van_stock`) لكل سيارة. |
| **تخصيص السائق والوردية وقراءة العداد (Driver & Shift Assignment)** | 🟢 | 100% | `fleet_vehicles`, `van_sales_trips`, `AssignDriverModal.tsx`, `van-sales.service.ts:reassignVehicleRep` | إسناد السائق للمركبة وتحديد اسم الوردية، وتتبع قراءة عداد الكيلومترات عند فتح الرحلة وعند إغلاقها (`start_odometer`, `end_odometer`) لكشف الاستهلاك ومعدل الوقود. |
| **التمييز الموحد لنوع المندوب (Rep Type Differentiation)** | 🟢 | 100% | `UpsertDeliveryRepModal.tsx`, `delivery-reps.service.ts` | نافذة موحدة شاملة تطبق مبدأ المصدر الموحد (Rule 11) تتيح التحديد الصريح بين: طيار دليفري (`delivery`)، مندوب توزيع فان (`van`)، أو مندوب شامل (`both`). |
| **مركز إدارة ومتابعة الفان سداسي التبويبات (6-Tab Van Sales Hub)** | 🟢 | 100% | `VanSalesAdminManagementPage.tsx` | شاشة إدارة متكاملة 1280px بـ 6 تبويبات سريعة: 1) رحلات وورديات التوزيع، 2) أذونات تحميل الصباح، 3) مرتجعات البيع الميداني، 4) تارجت ومستهدفات المناديب، 5) أسطول سيارات الشركة، 6) مناديب وسائقي الفان، مع عدادات حية للطلبات والمرتجعات المعلقة. |
| **أذونات تحميل الصباح وسير عمل الصرف والاعتماد (Morning Load Requisitions)** | 🟢 | 100% | `van_load_requisitions`, `DriverLoadRequisitionModal.tsx`, `VanLoadRequisitionsAdminTab.tsx`, `van-sales.service.ts` | دورة عمل مؤسسية تبدأ بإنشاء المندوب مسودة طلب تحميل بضاعة من هاتفه -> مراجعة الإدارة ومطابقة الكميات مع رصيد المستودع الفعلي وتعديل الكميات المعتمدة -> اعتماد وصرف وتحميل السيارة آلياً بنقرة واحدة وبدء رحلة التوزيع. |
| **مرتجع البيع الميداني مع اشتراط موافقة الإدارة (Admin-Approved Field Returns)** | 🟢 | 100% | `van_field_returns`, `VanCollectionTab.tsx`, `VanFieldReturnsAdminTab.tsx`, `van-sales.service.ts`, `accounting-posting.service.ts` | تسجيل المرتجع بحالة معلقة (`pending_approval`) دون أي أثر مالي أو مخزني حتى تتم مراجعته من الإدارة. عند الاعتماد (`approve`)، يتم قيد الحركة المخزنية وإشعار الدائن للعميل تلقائياً وترحيل قيد محاسبي عاكس (إيراد وتكلفة بضاعة مباعة)، مع دعم الرفض المسبب (`reject`).<br/>**تحصين سبتمبر 2026 (نفس يوم البناء):** مسار قديم (`recordFieldReturn` عبر `POST /returns`) كان لسه شغالاً **جنب** المسار الجديد على نفس بوابة المندوب — أي طلب مباشر له كان يسجّل المرتجع فوراً بلا أي موافقة، فيلتف كلياً على الميزة. أُزيل نهائياً (لا الخدمة ولا المسار). وكان فحص السعر الصافي مقابل الفاتورة موجوداً بلا أي سقف على الكمية — مندوب يقدر يرد أكتر من المباع، أو نفس الكمية مرتين عبر طلبين منفصلين. صار محسوباً بدالة مشتركة (`المباع ناقص كل مرتجع سابق pending أو معتمد`) وترفض أي تجاوز. وكان الاعتماد يحدّث المخزون وحساب العميل فقط بلا أي قيد محاسبي — صار يستدعي `postVanFieldReturn` (عكس إيراد + عكس تكلفة بضاعة مباعة بتكلفة الصنف الفعلية لا بسعر الرد). هجرة 156، جناح `test/e2e/van-field-returns.e2e.ts`. |
| **أسباب المرتجع القياسية المؤسسية (Standardized Return Reasons)** | 🟢 | 100% | `van_field_returns`, `VanCollectionTab.tsx`, `VanFieldReturnsAdminTab.tsx` | تصنيف المرتجعات الميدانية إلى 6 أسباب معتمدة: بضاعة تالفة / كسر (`damaged`)، منتهية الصلاحية (`expired`)، عيب صناعة / تعبئة (`manufacturing_defect`)، بضاعة راكدة (`stagnant`)، خطأ في التوريد (`order_mismatch`)، ورغبة العميل (`customer_request`). |
| **التحقق الصارم من أسعار وكميات الفاتورة الأصلية (Invoice Price Verification)** | 🟢 | 100% | `van-sales.service.ts:getCustomerEligibleSales`, `VanCollectionTab.tsx` | استرجاع فواتير العميل مع احتساب السعر الصافي الفعلي بعد الخصم (`netUnitPrice = total / qty`) والتحقق من الكميات المتبقية القابلة للرد، لمنع المندوب من رد أي صنف بسعر أعلى مما دفعه العميل أو تجاوز الكمية المشتراة. |
| **محرك مستهدفات المبيعات واستبعاد الجمع والعطلات (Sales Target & Daily Quota Engine)** | 🟢 | 100% | `rep-target.engine.ts`, `rep-target-engine.spec.ts`, `VanRepTargetsAdminTab.tsx`, `VanSalesMobilePage.tsx` | محرك حسابي مستقل يحدد المستهدف الشهري، نسبة التحقيق الفعلية MTD، المتبقي، وحساب المستهدف اليومي الصافي المطلوب بقسمة المتبقي على أيام العمل المتبقية مع استبعاد أيام الجمعة والعطلات الرسمية آلياً. |
| **إحداثيات GPS الجغرافية على الفواتير والسندات وروابط Google Maps** | 🟢 | 100% | `sales`, `customer_ledger`, `van-sales.service.ts`, `VanTripsTab.tsx` | التقاط إحداثيات GPS لموقع المندوب الجغرافي لحظة إصدار فاتورة البيع ولحظة تحصيل السند النقدي، وعرض أزرار فتح الموقع المباشر على Google Maps في نافذة تفاصيل الرحلة بالإدارة. |
| **رؤية مخزون المستودع الرئيسي المتزامن (Main Warehouse vs Van Stock)** | 🟢 | 100% | `van-sales.service.ts:getActiveTrip`, `VanInventoryTab.tsx` | إظهار رصيد المستودع الرئيسي المتاح بجانب رصيد سيارة الفان لحظياً في شاشة جرد السيارة لمساعدة المندوب على معرفة توفر الأصناف بالمستودع الأم قبل طلبها أو بيعها. |
| **البيع النقدي والآجل وقيد المديونيات الميدانية** | 🟢 | 100% | `van-sales.service.ts`, `VanSaleTab.tsx` | دعم البيع النقدي والآجل لمحلات البقالة المسجلة بخط السير أو محلات جديدة تُسجل بالشارع، مع قيد المديونية فورياً في كشف حساب العميل والتقاط GPS. |
| **تحصيل ديون العملاء الميداني (Field Collections)** | 🟢 | 100% | `van-sales.service.ts`, `VanCollectionTab.tsx` | إمكانية استلام دفعات نقدية لسداد ديون قديمة، توليد سند قبض فوري، وتخفيض مديونية العميل في قاعدة البيانات لحظياً مع زيادة كاش الفان والتقاط GPS. |
| **تصفية اليومية الذكية وجرد الفان وتوريد الكاش (End-of-Day Settlement)** | 🟢 | 100% | `van-sales.service.ts`, `VanSettleTab.tsx` | مطابقة مبيعات اليوم والنقدية المحصلة مع الكاش الفعلي المعدود مع احتساب العجز والزيادة، وخيار تفريغ البضاعة المتبقية في السيارة وإعادتها للمستودع الرئيسي تلقائياً بنقرة واحدة، وتوريد النقدية للخزينة. |
...
* **الجداول في قاعدة البيانات:** `online_orders` (أعمدة `order_type`, `table_number`, `kitchen_draft_sale_id` — هجرة 133), `sales` (مسودة المطبخ `status='draft'`), `sale_items`, `products`, `tenants`
...
| **تنزيل طلب المتجر في سلة الكاشير بخصمه وأسعاره** | 🟢 | 100% | `storefront.service.ts:prepareOrderForPos`, `sales-write.service.ts:resolveOnlineOrderTerms`, `pos/contracts.ts` | الكاشير ينزّل الطلب في السلة ويعدّل عليه، وخصم الكوبون وأسعار السطور اللي المتجر ثبّتها بتعدي من غير ما يطلب PIN مدير، لأن السيرفر بيقراها من الطلب المخزَّن نفسه. الطلب الملغي أو اللي اتحول لفاتورة قبل كده مابيعتمدش أي خصم |
| **رابط مستقل لكل متجر على سب دومين (`almhnds.zsystemai.com`)** | 🟢 | 100% | `store-public-url.engine.ts`, `frontend/src/lib/store-public-url.ts`, `root-router.tsx`, `PublicStorefrontPage.tsx`, `deploy/nginx/zsystemai-subdomains.conf` | كل متجر يفتح على `https://<slug>.<root>` بنفس قاعدة البيانات (الاسم بيتقري من الدومين بدل المسار). رابط المتجر في الإعدادات وصفحة الطلبات ومشاركة المنتج وروابط QR الطاولات ورابط تتبع الطلب ومعاينة واتساب ورد بوت الذكاء الاصطناعي كلها من محرك واحد (SF-10). الأسماء المحجوزة (`app`، `www`، `api`، ...) مرفوضة. الروابط القديمة `/st/` و`/store/` و`/shop/` بتتحول تلقائياً. **الحالة:** مفعّل في الإنتاج من 22 سبتمبر 2026. الجذر `zsystemai.com` بقى الموقع التسويقي، فالروابط القديمة على الجذر مابقتش بتفتح المتجر (مفيش روابط اتوزعت على عملاء)؛ `app.zsystemai.com/st/<slug>` بيحوّل للسب دومين. الديسكتوب ومن غير المتغيرين: `/st/<slug>` كما كانت |
...
| **التوجيه التلقائي لشاشات المطبخ (KDS) والكاشير فور الإرسال** | 🟢 | 100% | `storefront.service.ts`, `KitchenDisplayPage.tsx` | بمجرد تأكيد الزبون للطلب من هاتفه، يُنشئ النظام آلياً سجل بيع معتمد برقم الطاولة وينزل فوراً بشاشة المطبخ KDS مع تنبيه صوتي وتحديد رقم الطاولة لتحضيرها فوراً.<br/>**تصحيح سبتمبر 2026 (O53):** «سجل البيع المعتمد» كان فاتورة `posted` تُكتب مباشرة بلا خصم مخزون ولا قيد ولا دفع — إيراد في التقارير لا يعرفه الأستاذ، والطلب لا يمكن تنزيله في الكاشير. صار الطلب يُنشئ **مسودة للمطبخ** (`status='draft'`، تظهر فوراً على KDS بلا أي أثر مالي) في نفس معاملة الطلب. الكاشير ينزّل الطلب في السلة كطلب صالة لنفس الطاولة ويرحّله عبر محرك المبيعات، فتُحذف المسودة تلقائياً وتنتقل حالة التذكرة (قيد التحضير/جاهز) للفاتورة الحقيقية. الإلغاء يزيل التذكرة. العميل لا يستطيع إلغاء/تعديل طلب الطاولة بعد إرساله (يطلبه من فريق الخدمة). فواتير الطاولة القديمة قبل التصحيح موثقة في O61. |
...
* **الميزات والقدرات المفعلة:**
  ...
  4. **محرك المطابقة الذكية للباقات (Smart SaaS Plan Recommendation):** تحليل فوري للموديولات المختارة ومطابقتها التلقائية مع مصفوفة باقات الساس الـ 4 المعتمدة في النظام (الأساسية، الاحترافية، المتكاملة، والتجارة الشاملة) مع إظهار الملخص التشغيلي. **الأسعار مصدرها الوحيد `PRICING_AND_PACKAGING.md`، ولا تُكتب في الكود ولا في الواجهة**.
...
* **روابط وشاشات الوصول:** `/login` (شاشة دخول النظام الرئيسية)، `/settings/users` (إدارة المستخدمين والصلاحيات)، `/portal` (بوابة الموظف الذاتية)، `/punch` (بصمة الحضور بالموبايل)، `/driver-portal` (بوابة المناديب وسائقي الفان).
  ...
  * **مصدر الحقيقة لرموز البوابات:** `backend/src/core/auth/utils/portal-token.ts`, `backend/src/core/auth/utils/tenant-boundary.ts`, `backend/src/core/auth/guards/session-auth.guard.ts`
  * **الاختبارات الآلية الحساسة:** `backend/test/critical/session-auth.spec.ts`, `backend/test/critical/portal-token.spec.ts`, `backend/test/critical/tenant-boundary.spec.ts`, `backend/test/infra/phone-utils.spec.ts`, `backend/test/infra/users.helper.spec.ts`, `frontend/src/features/settings/hooks/useUserManagement.helpers.spec.ts`, `frontend/src/features/auth/pages/LoginPage.spec.tsx`
* **الميزات والقدرات المفعلة:**
  ...
  8. **حماية بوابة الموظفين وبصمة الموبايل (`/portal` و `/punch`):** توحيد شاشتي بوابة الموظف وبصمة الحضور للمطالبة الحصرية برقم الهاتف المحمول (`رقم الهاتف المحمول: مثال: 01012345678`) بدلاً من كود الموظف لمنع أي تضارب أو التباس لحسابات الموظفين بين المنشآت عند تشابه الأكواد والـ PIN الافتراضي، مع الإبقاء على دعم كود الموظف كخيار خلفي حصري عند اقتران الرابط بكود المنشأة (`?c=ragab`).<br/>**تحصين سبتمبر 2026 (المرحلة 17 — البند O20):** رمز الدخول السريع (PIN) كان يُخزَّن **نصاً صريحاً** في قاعدة البيانات (`pin_code VARCHAR(10)`) و**يُعاد ضمن بيانات الموظف** فتعرضه شاشة الملف الشخصي كاملاً — خرق مباشر للثابت الدستوري "صفر كلمات مرور نصية". صار مجزَّأً بـ bcrypt بملح مستقل لكل سجل (هجرة 130 تجزّئ القيم القائمة ثم **تحذف عمود النص الصريح**)، والتحقق عند الدخول يجري بالتوازي على المرشحين بعد ترشيحهم بالهاتف حفاظاً على منطق تعدد المنشآت، ولا يصل الرمز إلى الواجهة إطلاقاً — تُستقبل حالة وجوده فقط (`hasPinCode`).
  ...
  17. **المصدر الموحد لرموز البوابات الخارجية والفشل الآمن للسر (Unified Portal Token Authority & Fail-Closed Secret):** استُخرج توقيع رموز البوابات الثلاث (الموظف `/portal`، البصمة `/punch`، المندوب `/driver-portal`) إلى مصدر حقيقة واحد `backend/src/core/auth/utils/portal-token.ts` بدل ثلاث نسخ متباينة. في وضع `CLOUD_SAAS` **لا يوجد سر افتراضي إطلاقاً**: غياب `SESSION_SECRET` (أو سر أقصر من 16 محرفاً) يرفض إصدار الرموز والتحقق منها معاً برسالة صريحة، بدل التوقيع بمفتاح ثابت مكتوب في الكود. مقارنة التواقيع صارت ثابتة الزمن (`timingSafeEqual`)، وعمر الرمز موحّد في مكان واحد.
  18. **إعادة التحقق الحي من القاعدة عند كل طلب بوابة (Stateless Token Re-Validation):** رموز البوابات بلا حالة وعمرها 30 يوماً ولا تمر على جدول `sessions`، لذلك صار `verifyToken`/`verifyDriverToken` يتأكد على **كل** طلب أن الموظف/المندوب ما زال `active` **وداخل نفس المنشأة المذكورة في الرمز**، ويشتق نطاق المستأجر والفرع من صف قاعدة البيانات لا من حمولة الرمز. الأثر العملي: **إيقاف أو فصل الموظف/المندوب يقطع وصوله فوراً** بدل أن يظل نافذاً حتى انتهاء الرمز.
  19. **العزل الكامل لبيانات بوابة الموظف الذاتية (Employee Portal Tenant Scoping):** كل استعلامات البوابة — الحضور، مفردات المرتب وتسوياته، الإجازات وأنواعها، السلف، والعهد — صارت مقيّدة بـ`tenant_id` إلى جانب `employee_id`، وطلبات الإجازة والسلف تُكتب بنطاق المنشأة صراحة فتظهر لإدارة الموارد البشرية كما هو متوقع، ونوع الإجازة المُرسَل من العميل يُتحقق من أنه نشط ويخص نفس المنشأة قبل قبوله. كما أُصلح احتساب رصيد الإجازات ليقرأ من المصدر الحقيقي `hr_employees.annual_leave_balance` مخصوماً منه الإجازات المعتمدة خلال العام.
  20. **تقييد اعتماد الجلسة من رابط العنوان على الطرق الآمنة (URL Session Credential Hardening):** معرّف الجلسة المُمرَّر عبر `?token=`/`?sessionId=` (المخصص للتنزيلات المباشرة) لم يعد مقبولاً إلا على طلبات القراءة (GET/HEAD)، فلا يستطيع رابط مُسرَّب عبر سجلات الوكيل أو سجل المتصفح أو رأس `Referer` تنفيذ أي عملية تُغيّر البيانات.
  21. **إثبات ملكية الطلب قبل أي تأكيد سداد وارد (Webhook Order Ownership Proof):** نقاط الويب هوك الأربعة العامة (Paymob / XPay / Tap / Stripe) في المتجر كانت تبني شرط البحث عن الطلب **شرطياً**، فحمولة لا تحمل إشارة منشأة كانت تنتج بحثاً بـ`order_number` وحده على كل المنشآت — ورقم الطلب تسلسلي **لكل منشأة** (`ON-YYMMDD-0001`) فليس معرّفاً فريداً، وكانت الجملة التالية تقلب الطلب الناتج إلى "مدفوع". صار الحل عبر محرك نقي يفشل مغلقاً: وجود إشارة منشأة يلزم الاستعلام بها، وغيابها لا يقبل إلا معرّفاً صادراً من بوابة الدفع نفسها وبشرط أن يطابق طلباً واحداً بالضبط. وكل تحديث على `online_orders` صار يحمل معرّف المنشأة صراحة. يحرسه جناح `webhook-order-resolution.spec.ts`.
  22. **تفعيل حارس العهد الفعلي عند تصفية نهاية الخدمة (Custody Gate Reactivation):** سجلات العهد العينية كانت تُكتب بلا معرّف منشأة، بينما الحارس الذي يحجب ترحيل المخالصة يعدّ العهد غير المستردة **بمعرّف المنشأة** — فكان يرى صفراً دائماً ولا يحجب أبداً، وكان بإمكان صرف مخالصة نهاية خدمة كاملة لموظف ما زالت بعهدته أصول الشركة بلا إخلاء طرف ولا استقطاع. صارت سجلات العهد تُكتب بنطاق المنشأة، وشاشة المعاينة والحارس يقرآن بنفس النطاق فلا يختلفان، وهجرة 127 ترمّم السجلات القائمة.
  23. **وصول اعتماد الأوفرتايم إلى مسيّر الرواتب (Attendance Exception Approval Path):** استثناءات الحضور المُولَّدة آلياً كانت تُكتب بلا معرّف منشأة، بينما شاشة الاعتماد تجلب الاستثناء بمعرّف المنشأة — فكان اعتماد أي استثناء يعود دائماً بـ"غير موجود". والرواتب لا تحتسب دقائق الأوفرتايم إلا من استثناء بلغ حالة "معتمد"، فكان الأوفرتايم المعتمد لا يصل المسيّر إطلاقاً. أُصلح مصدر الكتابة ورُمِّمت السجلات القائمة.
  24. **قابلية البتّ في طلبات الإجازة المُنشأة من إدارة الموارد البشرية (HR-Created Leave Decisions):** طلبات الإجازة المُنشأة من شاشة الموارد البشرية كانت تُكتب بلا معرّف منشأة (طلبات البوابة الذاتية كانت قد أُصلحت سابقاً)، بينما الاعتماد والرفض والإلغاء تجلب الطلب بمعرّف المنشأة — فكان الطلب يعلق في حالة "قيد الانتظار" للأبد، وأيام الإجازة المعتمدة تدخل مسيّر الرواتب. أُصلح مصدر الكتابة ورُمِّمت السجلات القائمة.
  25. **ظهور تسويات الرواتب اليدوية في قسيمة الموظف (Payslip Adjustments Visibility):** تسويات بنود الراتب اليدوية كانت تُكتب بلا معرّف منشأة، فاختفت من قسيمة الراتب في بوابة الموظف فور تقييد استعلام البوابة بمعرّف المنشأة. أُصلح مصدر الكتابة ورُمِّمت السجلات القائمة.
  26. **قصر تعديل العهد على منشأتها (Custody Mutation Scoping):** تغيير حالة عهدة (مستردة/مفقودة/تالفة) كان يقرأ ويحدّث السجل بالمعرّف الأساسي وحده بلا أي شرط منشأة، وفرع "مفقودة/تالفة" يُنشئ بعدها استقطاعاً على راتب الموظف الذي وجده — أي أن مستخدماً في منشأة كان يستطيع شطب عهدة منشأة أخرى وقيد استقطاع عليها. صارت القراءة والتحديث مقيّدتين بالمنشأة.
  27. **التحقق من ملكية رحلة التوزيع قبل تسجيل تحصيل ميداني (Van Trip Ownership Check):** تسجيل تحصيل نقدي ميداني كان يأخذ رقم الرحلة من جسم الطلب مباشرة ويضيفه إلى كاش الرحلة بلا أي تحقق — بعكس تسجيل المرتجع وتصفية الرحلة اللذين يتحققان. صار يتحقق أولاً، وهو مهم لأن كاش الرحلة هو الرقم الذي تصالحه التصفية النهائية.
  28. **منح فترة الاشتراك مرة واحدة لكل دفعة (Subscription Grant Idempotency):** تأكيد الدفع الوارد من بوابات الاشتراك كان يمدّد الاشتراك ويسجّل الدفعة بلا أي فحص لمرجع المعاملة، وقراءة الاشتراك الحالي كانت تتم خارج المعاملة. وبما أن كل بوابة تعيد إرسال الإشعار إن لم تستلم استجابة ناجحة، وأن توقيع بعض البوابات تحقّق ثابت على جسم الطلب، فإن إشعاراً واحداً يُعاد إرساله كان يضيف فترة اشتراك جديدة في كل مرة. صار القرار في محرك مستقل: دفعة بلا مرجع صادر من البوابة تُرفض، ومرجع سبق تسجيله لا يفعل شيئاً، والفترة الجديدة تُبنى على نهاية الفترة الحالية إن كانت سارية فقط — وكل ذلك داخل معاملة واحدة تحت قفل صف المنشأة. يحرسه جناح `subscription-grant-idempotency.spec.ts`.
  29. **اكتمال تسجيل التجديد الإلكتروني (Online Renewal Completion):** كان سجل التدقيق الذي يُكتب بعد التجديد يمرّر معرّف مستخدم نصياً إلى عمود رقمي، فيفشل الإدراج **بعد** أن يكون التجديد قد رُحّل فعلاً — فترجع العملية بخطأ، وتعيد البوابة المحاولة، فيتضاعف المنح. صار سجل التدقيق يُكتب بفاعل نظام بلا مستخدم، وصارت العملية نفسها آمنة ضد التكرار، ويُبطَل كاش المنشأة فور المنح حتى لا يُرفض دخول عميل سدد للتو.
  30. **إيقاف لوحة المطوّرين ما لم يُضبط سر خاص بالتثبيت (Developer Panel Fail-Closed):** المسار الذي يمنح أي باقة وأي مجموعة ميزات كان يقبل كلمة مرور افتراضية مكتوبة داخل الكود المنشور، وكانت نسخة سطح المكتب تشحن القيمة نفسها لكل تثبيت — وفي وضع خادم الشبكة المحلية يستمع الخادم على كل الواجهات. صار: لا قيمة افتراضية إطلاقاً (السر غير المضبوط يعطّل اللوحة برسالة صريحة)، ونسخة سطح المكتب تولّد سراً مستقلاً لكل تثبيت وتحفظه في ملف الأسرار المحلي بصلاحيات مقيّدة، وفحص وضع التشغيل صار قائمة سماح لأوضاع سطح المكتب بدل استثناء وضع السحابة وحده.
  31. **منع كتابة ملفات التشخيص خارج مجلد التخزين (Diagnostics Upload Path Containment):** مسار رفع حزم التشخيص عام بالتصميم ليستخدمه عملاء سطح المكتب، وكان أحد حقول النموذج يدخل اسم الملف دون تنقية، فأمكن توجيه الكتابة خارج شجرة التخزين بملف يتحكم المرسل في محتواه ودون أي مصادقة. صار الحقل يمر بقائمة سماح للمحارف مع حد للطول، مع التحقق الإضافي من أن المسار النهائي يقع داخل مجلد العميل قبل الكتابة.
  32. **تأمين صفحتي محاكاة الدفع (Sandbox Checkout Output Encoding):** صفحتا المحاكاة تُبنيان كـHTML من داخل التطبيق، وكانت كل القيم القادمة من الرابط تُدرج فيهما دون ترميز، وكانت وجهة العودة تُدرج في رابط وفي سطر تحويل داخل سكربت — فرابط مُعد خصيصاً كان يشغّل كوداً في جلسة من يفتحه ويحوّله لأي موقع. صار كل شيء مُرمَّزاً، ووجهة العودة مقيّدة بمسار على نفس الموقع. يحرسه جناح `sandbox-checkout-render.spec.ts`.
  33. **إنهاء الجلسات القائمة عند إعادة تعيين كلمة مرور مالك المنشأة (Owner Password Reset Revocation):** إعادة التعيين من لوحة المنصة كانت تغيّر كلمة المرور وتترك كل الجلسات المفتوحة تعمل، رغم أن هذا الإجراء هو العلاج المعتاد عند الاشتباه في اختراق حساب. صارت كلمة المرور والجلسات تتغيّران معاً في معاملة واحدة.
  34. **حذف المنشأة كعملية واحدة لا تتجزأ (Atomic Tenant Deletion):** الحذف النهائي كان يمسح جداول المنشأة واحداً واحداً خارج أي معاملة، فأي تعثّر في المنتصف كان يترك المنشأة محذوفة جزئياً بلا إمكانية تراجع. صار الحذف كله في معاملة واحدة — إما أن تختفي المنشأة بالكامل أو لا يتغير شيء.
  35. **عزو إجراءات التصفّح كمالك لمن نفّذها فعلاً (Impersonation Attribution):** لما موظف المنصة يفتح نسخة عميل بخاصية "تصفّح كمالك"، كانت الجلسة تُفتح باسم مستخدم المالك ولا يُسجَّل غير بداية الزيارة ونهايتها — فكل إجراء بينهما (ترحيل قيد، اعتماد مخالصة، حذف بيانات) يظهر في سجل التدقيق منسوباً للعميل نفسه. صار كل صف في سجل التدقيق يحمل هويتين: الهوية التي جرى الإجراء تحتها، ومَن كان يقود الجلسة فعلاً — والقيمة تُشتق من صف الجلسة لا من الطلب. وشاشة تاريخ المنشأة في لوحة المنصة بتعرض المنتحِل صراحة.
  36. **سحب مفاتيح الموديولات عند تخفيض الباقة مع احترام التفعيل اليدوي (Plan Downgrade Switch Revocation):** مزامنة مفاتيح التشغيل كانت تفتح الموديولات ولا تغلقها أبداً، فبعد تخفيض الباقة تفضل عناصر القائمة ظاهرة للمستأجر ثم يقابله رفض صلاحية عند الضغط. (الوصول للـAPI نفسه كان محجوباً بشكل صحيح دائماً عبر فحص ميزات الباقة، فالمسألة ما كانتش تسريب ميزات مدفوعة.) صارت المزامنة تسجّل ما منحته، وتسحب عند التخفيض **ما منحته هي فقط** — وأي مفتاح فعّله المستأجر بنفسه ما بيتلمسش.
  37. **ترحيل قيد المستخلص صار ممكناً أصلاً، وعبر المحرك المحاسبي (Contracting IPC Journal Posting):** ترحيل القيد المحاسبي لأي مستخلص **فيه محتجزات** كان يفشل بالكامل: السطر الذي يقيّد المحتجز في سجل الضمانات كان يسمّي أعمدة غير موجودة في الجدول، فترفضه قاعدة البيانات وتسقط معاملة الترحيل كلها — والمحتجز بند قياسي في كل مستخلص إنشاءات، في مسار العميل ومسار مقاول الباطن معاً. وبعد إصلاحه، صار القيد يُرحَّل عبر المحرك المحاسبي الموحد بدل كتابته يدوياً، فصار خاضعاً لحارس توازن القيد ولفحص أقفال الفترات المحاسبية وللترقيم الآمن غير القابل للتضارب — وهي ثلاثة لم تكن مطبَّقة على هذا المسار إطلاقاً. **ملاحظة تشغيلية:** المستخلص الذي لا تتوازن سطوره صار **يُرفض بوضوح** بدل أن يُرحَّل قيداً مختلاً بصمت.
...
* **الميزات والقدرات المفعلة:**
  ...
  4. **تحصين سبتمبر 2026 (المرحلة 5):** حارسا سقف السند وسقف الفاتورة كانا موجودين ويعملان، لكن **كل القراءات الأربع كانت بلا `forUpdate()`** — فتخصيصان متزامنان على نفس السند أو نفس الفاتورة يقرآن الرصيد ذاته وكلاهما يجتاز الحد، فيُخصَّص أكثر من المتاح ويظهر `paid_amount` أقل من مجموع التخصيصات (نقص في الذمم). أُضيف قفل الصف على قراءات `customer_payments` و`supplier_payments` و`sales` و`purchases`.
...
* **مسارات الكود الأساسية:**
  * **قاعدة البيانات:** `backend/src/database/migrations/2040000000083_contracting_master_boq_library.ts`, `2040000000094_contracting_master_boq_comprehensive_expansion.ts`, `2040000000097_contracting_mep_fire_fighting_master_library.ts`, `2040000000128_contracting_master_boq_trade_categories_normalization.ts`. جدول `contracting_master_boq_library` مُحمّل بـ 153 بنداً معيارياً واقعياً موزعاً بدقة 100% وبدون أي تكرار عبر 12 تخصصاً هندسياً معتمداً.
  * **الباك إند:** `backend/src/modules/contracting/contracting.service.ts` (`getMasterBoqTrades` بالترتيب الإنشائي التنفيذي المعتمد، و`getMasterBoqLibrary` مع دعم التصفية الفردية والمتعددة للقطاعات والتخصصات).
  * **الفرونت إند:**
    - `frontend/src/features/contracting/components/ImportMasterBoqModal.tsx` (شريط تصفية قطاعي مزدوج مع أيقونات هندسية وتحديد فريد لكل تخصص).
    - `frontend/src/features/contracting/components/CreateMasterBoqItemModal.tsx` و `UniversalBoqItemModal.tsx` (توليد الأكواد التسلسلية وهوامش الأرباح التقديرية حسب التخصص).
    - `frontend/src/features/contracting/pages/ContractingMasterBoqPage.tsx` (شريط القطاعات والتخصصات الهندسية المؤسسي: كروت قطاعات كبرى Super-Tabs، كبسولات التخصصات بأيقونات هندسية مستقلة، ثبات الخط 600، زمن استجابة 0ms، وانعدام التحديد المزدوج نهائياً).
* **التخصصات الإنشائية والكهروميكانيكية الـ 12 المعتمدة بالتسلسل التنفيذي:**
  1. تجهيزات الموقع والأعمال التمهيدية (`site_mobilization` - 6 بنود)
  2. الأعمال المدنية والخرسانات (`civil_concrete` - 16 بنداً)
  3. أعمال المباني والعزل والفواصل (`masonry_insulation` - 14 بنداً)
  4. المنشآت المعدنية والجمالونات (`steel_structure` - 3 بنود)
  5. التشطيبات المعمارية والديكور (`finishing_decor` - 15 بنداً)
  6. الأبواب والشبابيك والواجهات (`doors_windows_aluminum` - 11 بنداً)
  7. الأعمال الكهربائية والإنارة (`electrical_lighting` - 22 بنداً)
  8. التيار الخفيف والأنظمة الذكية ELV (`smart_elv_systems` - 8 بنود)
  9. الأعمال الصحية وتغذية وصرف المياه (`plumbing_sanitary` - 17 بنداً)
  10. التكييف والتهوية الميكانيكية (`hvac_mechanical` - 8 بنود)
  11. شبكات مكافحة وإطفاء الحريق (`fire_fighting` - 24 بنداً)
  12. الموقع العام واللاندسكيب والبنية التحتية (`site_infrastructure` - 9 بنود)
* **المجموعات والقطاعات الإنشائية الكبرى (Super-Tabs):**
  - **كافة التخصصات (الكل):** 153 بنداً
  - **الإنشائي والمدني:** 39 بنداً (تجهيزات الموقع + المدني والخرسانات + المباني والعزل + الجمالونات)
  - **التشطيبات والمعماري:** 26 بنداً (التشطيبات والديكور + الأبواب والشبابيك والواجهات)
  - **الكهروميكانيك (MEP):** 79 بنداً (الكهرباء والإنارة + التيار الخفيف ELV + الصحي والسباكة + التكييف والتهوية + مكافحة الحريق)
  - **الموقع العام واللاندسكيب:** 9 بنود
* **القدرات التشغيلية:**
  - القضاء التام والنهائي على ظاهرة التحديد المزدوج للتابات (تمسك تابات تانية) بفضل توحيد الفئات البرمجية وتفرد المفاتيح.
  - شريط قطاعات علوي يقلص التشتت البصري بنسبة 70% ويمكّن المقاول من تصفية تخصصات الكهروميكانيك أو المعماري بنقرة زر.
  - التزام صارم بمعيار ثبات الخط (Font-Weight: 600 ثابت بدون رعشة) وزمن استجابة فوري 0ms.
  - سحب تخصص كامل أو بنود محددة بضغطة زر واحدة لمقايسة أي مشروع جديد.
...
## 145. محرك استخراج وتحليل مقايسات المشروعات والعطاءات من ملفات الـ PDF (Native PDF BOQ & SOV Ingestion Engine)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Enterprise Production Ready)
* **المسارات في الكود:**
  - `frontend/src/features/contracting/utils/boqPdfParser.ts`: محرك التحليل الهندسي لاستخراج الجداول من ملفات الـ PDF مع التجميع المكاني وتصحيح الحروف العربية وتجميع البنود متعددة الأسطر.
  - `frontend/src/features/contracting/utils/boqPdfParser.spec.ts`: حزمة الاختبارات الشاملة (7 من 7 اختبارات ناجحة 100%).
  - `frontend/src/features/contracting/components/ImportBoqModal.tsx`: النافذة الموحدة لاستيراد المقايسات مع دعم `.xlsx, .xls, .csv, .pdf`.
  - `frontend/src/features/contracting/utils/boqExcelParser.ts`: دوال المطابقة الدلالية للأعمدة وتصنيف الأنشطة الهندسية والتطبيع العددي للفواصل.
* **القدرات والوظائف المعتمدة:**
  1. **الاستخراج المحلي الفوري بدون ذكاء اصطناعي (100% Native & In-Browser):**
     - معالجة سريعة بأجزاء من الثانية داخل المتصفح أو بيئة العمل بدون إرسال بيانات العطاءات إلى أي خوادم خارجية أو تكبد تكاليف API.
  2. **التقسيم المكاني ثنائي الاتجاه للأعمدة (Dynamic RTL & LTR Slicing):**
     - التعرف التلقائي على اتجاه الجدول، وتحديد إحداثيات أعمدة (رقم البند، بيان الأعمال والمواصفات، الوحدة، الكمية، الفئة، الإجمالي) بدقة متناهية سواء كان الجدول عربياً (RTL) أو إنجليزياً (LTR).
  3. **تجميع المواصفات والبنود متعددة الأسطر (Multi-line Description Stitching):**
       - استيراد وسحب البنود من بنك المقايسات المرجعي (124 بنداً).
       - حاسبة الحصر السريع من واقع أبعاد اللوحات والمخططات المعمارية والإنشائية (طول × عرض × ارتفاع / مسطحات × عدد مع خصومات الفتحات) لحساب الكميات الصافية فورياً.
    3. **التفكيك التفاعلي اللحظي للكود الهندسي (Live BOM Exploder & Custom Rate Engine):**
       - لوحة تفكيك تفاعلية قابلة للفتح والطي تحت كل بند تعرض إجمالي كمية كل مادة أولية مطلوبة (كم طوبة، كم طن أسمنت، كم م³ رمل وزلط، كم طن حديد، كم لتر دهان، كم يومية مصنعية).
...
* **المسارات في الكود:**
  ...
  - `frontend/src/features/contracting/pages/ContractingCloseoutPage.tsx`:
    - **بيئة عمل متخصصة للمرحلة السابعة (Closeout & Handover):**
      ...
      - تحليل الأرباح والخسائر الختامي وتصفية حسابات المشروع (`BoqProfitabilityModal`).
      - متابعة إفراجات ضمان حسن التنفيذ المحتجز (`RetentionLedgerModal`).
      - خريطة تفاعلية لبروتوكول التسليم طبقاً مواصفات الفيديك (FIDIC Handover Protocol).
  - `frontend/src/features/contracting/pages/ContractingLayout.tsx`:
    - ربط المسار المتسلسل والموجه الذكي وحفظ كاش التابات في الذاكرة (Keep-Alive 0ms) مع تكامل كافة النوافذ المنبثقة الـ 39.
  - `frontend/src/features/contracting/routes.tsx`:
    - تحديث المسارات الملاحية السريعة لدعم الملاحة السلسة للمراحل السبع.
...
* **القدرات والوظائف المعتمدة:**
  ...
  2. **التقسيم المكاني ثنائي الاتجاه للأعمدة (Dynamic RTL & LTR Slicing):**
     - التعرف التلقائي على اتجاه الجدول، وتحديد إحداثيات أعمدة (رقم البند، بيان الأعمال والمواصفات، الوحدة، الكمية، الفئة، الإجمالي) بدقة متناهية سواء كان الجدول عربياً (RTL) أو إنجليزياً (LTR).
  3. **تجميع المواصفات والبنود متعددة الأسطر (Multi-line Description Stitching):**
     - دمج الأسطر المتتالية للبند الواحد في توصيف موحد متكامل، وربط ديباجات الأبواب ورؤوس الأقسام بالبنود التابعة لها دون تفكيك البند أو توليد أسطر فارغة.
  4. **معالجة فواصل الآلاف بالنقطة والتطبيع الرقمي:**
     - معالجة ذكية للأرقام المكتوبة بفواصل الآلاف النقطية (مثل `9.300.000` و `23.450.000`) والأرقام الهندية/المشرقية.
  5. **فلترة الإجماليات وهوامش الصفحات:**
     - الاستبعاد التلقائي لسطور الإجماليات النهائية وتكرارات الهيدرات وأرقام الصفحات.
  6. **الربط المباشر مع دراسة العطاءات والمقايسة التنفيذية:**
     - إتاحة مراجعة وتعديل وتصنيف البنود المستخرجة وإدراجها بضغطة زر في المقايسة التنفيذية للمشروع أو دراسة العطاء الفورية.

---

## 146. محرك التسعير الفوري الاستباقي والمكتبة الهندسية المرجعية الموحدة لمقايسات المشروعات (Proactive BOQ Auto-Pricing & Master Engineering Pricing Core)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Enterprise Production Ready)
* **المسارات في الكود:**
  - `frontend/src/features/contracting/utils/engineeringMasterPricingCore.ts`: المحرك الهندسي الشامل وقاموس الثوابت المرجعية المدمجة لكافة التخصصات التسعة (9 تخصصات كبرى: مكافحة الحريق Fire Fighting، القوى والبنية التحتية الكهربائية Electrical MV/LV، التيار الخفيف والشبكات Low Current، التكييف والتهوية HVAC، الصحي والتغذية والصرف Plumbing & Drainage، الخرسانات والأعمال المدنية Civil Concrete، أعمال المباني والقواطع Masonry، العزل المائي والحراري Waterproofing & Insulation، والتشطيبات المعمارية والأرضيات Architectural Finishes) بأكثر من 50 بنداً مرجعياً مفصلاً بتكاليفها وهوامشها ومكوناتها الـ BOM.
  - `frontend/src/features/contracting/utils/boqExcelParser.ts`: دمج التسعير الاستباقي التلقائي فور سحب وقراءة ملفات الإكسيل والـ PDF مع الدعم الكامل لدمج الأعمدة المتعددة (Item Name / Category + Technical Specification Description) وتوريث سياق الأقسام الرئيسية وتجاوز ملاحظات وديباجات الشروط التعاقدية العامة.
  - `frontend/src/features/contracting/utils/boqExcelParser.spec.ts`: حزمة الاختبارات الشاملة المعتمدة بنسبة نجاح 100% (14 اختباراً كاملاً لكافة سيناريوهات الملفات الحقيقية وشيتات التيار الخفيف LC BOQ والتخصصات).
  - `frontend/src/features/contracting/components/ImportBoqModal.tsx`: شاشة معاينة الاستيراد الذكية مع تمييز البنود المسعرة آلياً بشارة خضراء وحساب إجمالي العطاء التقديري لحظياً قبل الاعتماد.
  - `frontend/src/features/contracting/pages/ContractingTenderPage.tsx`: شاشة دراسة العطاءات وتفكيك شجرة الموارد (BOM Breakdown: خامات وتوريدات + مصنعيات وتركيبات) تلقائياً بناءً على الثوابت الهندسية.
  - `backend/src/database/migrations/2040000000097_contracting_mep_fire_fighting_master_library.ts`: التغذية السحابية وقاعدة البيانات لبنك البنود والثوابت الهندسية المركزية.
* **القدرات والوظائف المعتمدة:**
  1. **التسعير الاستباقي الفوري في شاشة المعاينة (Instant In-Modal Auto-Pricing):**
     - عند رفع أي كراسة مقايسة أو مناقصة (BOQ) غير مسعرة (أسعار الفئة فيها فارغة أو صفرية)، يقوم النظام فوراً بمطابقة مواصفات كل بند مع المكتبة الهندسية وتعبئة السعر التقديري الموصى به والتكلفة المباشرة فوراً داخل جدول المعاينة.
  2. **شارة التمييز البصري الذكية (Auto-Priced Status Badge):**
     - تمييز البنود المسعرة آلياً بحدود وحقل أخضر أنيق مع شارة `مسعر آلياً`، مع تحديث إجمالي قيمة العطاء التقديرية والتكلفة ونسبة الجاهزية إلى 100% فورياً.
  3. **التفكيك الكامل لشجرة الموارد (BOM Resource Decomposition):**
     - فك كافة بنود التخصصات الهندسية (خامات، مصنعيات، معدات، مقاولي باطن) مع حساب نسب الهالك، المصاريف الإدارية والعمومية (Overhead %)، وهامش الربح المستهدف (Profit Margin %).
  4. **التعرف الذكي على الأعمدة المتعددة والدمج التلقائي (Multi-Column Description Merging):**
     - معالجة ملفات المقايسات التي تفصل مسمى البند أو الفئة الرئيسية في عمود والتوصيف الفني والهندسي في عمود مجاور، ودمجها بذكاء لاستخراج كامل المواصفات الفنية والمطابقة الدقيقة.
  5. **عزل الديباجات وتوريث السياق الهندسي (Hierarchical Section Context & Preamble Shield):**
     - توريث التخصص الهندسي من رؤوس الأقسام والفصول للأبناء، مع حماية البنود من التوريث الخاطئ لديباجات الملاحظات العامة (Notes / Preamble).
  6. **دعم استيراد وإلحاق الشيتات المتعددة لتجميع تخصصات المشروع (Multi-Sheet Import & Append Mode):**
     - إتاحة خيارين فوريين عند استيراد أي ملف مقايسة جديد: «إلحاق وإضافة للبنود الحالية» لدمج شيتات التخصصات المختلفة (كهرباء، تكييف، صحي، تيار خفيف) في نفس دراسة العطاء، أو «استبدال كامل وتفريغ البنود السابقة».
  7. **تثبيت تابات التخصصات الهندسية (Always-Visible Discipline Filter Tabs):**
     - عرض شريط تصفية التخصصات وإجماليات كل تخصص بصورة دائمة حتى في المشاريع ذات التخصص الواحد، مما يوفر رؤية هندسية واضحة ومحكمة لإجمالي تكلفة وقيمة كل تخصص على حدة.
  8. **الحفظ التلقائي اللحظي لمسودة العطاء (Anti-Refresh Draft Auto-Save):**
     - حفظ كافة البنود الـ 65 والتخصصات والنسب وهوامش الربح تلقائياً في الكاش المحلي للمتصفح؛ بحيث لا تضيع أي بنود أو مجهود حصر عند عمل Refresh أو انقطاع الاتصال مع زر تفريغ المسودة والبدء من جديد.
  9. **دورة حفظ وترسية العطاءات الثنائية (Planning / Bidding vs Active Award Workflow):**
     - إتاحة خيارين فوريين للمهندس في أسفل الشاشة: «حفظ كعطاء قيد الدراسة والتسعير (Planning)» لحفظ المشروع في قاعدة البيانات ومتابعة المفاوضات وعروض الأسعار مع العميل، أو «اعتماد وترسية فورية كمشروع رسمي ساري (Active)» لتحويل المقايسة فوراً إلى SOV تنفيذي لبدء التوريدات والمستخلصات.

---

## 147. منظومة التبويب المستقل للشيتات وعروض الأسعار متعددة الحزم وتصدير الإكسيل المسعر بهوية الشركة والـ PDF المطبوع (Multi-Sheet Preserved Tabs, Zero-Scroll Multi-line Quotations & Priced Excel/PDF Engine)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Enterprise Production Ready)
* **المسارات في الكود:**
  - `frontend/src/features/contracting/components/ClientQuotationModal.tsx`: نافذة عرض السعر المالي الرسمي الشامل مع محدد نطاق العرض الفوري (كافة البنود المجمعة أو حسب ملف وشيت كل تخصص)، ونظام التوزيع الهندسي بدون أي سكرول أفقي (Zero Horizontal Scroll)، وتصدير إكسيل مسعر مطابق لترتيب كراسة العميل، وترويسة ديناميكية مقروءة من إعدادات الشركة (الاسم، الشعار، الهاتف، العنوان، الرقم الضريبي)، وطباعة رسمية A4 معتمدة وموجهة للمالك والضرائب والتأمينات.
  - `frontend/src/features/contracting/utils/originalWorkbookPricer.ts`: محرك التصدير الثنائي الذكي (Dual-Engine Export): يدعم التصدير بهوية وترويسة الشركة الملكية مع حفظ الديباجة والعناوين التبويبية وتسلسل البنود والتحليل المالي للضرائب والتأمينات، أو التصدير بالحقن الموضعي المباشر في شيت العميل الأصلي بدون إزاحة صفوف لمقارنات الاستشاري.
  - `frontend/src/features/contracting/pages/ContractingTenderPage.tsx`: دعم التبويب المستقل لكل ملف وشيت مستورد والحفاظ على التسلسل الأصلي لبنود العميل 100% بدون أي تشتيت، مع زر التصدير المباشر للإكسيل المسعر من شريط أدوات العطاءات.
  - `frontend/src/features/contracting/utils/boqExcelParser.ts`: تخزين ونقل اسم الملف واسم الشيت المصدر (`sourceFileName`, `sourceSheetName`) ورقم الصف الأصلي (`rawRowIdx`) لكل بند مع التصنيف الدقيق لمواسير التبريد والمعدات تحت تخصص التكييف.
* **القدرات والوظائف المعتمدة:**
  1. **تصدير الإكسيل بهوية وترويسة الشركة الديناميكية (Company-Branded Excel Quotation):**
     - جلب بيانات الشركة تلقائياً من إعدادات المنظومة المركزية (الاسم التجاري، الهاتف، العنوان، الرقم الضريبي، اللوجو) ووضع ترويسة مؤسسية فاخرة في أعلى شيت الإكسيل متبوعة بكامل بيانات المشروع، المالك، التاريخ، وتفاصيل النطاق.
  2. **الحفاظ التام على الديباجة، العناوين التبويبية، وتسلسل البنود (WBS & Section Hierarchy Preservation):**
     - تقسيم شيت الإكسيل إلى فصول وتخصصات هندسية واضحة (`【 أعمال القوى والشبكات الكهربائية 】`، `【 أعمال التكييف والتهوية 】`، إلخ) مع الحفاظ التام على ترتيب كراسة العطاء وشروطها التعاقدية وإجماليات التأمينات والقيمة المضافة في أسفل الشيت.
  3. **التبديل بين هوية الشركة والشيت الأصلي النقي (Branded Proposal vs Pure In-Place Toggle):**
     - إتاحة مفتاح تحكم فوري للمهندس: تصدير عرض سعر رسمي بهوية وترويسة الشركة، أو تصدير شيت العميل الأصلي نبيلاً بدون إزاحة خلايا لمطابقة شيتات الاستشاري وماكرو المقارنات.
  4. **التبويب المستقل للشيتات والملفات (File-Level Preserved Tabs):**
     - عند استيراد عدة ملفات مقايسات (مثل شيت التكييف HVAC، شيت مكافحة الحريق Fire Fighting، وشيت الكهرباء)، يتم إنشاء تابة مستقلة لكل ملف باسمه وعدد بنوده، مع الحفاظ التام على الترتيب والتسلسل الأصلي للبنود دون نقل أي بند خارج سياق ملفه الأصلي.
  5. **معيار منع السكرول الأفقي والضبط المتوازي للنصوص (Zero Horizontal Scroll & Responsive Word-Wrap):**
     - تطبيق جدول ثابت هندسياً (`table-layout: fixed`) مع توزيع أعمدة محكم (`<colgroup>`) وضبط متوازي للفقرات (`word-break: break-word`, `overflow-wrap: break-word`) لكافة النصوص والمواصفات الفنية الطويلة باللغتين العربية والإنجليزية لتظهر بسلاسة تحت بعضها بدون أي سكرول بالعرض.
  6. **عروض الأسعار المخصصة والمجمعة (Multi-Package Scope Filter):**
     - إتاحة خيار استخراج وطباعة عرض سعر رسمي مجمع لكافة أعمال المشروع، أو استخراج عرض سعر منفصل لكل ملف أو تخصص مستقل مع إعادة حساب كافة الضرائب والتأمينات والأتعاب الإدارية والإجماليات الخاصة بتلك الحزمة تلقائياً.
  7. **الطباعة الرسمية الفاخرة المعتمدة (Pixel-Perfect A4 Printout & PDF):**
     - قالب طباعة رسمي عالي الدقة يدعم الورق القياسي A4 ومزود بلوجو وبيانات شركتكم من الإعدادات، تفاصيل المشروع والجهة المالكة، جدول المواصفات المضبوط، بوكس التحليل المالي والضرائب، الشروط والمحددات التعاقدية، وثلاث خانات اعتماد رسمية (المكتب الفني، المدير التنفيذي، اعتماد المالك).
  8. **محرك الحقن الموضعي في شيت العميل الأصلي ومقاومة تحديث الصفحة عبر IndexedDB (IndexedDB-Backed In-Place Native BOQ Injection):**
     - تخزين محتوى وBuffer ملفات كراسات المقايسات الأصلية المرفوعة في قاعدة بيانات المتصفح المحلية (`IndexedDB`) بجانب ذاكرة التشغيل، لضمان بقائها حتى عند عمل Refresh أو إعادة تشغيل المتصفح.
     - حقن أسعار الفئات والإجماليات في خاناتها الأصلية بدقة متناهية مع الاحتفاظ الكامل بنسبة 100% بكافة تنسيقات العميل الأصلية وألوانه وخطوطه وديباجته وشروطه بدون المساس بخلية واحدة.
  9. **الشيت المؤسسي الملون الفاخر متعدد الملفات (Branded Enterprise Colored Spreadsheet):**
     - تصميم شيت إكسيل ملون مؤسسي مخصص متكامل يجمع بين هوية الشركة وترويستها الملكية الكحلية (`#170e5e`) وبين المحتوى الأصلي للمقايسة بنفس التسلسل والترتيب، مع تنسيق مالي معتمد للأرقام (`#,##0.00`)، ودمج كامل لأسطر الملاحظات والشروط التعاقدية عبر الأعمدة من A إلى H وتفعيل اتجاه RTL التلقائي، ودعم التصدير المجمع متعدد الملفات بنقرة واحدة.
  10. **زر التصدير المباشر في شاشة العطاء (1-Click Direct Excel Export Action):**
      - زر مخصص مميز باللون الأخضر في شريط إجراءات العطاءات يتيح تصدير شيت العميل المسعر فورياً بضغطة زر واحدة دون الحاجة للدخول لنافذة عرض السعر.
  11. **اعتماد وقبول حالة المشروع عند الحفظ والترسية (`status in CreateProjectDto`):**
      - دعم استقبال والتحقق من حقل `status` (مثل `planning` للعطاءات قيد الدراسة أو `active` للمشاريع المعتمدة والراشية) في `CreateProjectDto` وخدمة المقاولات بالباك إند مع التحقق الصارم (`@IsIn`), مما يمنع خطأ الـ Whitelist Validation ويضمن حفظ العطاءات بحالتها الدقيقة في قاعدة البيانات.
  12. **منع تعارض وتكرار أكواد البنود وحماية القيد الفريد (`BOQ Item Code Collision & Auto-Disambiguation Shield`):**
      - تطبيق آلية ذكية مزدوجة بالفرونت إند والباك إند لمنع تكرار `item_code` داخل نفس المشروع (مثل تكرار الأحرف `a` أو الترقيم الفرعي `1`, `2` بين الشيتات والفصول المختلفة) عبر ترقيم تسلسلي فرعي آلي (`a-2`, `a-3`...), مما يحمي النظام من خطأ كسر القيد الفريد `uq_contracting_boq_tenant_proj_code` ويضمن إدراج وحفظ كافة بنود المقايسة بسلاسة تامة.

---

## 148. دورة حياة العطاءات والمناقصات الهندسية ومسار الترسية والأرشفة المرجعية (Tender Lifecycle, Revisions, Historical Archive & Awarding Pipeline)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Enterprise Production Ready)
* **المعايير الهندسية والتعاقدية المعتمدة:** PMI Construction Extension / FIDIC Red Book / AIA Document A201 General Conditions.
* **المسارات في الكود:**
  - `backend/src/database/migrations/2040000000098_contracting_tender_lifecycle_and_archive.ts`: ترحيل قاعدة البيانات بإضافة حقول دورة حياة العطاء (`loss_reason`, `loss_notes`, `competitor_price`, `revision_number`, `original_tender_id`, `submitted_at`, `awarded_at`, `consultant_name`, `contract_ref`, `contract_date`).
  - `backend/src/modules/contracting/contracting.service.ts` & `contracting.controller.ts`: محرك إدارة دورة حياة العطاءات (إنشاء مراجعات تفاوضية `createTenderRevision`، أرشفة العطاء غير الموفق `markTenderLost`، وترسية العطاء وتحويله لمشروع تنفيذي `awardTender`).
  - `frontend/src/features/contracting/components/ContractingProjectsTab.tsx`: الفصل الهندسي الصارم بين مسار دراسة العطاءات والمشاريع التنفيذية عبر 3 مسارات مستقلة (المشاريع التنفيذية السارية، مسار العطاءات والمناقصات، وأرشيف العطاءات غير المرسّاة).
  - `frontend/src/features/contracting/components/MarkTenderLostModal.tsx`: نافذة أرشفة العطاء وحفظ أسباب عدم الترسية وسعر المنافس الفائز ودروس المستفادة.
  - `frontend/src/features/contracting/components/AwardTenderModal.tsx`: نافذة اعتماد وترسية العطاء رسمياً وتوثيق رقم وتاريخ العقد والاستشاري والدفعة المقدمة ونسبة ضمان الأعمال.
  - `frontend/src/features/contracting/pages/ContractingTenderPage.tsx`: شريط حالة دورة حياة العطاء التفاعلي مع أزرار تقديم العرض، إنشاء مراجعة، ترسية العطاء، أو أرشفته.
  - `frontend/src/shared/components/icons/AppIcons.tsx`: تسجيل أيقونة الأرشيف المعتمدة `ArchiveIcon` لضمان خلو الواجهات التام من أخطاء المكونات غير المعرفة.
* **القدرات والوظائف المعتمدة:**
  1. **الفصل الصارم بين دراسة العطاءات والمشاريع التنفيذية (Tender Study vs Active Project Segregation):**
     - تقسيم شاشة المشاريع إلى 3 مسارات واضحة بتبويبات مستقلة:
       - **المشاريع التنفيذية السارية:** تعرض المشاريع المتعاقد عليها فعلياً والمستمرة في الموقع لتتبع التوريدات واليوميات وإصدار المستخلصات دون أي اختلاط بالعطاءات غير المعتمدة.
       - **مسار العطاءات والمناقصات:** يضم كافة المشروعات قيد الدراسة والتسعير أو بانتظار رد العميل أو الجارية مراجعتها وتفاوضها.
       - **أرشيف العطاءات غير المرسّاة (المرجع التاريخي):** يحتفظ بكافة العطاءات التي تم الاعتذار عنها أو لم يُوفّق فيها دون حذفها نهائياً من قاعدة البيانات.
  2. **نظام المراجعات التفاوضية (Negotiation Revision Engine - Rev-1, Rev-2...):**
     - إمكانية إنشاء مراجعة تفاوضية جديدة بضغطة زر عند طلب العميل تعديل الأسعار؛ يقوم النظام باستنساخ العطاء بكافة بنود مقايسته وتوليد كود مراجعة جديد وتجميد النسخة السابقة، مما يتيح تتبع تاريخ التفاوض بدقة.
  3. **أرشفة العطاءات غير الموفقة كمرجع تسعيري دائم (Permanent Historical Pricing Archive):**
     - حظر حذف أي عطاء تم بذل مجهود في دراسته وتسعيره؛ بل أرشفته مع توثيق سبب عدم الترسية (السعر أعلى من المنافس، إلغاء المشروع من المالك، شروط الدفع، اشتراطات فنية)، وسعر المنافس الفائز ليكون مرجعاً تسعيرياً للمكتب الفني.
  4. **إعادة فتح وتفعيل العطاءات المؤرشفة (Tender Revive Action):**
     - إمكانية إعادة العطاء المؤرشف إلى مسار التفاوض النشط بنقرة زر في حال تواصل العميل مجدداً أو أعاد طرح المشروع.
  5. **محرك الترسية والتحويل إلى مشروع تنفيذي ساري (Formal Awarding & Baseline SOV Lock):**
     - اعتماد وترسية العطاء بضغطة زر وتوثيق رقم العقد، الاستشاري، الدفعة المقدمة، ونسبة ضمان الأعمال (Retention %)، وتحويل العطاء فورياً إلى مشروع تشغيلي ساري في الموقع وتثبيت مقايسته كـ Baseline SOV.
  6. **دعم تحديث العطاءات وهوية العميل وحماية التحقق الصارم (`UpdateProjectDto Whitelist Shield`):**
     - اعتماد حقول `code` و `clientId` و `clientName` في `UpdateProjectDto` ومعالجتها بسلاسة في خدمة المقاولات بالباك إند، مع القضاء التام على خطأ 400 Whitelist Validation عند حفظ وتحديث العطاءات، والالتزام الصارم بالهوية البصرية الرصينة (0 Emojis ودرجات هادئة بدون ألوان فاقعة).
  7. **زر تسعير عطاء جديد، وسجل العطاءات السابقة (Tender Switcher)، وخريطة المسار وسيناريوهات المشروع التفاعلية:**
     - إضافة زر `[ + تسعير عطاء جديد ]` لتفريغ الشاشة الفوري وتهيئتها لاستقبال مقايسة جديدة دون البقاء عالقاً في المشروع القديم.
     - نافذة `TenderSwitcherModal` للبحث والتنقل السريع بين كافة العطاءات المحفوظة قيد الدراسة والمشاريع السارية وأرشيف العطاءات بنقرة واحدة.
     - شريط سياق ذكي أعلى جدول المقايسة يوضح ما إذا كانت الشاشة تعرض مسودة فارغة جديدة أم عطاء محفوظاً مع إمكانية التبديل الفوري.
     - نافذة `ContractingWorkflowMapModal` التفاعلية الشاملة التي ترسم خريطة دورة حياة المشروع من حصر الكميات والتسعير، مروراً بالسيناريوهات الميدانية الثلاثة (التفاوض Rev+، الرفض والأرشفة، والترسية والتحويل لمشروع تنفيذي)، وصولاً إلى التوريدات والمستخلصات والتسليم الختامي.
  8. **دستور ثبات أبعاد النوافذ المنبثقة ومنع رعشة التابات (Zero Modal Ballooning & Zero Tab-Flicker Standard):**
     - تثبيت ارتفاع الحاوية الداخلية لنافذة سجل العطاءات `TenderSwitcherModal` وكافة النوافذ متعددة التابات عند أبعاد ثابتة وصارمة (475px للغلاف الداخلي و 335px لمنطقة القائمة/الجدول)، بحيث تحتفظ النافذة بنفس الأبعاد الثابتة 100% سواء كان التاب يحتوي على 14 عنصراً أو كان فارغاً (0 items)، وتوسيط الحالة الفارغة دون أن ينكمش الفوتر أو تهتز النافذة نهائياً.
     - القضاء التام على رعشة التابات الأفقية والوميض (Tab Flicker) عبر تثبيت وزن الخط `fontWeight: 600` وإلغاء انتقالات CSS البطيئة لتكون الاستجابة لحظية 0ms دون أي تذبذب أفقياً أو رأسياً.
     - اعتماد وتوثيق هذه المعايير كدستور دائم وإلزامي في `AGENTS.md` و `GEMINI.md` لكافة النوافذ والمكونات في المنظومة.

---

## 149. منظومة إدارة مقاولي الباطن، دليل المهن، كشف الحساب المالي التراكمي، وسندات الصرف (Subcontractors Master Directory, Real-Time Financial Ledger & Disbursements Management)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Enterprise Production Ready)
* **المعايير الهندسية والتعاقدية المعتمدة:** الفصل المعماري والمالي الكامل بين الموردين (توريد الخامات والمواد) ومقاولي الباطن (التنفيذ والمصنعيات وحزم الأعمال التخصصية)، مع الامتثال لمحددات القيود المزدوجة وضمان الأعمال (Retention Held) وإدارة كشوف الحساب التحليلية (Statements of Account).
* **المسارات في الكود:**
  - `backend/src/database/migrations/2040000000099_contracting_subcontractors_and_ledger.ts`: ترحيل قاعدة البيانات لإنشاء جدول سجل ودليل مقاولي الباطن `contracting_subcontractors` بكافة تفاصيل النشاط والمهنة والحسابات البنكية، وجدول سندات الصرف والمدفوعات `contracting_subcontractor_payments` بترقيم موحد `SPAY-YYMMDD-XXXX`، مع ترحيل آمن وتزامن متسلسلات الهوية `setval`.
  - `backend/src/database/database.types.ts`: تعريف الأنواع والجداول `ContractingSubcontractorTable` و `ContractingSubcontractorPaymentTable` وربطها بواجهة قاعدة البيانات المركزية.
  - `backend/src/modules/contracting/dto/contracting.dto.ts`: إضافة كائنات نقل البيانات `CreateSubcontractorDto` و `UpdateSubcontractorDto` و `CreateSubcontractorPaymentDto` مع التحقق الصارم من صحة الحقول.
  - `backend/src/modules/contracting/contracting.service.ts` & `contracting.controller.ts`: محرك إدارة مقاولي الباطن (CRUD)، محرك كشف الحساب المالي التحليلي اللحظي `getSubcontractorLedger` لحساب ما له وما عليه وصافي المستحق، ومحرك تسجيل سندات الصرف `createSubcontractorPayment` مع التوليد التسلسلي الموحد.
  - `frontend/src/features/contracting/contracting.types.ts`: تعريف الأنواع للواجهات الأمامية `ContractingSubcontractor` و `ContractingSubcontractorPayment` و `SubcontractorLedgerResponse`.
  - `frontend/src/features/contracting/api/contracting.api.ts`: دوال الاستدعاء والربط السحابي للخدمات المالية ودليل المقاولين وسندات الصرف.
  - `frontend/src/features/contracting/components/SubcontractorsDirectoryModal.tsx`: دليل وسجل مقاولي الباطن مع شريط تصفية التخصصات المهنية والبحث الفوري وعرض ملخص العقود وصافي المستحقات.
  - `frontend/src/features/contracting/components/CreateSubcontractorModal.tsx`: نافذة تسجيل مقاول باطن جديد بالتخصص والبيانات الضريبية والتجارية والبنكية.
  - `frontend/src/features/contracting/components/SubcontractorLedgerModal.tsx`: كشف حساب مقاول الباطن («ما له وما عليه») مع 5 بطاقات KPI فورية، سجل حركات تفصيلي برصيد تراكمي Running Balance، خيارات فلترة بالمشروع والتاريخ، وطباعة كشف حساب معتمد.
  - `frontend/src/features/contracting/components/RecordSubcontractorPaymentModal.tsx`: نافذة تسجيل وصرف دفعات مقاولي الباطن نقداً أو بتحويل بنكي أو شيك، مع تحديث فوري لكشف الحساب وعقود الإسناد.
  - `frontend/src/features/contracting/components/CreateSubcontractModal.tsx`: ربط أوامر الإسناد الحصري بدليل مقاولي الباطن بدلاً من قائمة الموردين، مع زر إضافة مقاول سريع (+ مقاول جديد) والتحديد التلقائي.
  - `frontend/src/features/contracting/components/ContractingSubcontractsTab.tsx`: أزرار الوصول السريع لدليل المقاولين وصرف الدفعات بأعلى الشاشة، وأزرار سريعة بكل سطر عقد لكشف الحساب والصرف الفوري، مع ضبط توزيع الأعمدة `<colgroup>`.
* **القدرات والوظائف المعتمدة:**
  1. **الفصل المعماري الصارم بين الموردين ومقاولي الباطن (Suppliers vs Subcontractors Isolation):**
     - التمييز الجوهري بين موردي الخامات (مواد، أسمنت، حديد، أجهزة) ومقاولي الباطن (أصحاب الحزم التخصصية والمصنعيات)، وإنشاء سجل مستقل تماماً بمحددات وبيانات متخصصة لمقاولي الباطن.
  2. **دليل وسجل مقاولي الباطن التخصصي (Subcontractors Master Directory):**
     - سجل مركزي يضم التخصص المهني (خرسانات، مباني، لياسة، كهرباء، صحي، عزل، تكييف، دهانات، سيراميك، واجهات، مصاعد)، السجل التجاري، الرقم الضريبي، الحساب البنكي والـ IBAN، التقييم، والحالة.
  3. **كشف الحساب المالي التحليلي اللحظي («ما له وما عليه» - Real-Time Ledger):**
     - كشف حساب تراكمي دقيق يحسب:
       - **ما له (Credits):** مستخلصات الأعمال المعتمدة (IPCs).
       - **ما عليه (Debits):** الدفعات والتحويلات المسددة للمقاول + الخصومات والتحميلات الميدانية (Back-charges).
       - **المبالغ المحتجزة:** ضمان الأعمال المحتجز (Retention Held).
       - **صافي الرصيد المستحق (Net Balance Due):** رصيد لحظي يبين بدقة المتبقي للمقاول أو عليه.
  4. **سجل الحركات والرصيد التراكمي (Chronological Running Balance Ledger):**
     - جدول حركات مالي مرتب زمنياً يعرض نوع الحركة (مستخلص، دفعة مسددة، خصم ميداني)، رقم السند والمستند، المشروع، المدين والدائن، والرصيد التراكمي بعد كل حركة مع فلترة متقدمة بالمشروع والتاريخ وزر طباعة مخصص.
  5. **سندات صرف ودفعات مقاولي الباطن (Subcontractor Disbursements Engine):**
     - نافذة متكاملة لتسجيل صرف الدفعات نقدياً أو عبر تحويل بنكي أو شيك مع توثيق رقم الحوالة/الشيك وتاريخ الاستحقاق والخصم المباشر من مستحقات المقاول وتحديث كشف الحساب لحظياً.
  6. **التوليد التسلسلي الموحد لسندات الصرف (`SPAY-YYMMDD-XXXX`):**
     - التزام تام بدستور الترقيم الموحد لجميع سندات الصرف الصادرة من المنظومة.
  7. **تكامل الإسناد الفوري (+ مقاول جديد داخل نافذة الإسناد):**
     - إمكانية إنشاء مقاول باطن جديد مباشرة من داخل شاشة إسناد الأعمال واختياره تلقائياً دون الحاجة لمغادرة الشاشة أو فقدان البيانات المدخلة.

---

## 150. محرك توليد الجداول الزمنية الشامل متعدد الحزم والتخصصات والمزامنة التلقائية مع المقايسة (Universal Multi-Trade & Modular Schedule Engine)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Enterprise Production Ready)
* **المعايير الهندسية والتعاقدية المعتمدة:** PMI Construction Extension / WBS / Critical Path Method (CPM) / CSI MasterFormat Trade Packages.
* **المسارات في الكود:**
  - `frontend/src/features/contracting/components/ScheduleGeneratorModal.tsx`: محرك التوليد الزمني الشامل مع دعم 8 قوالب هندسية قياسية، وباني الحزم الموديولار المخصص (16 حزمة متخصصة)، وخاصية التعرف الذكي والمزامنة الفورية مع مقايسة المشروع التعاقدية (Sync from Project BOQ).
* **القدرات والوظائف المعتمدة:**
  1. **التوافق التام مع أي مقاول وأي حزمة تعاقدية (Any Contractor, Any Trade Scope):**
     - القضاء التام على حصر الجداول الزمنية في المقاول العام فقط؛ حيث يتيح النظام توليد جداول زمنية دقيقة 100% لأي مقاول تخصصي منفرد (مثل مقاول أعمال الكهرباء والتيار الخفيف، مقاول السباكة والصحي، مقاول التكييف HVAC، مقاول الحريق، مقاول العوازل، مقاول المحارة، مقاول الدهانات، إلخ) أو مقاول الحزم المدمجة.
  2. **حزمة أعمال الكهرباء والتيار الخفيف المتكاملة (Full Electrical & Low Current Pipeline):**
     - توليد تسلسلي دقيق لمسار مقاول الكهرباء من اليوم الأول:
       - شبكة التأريض العمومية للأساسات (Earthing Grid) وربطها بالقواعد.
       - تمديد وتثبيت خراطيم الـ PVC ومخارج الإنارة بحديد الأسقف قبل الصب لكل دور.
       - شق الحوائط، علب الماجيك، وصناديق لوحات التوزيع الفرعية.
       - مسارات التيار الخفيف (داتا، كاميرات، إنذار حريق).
       - سحب وتدكيك الأسلاك والكابلات المعزولة.
       - تجميع اللوحات وتثبيت القواطع (DB Dressing).
       - تركيب الوشوش والبرايز ووحدات الإنارة والكشافات.
       - اختبارات العزل ومقاومة التأريض والميجر (Megger Test) وإطلاق التيار والتسليم.
  3. **باقة القوالب التخصصية الجاهزة (8 Ready Preset Templates):**
     - مشروع متكامل تسليم مفتاح (Full Turnkey).
     - هيكل إنشائي وعظم ومباني (Structure & Shell).
     - تشطيبات متكاملة لكافة التخصصات (Full Finishing).
     - كهروميكانيك متكامل (Full MEP: كهرباء + صحي + تكييف + حريق).
     - أعمال كهربائية وتيار خفيف فقط (Electrical Only).
     - أعمال صحية وتغذية وصرف فقط (Plumbing Only).
     - أعمال تكييف وتهوية فقط (HVAC Only).
     - تشطيبات معمارية فقط (Architectural Finishes).
  4. **الباني الموديولار المخصص وشبكة الـ 16 حزمة تفاعلية (Custom Modular Builder):**
     - شبكة اختيارات تفاعلية بـ 16 حزمة عمل مستقلة تمكن المقاول من تفعيل أو إلغاء أي حزمة، وتوليد جدول زمني مترابط تلقائياً لحزم تعاقده فقط.
  5. **التعرف والمزامنة الذكية مع مقايسة المشروع (Sync from Project BOQ):**
     - فحص فوري لبنود المقايسة التعاقدية للمشروع (`contractingApi.getBoqItems`) واكتشاف التخصصات الموجودة فعلياً وتحديد حزم الجدول الزمني آلياً بنقرة واحدة.
  6. **حساب التواريخ والمسار الحرج (Dynamic CPM & Cascade Scheduling):**
     - حساب مدد الأنشطة والتواريخ ومسار الأنشطة الحرجة (Critical Path) والتسلسل المنطقي بدقة تامة وبدون أي تداخلات أو فجوات عشوائية.

---

## 151. إحكام المعمارية التعاقدية الشاملة وتوحيد هوية مقاولي الباطن وتفكيك خصومات المستخلصات (Phase 0 Contracting Architecture Hardening)
* **حالة الموديول العامة:** 🟢 مغلق ومحكم 100% بنجاح كامل ومثبت باختبارات التدقيق المالي الآلية (Enterprise Production Ready & Audited).
* **المعايير الهندسية والتعاقدية المعتمدة:** FIDIC Red/Yellow Books / Double-entry Ledger Invariants / Single Source of Truth Identity / Zero Dead-Code Standard.
* **المسارات في الكود:**
  - `backend/src/database/migrations/2040000000100_contracting_phase0_reconciliation_and_schema_hardening.ts`: هجرة توحيد المفاتيح الخارجية وإلغاء الارتباط بـ `suppliers`، وإضافة قيود `fk_subcontracts_subcontractor` و `fk_invoices_subcontractor` بصيغة `NOT VALID` لتمكين إضافة الهيكل قبل الترحيل البياني. إضافة 23 عموداً تعاقدياً جديداً لشروط العقد المتقدمة، وتفكيك صندوق الخصومات المجمعة إلى أعمدة مستقلة وجدول سندات الخصم المفصلة `contracting_ipc_deductions`.
  - `backend/src/database/migrations/2040000000101_contracting_phase0_closure_composite_fks_and_checks.ts`: هجرة إغلاق المرحلة صفر الشاملة: جدول التعيين الدائم للهويات التاريخية `contracting_subcontractor_id_map`، فرض قيود المفاتيح الأجنبية المركبة المعزولة على مستوى المستأجر `(tenant_id, subcontractor_id)` بصيغة `NOT VALID` تمهيداً للترحيل مع حماية السجلات المالية `ON DELETE RESTRICT`، إضافة 10 قيود فحص `CHECK` لكافة المسميات التعاقدية وصحة نسب استرداد الدفعة المقدمة وعدم سلبية القيم المالية.
  - `backend/src/database/migrations/2040000000102_contracting_phase0_backfill_mapping_and_reconciliation.ts`: هجرة الترحيل البياني الفعلي (DML Data Migration & Backfill): تسجيل لقطة تدقيق قبلية لعدد الصفوف والمبالغ المالية لكل مستأجر (`_pre_recon_subcontracts`, `_pre_recon_invoices`)، نقل وترحيل كافة سجلات الموردين التاريخية المرتبطة بعقود ومستخلصات مقاولات إلى `contracting_subcontractors`، وتعبئة جدول التعيين الدائم `contracting_subcontractor_id_map`، وتحديث مراجع المفاتيح الخارجية، والتحقق البعدي الإلزامي من انعدام الفروقات (Zero Row Loss & Zero Value Drift)، والتحقق الصارم الصريح من قيود المفاتيح (`VALIDATE CONSTRAINT`) دون أي ابتلاع للأخطاء.
  - `backend/src/database/database.types.ts`: تحديث تعريفات Kysely لكافة الجداول المتأثرة وإضافة واجهة `ContractingIpcDeductionTable`.
  - `backend/src/modules/contracting/ipc-calculation.engine.ts`: المحرك الحسابي المركزي المستقل والنقي (Pure Functional Single Source of Truth) لحساب كافة طبقات المستخلصات الأربع طبقاً لمعايير FIDIC/MRICS مع تطبيق صارم لكافة الثوابت الحاكمة ($I_1, I_2, I_3, I_{12}$)، وتوليد لقطة التدقيق `calc_inputs_snapshot`.
  - `backend/src/modules/contracting/dto/contracting.dto.ts`: توسيع DTOs لعقود مقاولي الباطن والمستخلصات وبنود الحصر، مع إضافة خاصية `advanceRecoveryOverride?: boolean` للتحكم الصارم في التجاوز اليدوي.
  - `backend/src/modules/contracting/contracting.service.ts`: استدعاء المحرك المركزي `computeIpc` لحساب مبالغ المستخلص واشتقاق نسبة الإنجاز $P_n$ من واقع الدفاتر وقاعدة البيانات، مع ترحيل العجز المدين وحفظ المستخلص وبنود الخصم المفصلة.
  - `backend/test/critical/contracting-ipc-invariants.spec.ts`: جناح اختبارات تدقيق محاسبي آلي يستورد ويختبر محرك الإنتاج `computeIpc` مباشرة، يثبت بالبرهان الرياضي والسيناريوهات الرقمية انضباط الثوابت I1 و I2 و I3 و I12 والمسار الافتراضي بدون تجاوز بنسبة 100%.
  - `frontend/src/features/contracting/contracting.types.ts`: تحديث الأنواع وإضافة `SubcontractType` و `PaymentLinkageMode`.
  - `frontend/src/features/contracting/components/CreateSubcontractModal.tsx`: إضافة قسم شروط التعاقد والتحكم المالي FIDIC مع إمكانية ضبط نافذة استرداد الدفعة المقدمة (بدء الاسترداد 10% ونهايته 80%)، وسقف الضمان وسقف غرامات التأخير.
  - `frontend/src/features/contracting/components/CreateIpcInvoiceModal.tsx`: تطهير الواجهة من فرض استرداد 10% الثابت، والاعتماد التلقائي على نافذة العقد التعاقدية (Bounded Window)، مع خيار التجاوز اليدوي عند الحاجة فقط، وحساب الضمان حصرياً على الأعمال المنفذة GWD.
  - `AGENTS.md`: ترسيخ وتوسيع القاعدة السيادية رقم 13 (دستور التدقيق الحسابي ومنع الأخطاء الصامتة والكود الميت) لتشمل استيراد كود الإنتاج في الاختبارات، ونمط القيود المؤجلة `NOT VALID`، وحظر ابتلاع أخطاء التحقق، واشتراط تقرير التسوية القبلي والبعدي.
* **القدرات والوظائف المعتمدة:**
  1. **توحيد هوية مقاول الباطن والترحيل البياني الكامل (Unified Subcontractor Identity & DML Backfill):**
     - ترحيل آلي كامل لكافة مقاولي الباطن من جدول الموردين `suppliers` إلى جدول مقاولي الباطن الموحد `contracting_subcontractors`.
     - توثيق دائم لكافة روابط الهوية القديمة والجديدة في `contracting_subcontractor_id_map` مع قفل وتحقق قيود المفاتيح الأجنبية المركبة `(tenant_id, subcontractor_id)`.
  2. **العزل التعددي الصارم وحماية القيود المالية (Multi-Tenant Composite FKs & CHECK Constraints):**
     - فرض قيود مفاتيح أجنبية مركبة تشترط تطابق المستأجر `(tenant_id, subcontractor_id)` وتمنع الإسناد بين مستأجرين مختلفين.
     - تطبيق قاعدة `ON DELETE RESTRICT` الصارمة لحظر حذف أي مقاول باطن ترتبط به عقود أو مستخلصات مالية لحماية القيود اليومية والمستندات المحاسبية.
     - 10 قيود فحص `CHECK` تضمن منطقية نسب استرداد الدفعة المقدمة (`advance_recovery_end_pct > advance_recovery_start_pct`) وعدم سلبية المبالغ وصحة قيم التعدادات.
  3. **المحرك الحسابي الرباعي للمستخلصات المحصن بالثوابت الرياضية (FIDIC Standard 4-Layer IPC Engine with Invariants):**
     - **الطبقة الأولى (Gross Certified Work):** جمع إجمالي الأعمال المنفذة والفروق السعرية (Escalation) والمواد في الموقع (MOS) مع اشتقاق نسبة الإنجاز $P_n$ من السجل التراكمي.
     - **الطبقة الثانية (Contractual Holdbacks):** استرداد الدفعة المقدمة بنافذة مقيدة زمنياً وتقدمياً وفق المعادلة التعاقدية مع تطبيق سقف إلزامي يحظر أي استرداد يتجاوز الرصيد المتبقي للدفعة المقدمة (Invariant I1). وحساب حجز الضمان حصرياً على الأعمال المنفذة الفعلية GWD دون المواد ودون الفروق السعرية وبما لا يتجاوز سقف الضمان (Invariant I2).
     - **الطبقة الثالثة (Project Set-Offs):** تجميع دقيق لكافة بنود الخصومات والمحملات الميدانية عبر تجميع المصفوفات (Map Aggregation) لمنع إسقاط السجلات المتشابهة، وتطبيق سقف غرامات التأخير التعاقدي (Invariant I3)، والتحقق من التطابق التام بين الإجماليات والتفاصيل (Invariant I12).
     - **الطبقة الرابعة (Statutory Taxes & Carry-Forward):** تسوية الضرائب على الوعاء المعتمد الفعلي، وترحيل أي رصيد سالب إلى المستخلص التالي عبر `carried_forward_debit_out` مع تدوين لقطة الحسابات الكاملة `calc_inputs_snapshot`.
  4. **انعدام الكود الميت وتوافق الواجهة الأمامية مع فلسفة المحرك (Zero Dead-Code Alignment):**
     - التخلص التام من أي قيم افتراضية تعطل المنطق الخلفي في الواجهات الأمامية، وإتاحة نافذة استرداد الدفعة المقدمة Bounded Window كمسار افتراضي أساسي، مع دعم التجاوز اليدوي عند الحاجة.
  5. **جناح الاختبارات البرمجية المحاسبية المعتمد المباشر لكود الإنتاج (Production-Linked Automated Audit Suite):**
     - تغطية شاملة لجميع ثوابت المحرك في `contracting-ipc-invariants.spec.ts` تستورد مباشرة المحرك الحقيقي `computeIpc` وتثبت بنسبة 100% أن العمليات المالية والمحاسبية مطبقة ومختبرة حسابياً على كود الإنتاج الفعلي.

---

## 152. منظومة خطابات الضمان البنكية والبوابة الرقابية G1 وحجب المستخلصات والغطاء التراكمي (Phase 1: Bank Guarantees, Gateway G1, Invariant G1-C, Expiry Alerts & IPC Interlocking)
* **حالة الموديول العامة:** 🟢 مغلق ومحكم 100% ومعتمد كوداً وتدقيقاً محاسبياً واختبارات آلية (Phase 1 Fully Audited & Signed Off).
* **المعايير الهندسية والتعاقدية المعتمدة:** FIDIC Red Book / URDG 758 (Uniform Rules for Demand Guarantees) / MRICS Financial Controls / Gateway G1 Strict Advance Protection / Cumulative Exposure Invariant G1-C / Objective Substantive Inference.
* **المسارات في الكود:**
  - `backend/src/database/migrations/2040000000103_contracting_guarantees_and_gateway_g1.ts`: ترحيل قاعدة البيانات لإنشاء جدول خطابات الضمان `contracting_guarantees` بروابط مركبة `(tenant_id, id)`، وتقييد الحذف `ON DELETE RESTRICT` على المشروعات والعقود والمقاولين لمنع إسقاط التزامات بنكية قائمة، مع قيود CHECK لأنواع الضمانات والحالات والتواريخ.
  - `backend/src/database/migrations/2040000000104_contracting_g1_cumulative_and_composite_fks.ts`: هجرة إغلاق المرحلة الأولى التجميعية: إضافة عمودي `payment_category` و `status` لجدول `contracting_subcontractor_payments` مع قيود `CHECK` وفهرس مركب، إضافة قيد `UNIQUE (tenant_id, id)` على `contracting_projects`، وإعادة بناء المفتاح الأجنبي لخطابات الضمان مركباً `fk_guarantees_project_tenant` على مستوى المستأجر والمشروع معاً، والمصادقة الصريحة النهائية `VALIDATE CONSTRAINT` على القيود المؤجلة الثلاثة من المرحلة صفر (`fk_invoices_subcontract_tenant`, `fk_ipc_deductions_invoice_tenant`, `fk_backcharges_subcontractor_tenant`).
  - `backend/src/database/database.types.ts`: تعريف واجهة Kysely `ContractingGuaranteeTable` و `ContractingSubcontractorPaymentTable` مع أعمدة التصنيف والحالة وربطها بنوع `Database`.
  - `backend/src/modules/contracting/guarantee-gateway.engine.ts`: المحرك الرقابي المستقل والنقي (Pure Functional Gateway G1 Engine) لتطبيق فحص البوابة G1 ومنع صرف الدفعات المقدمة بدون غطاء بنكي كافٍ وسارٍ، وفرض ثابت الغطاء التراكمي **Invariant G1-C** لحظر الدفعات الجزئية المتعددة التي تتجاوز سقف الضمان بمجموعها، وفحص حجب المستخلصات IPC Interlocking عند وجود رصيد دفعة مقدمة غير مسترد مع انتهاء الضمان، ومحرك حساب وتصنيف التنبيهات المبكرة T-60 و T-30 و T-7.
  - `backend/src/modules/contracting/dto/contracting.dto.ts`: إضافة كائنات نقل البيانات `CreateGuaranteeDto`, `UpdateGuaranteeDto`, `ExtendGuaranteeDto`, `ReleaseGuaranteeDto`, `InvokeGuaranteeDto`, وتحديث `CreateSubcontractorPaymentDto` بدعم تصنيف `operational_advance`.
  - `backend/src/modules/contracting/contracting.service.ts` & `contracting.controller.ts`: دوال دورة حياة خطابات الضمان، والربط الفعلي بـ 4 نقاط إنتاجية، وتطبيق الاستدلال الموضوعي الصارم على الخادم (`isSubstantivelyAdvance`) لمنع التجاوز الصامت للبوابة من القوائم المنسدلة، وحساب رصيد الصرف التراكمي السابق، وإبراز التصنيفات المالية في كشف حساب المقاول، وفحص حجب المستخلصات `checkIpcGuaranteeInterlocking` في `createInvoice` و `approveInvoice`.
  - `backend/test/critical/contracting-guarantee-gateway.spec.ts`: جناح اختبارات آلي يستورد محرك الإنتاج النقي مباشرة (11 سيناريو تدقيقي ناجح 100%)، يغطي حالات الصرف الفردي والتراكمي وسد ثغرة الدفعات المتعددة التتابعية واختبارات حجب المستخلص والتنبيهات المبكرة.
  - `frontend/src/features/contracting/contracting.types.ts`: تعريف أنواع `ContractingGuarantee` و `GuaranteeExpiryAlert` والتعدادات.
  - `frontend/src/features/contracting/api/contracting.api.ts`: دوال الاستدعاء السحابي لخطابات الضمان وإجراءات التمديد والإفراج والتسييل.
  - `frontend/src/features/contracting/components/CreateGuaranteeModal.tsx`: نافذة تسجيل وتفعيل خطاب ضمان بنكي جديد وربطه بالمشروع أو عقد مقاول الباطن.
  - `frontend/src/features/contracting/components/GuaranteesDirectoryModal.tsx`: دليل وسجل خطابات الضمان البنكية مع بطاقات المؤشرات الإحصائية والتنبيهات المبكرة (T-60 / T-30 / T-7)، تبويبات التصفية، وإجراءات التمديد البنكي والإفراج والتسييل المدمجة دون تشتت.
  - `frontend/src/features/contracting/components/RecordSubcontractorPaymentModal.tsx`: دعم تصنيفات الدفعات (دفعة جارية، دفعة مقدمة خاضعة لـ G1، سلفة تشغيلية نقدية للموقع معفاة من G1).
  - `frontend/src/features/contracting/components/ContractingSubcontractsTab.tsx`: زر وصول مباشر بالهيدر لخطابات الضمان البنكية، وأزرار سريعة بكل سطر عقد لمتابعة وتفعيل الضمانات.
* **القدرات والوظائف المعتمدة:**
  1. **البوابة الرقابية الأولى ومعيار الغطاء التراكمي (Gateway G1 & Cumulative Invariant G1-C):**
     - صمام أمان مالي يحظر بشكل قاطع صرف أي دفعة مقدمة (Advance Payment) لمقاول أو مشروع ما لم يتوفر خطاب ضمان بنكي سارٍ يغطي كامل **إجمالي المنصرف التراكمي السابق مضافاً إليه الدفعة الحالية المطلوبة** (`priorAdvanceDisbursed + requestedAmount <= guaranteeAmount`).
     - حماية كاملة ومثبتة برمجياً من سيناريو الانكشاف المالي الناتج عن صرف دفعات مقدمة جزئية متعددة على مراحل.
  2. **الاستدلال الموضوعي الصارم بالخادم (Objective Server-Side Substantive Inference):**
     - حظر تجاوز البوابة بالصمت أو الخطأ: إذا كانت الدفعة غير مرتبطة بمستخلص، والعقد ينص على دفعة مقدمة لم تستنفد بالكامل، يستدل الخادم على أنها دفعة مقدمة ويخضعها للبوابة G1 قسراً حتى لو ترك المستخدم التصنيف على وضعه الافتراضي.
  3. **حجب المستخلصات والربط المحكم (IPC Guarantee Interlocking):**
     - عند وجود رصيد دفعة مقدمة غير مسترد للمقاول (`unrecoveredAdvanceBalance > 0`)، يفحص النظام صلاحية وسريان خطاب الضمان البنكي. وإذا انتهت صلاحية الضمان قبل اكتمال الاسترداد، يتم حجب اعتماد وصرف المستخلص فورياً مع إطلاق إشعار حظر صريح يطالب بتمديد الخطاب بنكياً أولاً.
  4. **محرك التنبيهات المبكرة المتدرج زمنيّاً (T-60 / T-30 / T-7 Early Expiry Alerts):**
     - نظام إنذار مبكر يحسب الأيام المتبقية على انتهاء أي خطاب ضمان ويصنفها:
       - **T-60 (إشعار استباقي):** تنبيه أزرق قبل 60 يوماً للمتابعة المبكرة.
       - **T-30 (تحذير إجرائي):** تنبيه برتقالي قبل 30 يوماً لبدء إجراءات التمديد مع البنك.
       - **T-7 (إنذار حرج أحمر):** تحذير أحمر عاجل قبل 7 أيام لتفادي ضياع حق تسييل أو تجديد الخطاب.
       - **Expired (منتهي الصلاحية):** وسم أحمر داكن يوجب التمديد الفوري أو الحجب.
  5. **إدارة دورة حياة خطابات الضمان المتكاملة (Full Lifecycle Actions):**
     - تسجيل الخطاب برقم بنكي رسمي والربط بالبنك والمشروع والعقد.
     - تمديد بنكي سريع يحدث تاريخ الانتهاء ويعيد تفعيل الخطاب إذا كان منتهياً.
     - إفراج رسمي عند انتهاء الالتزام التعاقدي ورد أصل الخطاب للبنك.
     - تسييل ومصادرة مع توثيق الأسباب القانونية وتاريخ المعاملة والمبلغ المصادر.
  6. **الامتثال لدستور الاختبارات والبرهان الحسابي (Rule 13 Invariant Proof):**
     - 11 سيناريو تدقيق واختبار آلي في `contracting-guarantee-gateway.spec.ts` تستورد مباشرة كود الإنتاج النقي بدون تكرار، مع بقاء كافة اختبارات المرحلة صفر خضراء بنسبة 100%.

---

## 153. محرك توزيع المصاريف غير المباشرة للموقع والربحية الحقيقية لبنود المقايسة (Track 1: Field Indirect Costs & Distributables Allocation Engine - AACE RP 10S-90 / 34R-05)
* **حالة الموديول العامة:** 🟢 مكتمل 100% ومختبر تدقيقياً ومحاسبياً مع تغطية شاملة لكافة الثوابت (Track 1 Production-Ready & Invariant-Audited).
* **المعايير الهندسية والمحاسبية المعتمدة:** AACE International Recommended Practice 10S-90 / 34R-05 / Activity-Based Costing (ABC) / Single Source of Truth Invariant / Strict Raw Materials Exclusion Guard / Zero-Production Mobilization Carry-Forward / Subcontractor Backcharge Recovery.
* **المسارات في الكود:**
  - `backend/src/database/migrations/2040000000105_contracting_field_indirect_cost_allocation.ts`: ترحيل قاعدة البيانات لإنشاء جداول أوعية التكاليف `contracting_cost_pools`، ودفعات التوزيع الدورية `contracting_allocation_batches`، والمصاريف الميدانية غير المباشرة `contracting_indirect_expenses`، وتوزيعات البنود التفصيلية `contracting_boq_indirect_allocations`، وإضافة أعمدة التكلفة الفعلية الشاملة في جدول البنود `contracting_boq_items` (`allocated_indirect_cost`, `direct_labor_cost`, `direct_material_cost`, `direct_equipment_cost`, `total_actual_cost`) مع قيود المفاتيح المركبة `(tenant_id, id)` وحظر الحذف `ON DELETE RESTRICT`.
  - `backend/src/modules/contracting/field-indirect-allocation.engine.ts`: المحرك الحسابي النقي والمستقل (Pure Functional Engine) لتوزيع مصاريف الأوعية الأربعة على بنود المقايسة، ومعالجة صفرية الإنتاج في مرحلة التجهيز بترحيل الرصيد (`deferred_out`) دون خطأ قسمة على صفر، واستيعاب الرصيد المرحل في الفترات المنتجة اللاحقة (`deferred_in`)، والخصم المسبق لاستقطاعات مقاولي الباطن (Backcharges) لمنع الازدواجية، وحظر استخدام تكلفة المواد كمحرك توزيع لمنع تشويه التكاليف، مع مطابقة السنتات وجبر الفروق الحسابية بالمليم (Zero Discrepancy Reconciliation Invariant).
  - `backend/src/modules/contracting/contracting.service.ts` & `contracting.controller.ts`: دوال إدارة أوعية التكاليف (التهيئة التلقائية للأوعية الأربعة، تسجيل المصاريف، معاينة التوزيع اللحظي، اعتماد وترحيل الدفعات بالترقيم الموحد `ALLOC-YYMMDD-XXXX`، وتحديث التكلفة الفعلية للبنود ومحرك تحليل الربحية).
  - `backend/test/critical/contracting-indirect-allocation.spec.ts`: جناح اختبارات تدقيقي محاسبي يستورد المحرك النقي مباشرة ويختبر 8 سيناريوهات تدقيقية بنجاح 100%.
  - `frontend/src/features/contracting/contracting.types.ts`: تعريف أنواع `CostPool`, `IndirectExpense`, `AllocationBatch`, `BatchAllocationSummary` وتحديث `BoqProfitabilityItem`.
  - `frontend/src/features/contracting/api/contracting.api.ts`: دوال الاستدعاء السحابي لإدارة أوعية التكاليف والمصاريف الميدانية والمعاينة والاعتماد.
  - `frontend/src/features/contracting/components/FieldIndirectCostModal.tsx`: نافذة شاملة ومحكمة لإدارة أوعية التكاليف والمصاريف، وتشغيل دورات التوزيع الدورية مع المعاينة اللحظية للمحمل على كل بند، وسجل الدفعات التاريخية.
  - `frontend/src/features/contracting/components/BoqProfitabilityModal.tsx`: تحديث جدول تحليل الربحية ليعرض تفاصيل التكلفة الفعلية المباشرة وغير المباشرة الموزعة وهامش الربح الحقيقي.
  - `frontend/src/features/contracting/components/ContractingBoqTab.tsx`: زر وصول سريع بنافذة شريط الأدوات المتقدمة لفتح محرك توزيع المصاريف غير المباشرة.
* **القدرات والوظائف المعتمدة:**
  1. **أوعية التكاليف الأربعة المعتمدة دولياً (The 4 AACE Cost Pools):**
     - الوعاء 1 ($P_1$) - سكن وإعاشة وانتقالات العمالة: محرك التوزيع أيام العمل الفعلية ($D_{\text{labor}}$) من واقع كشوف الحضور.
     - الوعاء 2 ($P_2$) - أعباء وبدلات وتأمينات العمالة: محرك التوزيع تكلفة الأجور المباشرة ($C_{\text{labor}}$).
     - الوعاء 3 ($P_3$) - المعدات العامة ومولدات الموقع والمحروقات المشتركة: محرك التوزيع ساعات تشغيل المعدات ($D_{\text{equip}}$).
     - الوعاء 4 ($P_4$) - إدارة وإشراف الموقع وكرفانات المهندسين: محرك التوزيع الجهد الميداني المباشر ($\text{Direct Effort} = \text{Labor} + \text{Equipment}$).
  2. **الحظر الدستوري لاستخدام تكلفة المواد كمحرك توزيع (Strict Materials Exclusion):**
     - استبعاد تكلفة المواد الموردة تماماً من مقام التوزيع لمنع تحميل بند به توريد معدات كبرى أو تشيلر مستورد بحصة جائرة من رواتب مهندسي الموقع.
  3. **معالجة فترة التجهيز وصفرية الإنتاج (Mobilization Carry-Forward):**
     - عند صرف مصاريف تأسيسية قبل بدء إنتاج بنود المقايسة، يرحل الرصيد بالكامل تلقائياً للفترة التالية دون القسمة على صفر أو التوزيع العشوائي.
  4. **الخصم المسبق لاستقطاعات مقاولي الباطن (Subcontractor Backcharges Deduction):**
     - خصم أي مبالغ تم تحميلها على المقاولين مقابل السكن أو النقل من إجمالي الوعاء لتوزيع الصافي فقط ومنع الازدواج المحاسبي.
  5. **الثابت المحاسبي الحاكم ومطابقة السنتات (Rule 13 Invariant Proof):**
     - مطابقة تامة بين صافي مصاريف الأوعية والمبالغ الموزعة والرصيد المرحل بنسبة خطأ 0.000%.

---

## 154. موديول اللوجستيات وحركات المناديب والأسطول والشحن ودستور الترقيم الموحد (Phase 8: Logistics, Fleet & Maritime Invariants & Universal Document Numbering)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Phase 8 Production-Ready & Invariant-Audited).
* **المعايير الهندسية والتشغيلية المعتمدة:** Rule 10 Universal Document Numbering (`PREFIX-YYMMDD-XXXX`) / Rule 13 Financial & Operational Audit Protocol / Pessimistic Row Locking (`forUpdate`) / Race-Condition Prevention (Pattern F1) / Atomic Numbering via Returning Identity (Pattern F6) / Single Source of Truth / 0 Emojis Policy.
* **المسارات في الكود:**
  - `backend/src/modules/delivery-reps/delivery-reps.service.ts`:
    1. تحصين دالة تسوية أوردرات المناديب `settleOrder` بنقل استعلام الفاتورة لداخل المعاملة `this.db.transaction()` مع فرض قفل تشاؤمي `forUpdate()`، مما يقضي جذرياً على خطر التسوية المزدوجة المتزامنة وحركات الخزينة والقيود المحاسبية المضاعفة (Pattern F1).
    2. تحصين دالة تسليم المندوب الميدانية `driverSettleOrder` وتغليفها بمعاملة ذرية وقفل تشاؤمي لمنع التكرار المتزامن عند النقر المتكرر.
  - `backend/src/modules/delivery-reps/van-sales.service.ts`:
    1. توحيد وإلزام معيار الترقيم الموحد للوثائق `formatDailyDocumentNumber` لجميع حركات مبيعات سيارات التوزيع:
       - فواتير البيع الميداني: `VAN-YYMMDD-XXXX`.
       - سندات التحصيل النقدي الميداني: `COL-YYMMDD-XXXX`.
       - أذونات مرتجعات العملاء الميدانية: `RET-YYMMDD-XXXX`.
    2. القضاء على نمط الترقيم العشوائي بـ `Date.now().slice(-6)` وربط أرقام الوثائق بالمعرف الذاتي الحقيقي في جدول المبيعات والمدفوعات والأستاذ المساعد.
  - `backend/src/modules/maritime-freight/maritime-freight.service.ts`:
    1. القضاء التام على ثغرة ترقيم `COUNT(*) + 1` (Open Item O9 / Pattern F6) في كافة عمليات الشحن البحري:
       - طلبات الاستفسار الملاحي: `INQ-YYMMDD-XXXX`.
       - طلبات عروض الأسعار للخطوط: `RFQ-YYMMDD-XXXX`.
       - عروض أسعار العملاء: `QUO-YYMMDD-XXXX`.
       - أوامر تشغيل الشحنات: `JOB-YYMMDD-XXXX`.
  - `backend/src/database/database.types.ts`: تعريف الأنواع والجداول `ContractingSubcontractorTable` و `ContractingSubcontractorPaymentTable` وربطها بواجهة قاعدة البيانات المركزية.
  - `backend/src/modules/contracting/dto/contracting.dto.ts`: إضافة كائنات نقل البيانات `CreateSubcontractorDto` و `UpdateSubcontractorDto` و `CreateSubcontractorPaymentDto` مع التحقق الصارم من صحة الحقول.
  - `backend/src/modules/contracting/contracting.service.ts` & `contracting.controller.ts`: محرك إدارة مقاولي الباطن (CRUD)، محرك كشف الحساب المالي التحليلي اللحظي `getSubcontractorLedger` لحساب ما له وما عليه وصافي المستحق، ومحرك تسجيل سندات الصرف `createSubcontractorPayment` مع التوليد التسلسلي الموحد.
  - `frontend/src/features/contracting/contracting.types.ts`: تعريف الأنواع للواجهات الأمامية `ContractingSubcontractor` و `ContractingSubcontractorPayment` و `SubcontractorLedgerResponse`.
...
* **المسارات في الكود:**
  - `backend/src/database/migrations/2040000000105_contracting_field_indirect_cost_allocation.ts`: ترحيل قاعدة البيانات لإنشاء جداول أوعية التكاليف `contracting_cost_pools`، ودفعات التوزيع الدورية `contracting_allocation_batches`، والمصاريف الميدانية غير المباشرة `contracting_indirect_expenses`، وتوزيعات البنود التفصيلية `contracting_boq_indirect_allocations`، وإضافة أعمدة التكلفة الفعلية الشاملة في جدول البنود `contracting_boq_items` (`allocated_indirect_cost`, `direct_labor_cost`, `direct_material_cost`, `direct_equipment_cost`, `total_actual_cost`) مع قيود المفاتيح المركبة `(tenant_id, id)` وحظر الحذف `ON DELETE RESTRICT`.
  - `backend/src/modules/contracting/field-indirect-allocation.engine.ts`: المحرك الحسابي النقي والمستقل (Pure Functional Engine) لتوزيع مصاريف الأوعية الأربعة على بنود المقايسة، ومعالجة صفرية الإنتاج في مرحلة التجهيز بترحيل الرصيد (`deferred_out`) دون خطأ قسمة على صفر، واستيعاب الرصيد المرحل في الفترات المنتجة اللاحقة (`deferred_in`)، والخصم المسبق لاستقطاعات مقاولي الباطن (Backcharges) لمنع الازدواجية، وحظر استخدام تكلفة المواد كمحرك توزيع لمنع تشويه التكاليف، مع مطابقة السنتات والجبر الفروق الحسابية بالمليم (Zero Discrepancy Reconciliation Invariant).
  - `backend/src/modules/contracting/contracting.service.ts` & `contracting.controller.ts`: دوال إدارة أوعية التكاليف (التهيئة التلقائية للأوعية الأربعة، تسجيل المصاريف، معاينة التوزيع اللحظي، اعتماد وترحيل الدفعات بالترقيم الموحد `ALLOC-YYMMDD-XXXX`، وتحديث التكلفة الفعلية للبنود ومحرك تحليل الربحية).
  - `backend/test/critical/contracting-indirect-allocation.spec.ts`: جناح اختبارات تدقيقي محاسبي يستورد المحرك النقي مباشرة ويختبر 8 سيناريوهات تدقيقية بنجاح 100%.
  - `frontend/src/features/contracting/contracting.types.ts`: تعريف أنواع `CostPool`, `IndirectExpense`, `AllocationBatch`, `BatchAllocationSummary` وتحديث `BoqProfitabilityItem`.
  - `frontend/src/features/contracting/api/contracting.api.ts`: دوال الاستدعاء السحابي لإدارة أوعية التكاليف والمصاريف الميدانية والمعاينة والاعتماد.
...
* **القدرات والوظائف المعتمدة:**
  ...
  3. **معالجة فترة التجهيز وصفرية الإنتاج (Mobilization Carry-Forward):**
     - عند صرف مصاريف تأسيسية قبل بدء إنتاج بنود المقايسة، يرحل الرصيد بالكامل تلقائياً للفترة التالية دون القسمة على صفر أو التوزيع العشوائي.
  4. **الخصم المسبق لاستقطاعات مقاولي الباطن (Subcontractor Backcharges):**
     - خصم أي مبالغ تم تحميلها على المقاولين مقابل السكن أو النقل من إجمالي الوعاء لتوزيع الصافي فقط ومنع الازدواج المحاسبي.
  5. **الثابت المحاسبي الحاكم ومطابقة السنتات (Rule 13 Invariant Proof):**
     - مطابقة تامة بين صافي مصاريف الأوعية والمبالغ الموزعة والرصيد المرحل بنسبة خطأ 0.000%.
...
* **المسارات في الكود:**
  ...
  - `backend/src/modules/maritime-freight/maritime-freight.service.ts`:
    1. القضاء التام على ثغرة ترقيم `COUNT(*) + 1` (Open Item O9 / Pattern F6) في كافة عمليات الشحن البحري:
       ...
       - عروض أسعار العملاء: `QUO-YYMMDD-XXXX`.
       - أوامر تشغيل الشحنات: `JOB-YYMMDD-XXXX`.
    2. اعتماد الترقيم الذري عبر المعاملات وبمعرف السجل الذاتي (`returningAll()`)، والربط المباشر مع مراكز التكلفة المحاسبية بنظام الأبعاد.
  - `backend/src/modules/bosta/bosta.service.ts` & `gcc-shipping/gcc-shipping.service.ts`:
    - التحقق التدقيقي من عزل المستأجرين الصارم `tenant_id` وحساب مبالغ التحصيل COD بدقة (تصفير مبالغ الشحنات المسددة إلكترونياً واعتماد قيمة الفاتورة لطلبات الدفع عند الاستلام).
  - `backend/test/critical/phase8-logistics.spec.ts`:
    - إنشاء جناح اختبارات تدقيقي صارم يغطي 6 سيناريوهات حتمية: الترقيم الموحد لجميع الوثائق، حساب هوامش البيع ومنع البيع السالب، تسوية المناديب وحساب عمولات أسطول المتجر والمناديب المستقلين، وقواعد COD لبوسطة، ومطابقة حركة الأستاذ المساعد لمدفوعات ومردودات العملاء، وحراس حظر التسوية المكررة.
* **فحص الجودة والسلامة البرمجية:**
  - جناح الاختبارات الحاسم `npm run test:critical` (27 جناح اختبار): **اجتياز بنسبة 100% بنجاح تام (Exit code 0)**.

---

## 155. موديول الوحدات القطاعية ونقاط البيع والورديات والتصنيع والصيدليات والاستبدال (Phase 9: Sectoral Invariants, Manufacturing WAC, Pharmacy FEFO & Universal Numbering)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Phase 9 Production-Ready & Invariant-Audited).
* **المعايير الهندسية والتشغيلية المعتمدة:** Rule 10 Universal Document Numbering (`PREFIX-YYMMDD-XXXX`) / Rule 13 Financial & Operational Audit Protocol / Pessimistic Row Locking (`forUpdate`) / Race-Condition Prevention (Pattern F1) / Atomic Numbering via Returning Identity (Pattern F6) / Pharmacy FEFO Engine / 0 Emojis Policy.
* **المسارات في الكود:**
  - `backend/src/modules/cash-drawer/helpers/cash-drawer.helper.ts` & `cash-drawer.service.ts`:
    1. توحيد ترقيم وثائق الورديات بصيغة `SHIFT-YYMMDD-XXXX` عبر `formatDailyDocumentNumber('SHIFT', shiftId)`.
    2. فرض القفل التشاؤمي `forUpdate()` على جدول `cashier_shifts` أثناء تسجيل الحركات النقدية وإغلاق الوردية لمنع تضارب التعديلات وفقدان التحديثات (Lost Updates).
    3. آلية الإغلاق الأعمى (Blind Close) للكاشير ومراجعة واعتماد المدير (Maker-Checker Governance).
  - `backend/src/modules/manufacturing/services/manufacturing.service.ts`:
    1. فرض قفل تشاؤمي `forUpdate()` على جدول `products` عند اكتمال أوامر التصنيع `completeWorkOrder` لحماية احتساب متوسط التكلفة المرجح WAC للمنتج التام من التسابق المتزامن.
    2. توحيد ترقيم أوامر التفكيك `createUnbuildOrder` إلى `UB-YYMMDD-XXXX` وربط الحركات المخزونية برقم المعرف الحقيقي `reference_id: unbuildId` بدلاً من القيمة الصفرية.
  - `backend/src/modules/tradein/tradein.service.ts`:
    1. القضاء التام على ثغرة ترقيم `COUNT(*) + 1` واعتماد الترقيم الموحد الذري `TRD-YYMMDD-XXXX`.
    2. ربط حركات المخزون الناتجة عن شراء الأجهزة المستعملة برقم العملية الحقيقي `txnId` مباشرة مع تقييد الأجهزة برقم السيريال IMEI في `product_serials`.
    3. إثبات صرف القيمة من الخزينة بحركة مصروفات فورية.
  - `backend/src/modules/pharmacy/pharmacy.service.ts`:
    1. توحيد ترقيم الروشتات والتشغيلات: `RX-YYMMDD-XXXX` و `BATCH-YYMMDD-XXXX`.
    2. بناء محرك FEFO (First Expired, First Out) المعتمد في `getBatchesFEFO` لصرف التشغيلات الأقرب انتهاءً أولاً واستبعاد أو حجر التشغيلات منتهية الصلاحية قطيعاً.
  - `backend/test/critical/phase9-sectoral.spec.ts`:
    - إنشاء جناح اختبارات تدقيقي يغطي 6 سيناريوهات حتمية: الترقيم الموحد للوثائق القطاعية (SHIFT, UB, TRD, RX, BATCH)، إعادة احتساب WAC في التصنيع، تفكيك المنتجات واستعادة المكونات والتكلفة، عجز وفائض الورديات، محرك FEFO الصيدلاني، وتقييم الاستبدال وحركات الخزينة.
* **فحص الجودة والسلامة البرمجية:**
  - جناح الاختبارات الحاسم `npm run test:critical` (28 جناح اختبار): **اجتياز بنسبة 100% بنجاح تام (Exit code 0)**.

---

## 156. موديول الأصول الثابتة ومراكز التكلفة وإعادة تقييم العملات والحوكمة (Phase 10: Fixed Assets, Cost Centers, Forex Revaluation & Accounting Governance)
* **حالة الموديول العامة:** 🟢 مكتمل ومحكم 100% (Phase 10 Production-Ready & Invariant-Audited).
* **المعايير الهندسية والتشغيلية المعتمدة:** Rule 10 Universal Document Numbering (`PREFIX-YYMMDD-XXXX`) / Rule 13 Financial & Operational Audit Protocol / Pessimistic Row Locking (`forUpdate`) / Race-Condition Prevention (Pattern F1) / IAS 21 §23 Multi-Currency Accounting Standard / Zero `as any` Casting / 0 Emojis Policy.
* **المسارات في الكود:**
  - `backend/src/modules/accounting/accounting.service.ts`:
    1. فرض قفل تشاؤمي `forUpdate()` على جدول `fixed_assets` في دالة `depreciateFixedAsset` لمنع تضارب التحديثات وازدواجية إثبات قيود الإهلاك عند تشغيل إهلاك متزامن (Pattern F1).
    2. إحكام معادلات القسط الثابت والقسط المتناقص وسقف الأساس القابل للإهلاك عند قيمة الخردة (`cost - salvage_value`) ومنع تجاوز مجمع الإهلاك للقيمة الدفترية.
    3. حماية حذف الأصول: حظر الحذف الصلب للأصول التي تم إهلاكها ولها سجلات في `asset_depreciation_logs` وتحويلها إلى حالة متقاعد (`status = 'retired'`) لحماية قيود اليومية واتصال سجل الأستاذ العام.
  - `backend/src/modules/accounting/services/cost-center-allocations.service.ts` & `database.types.ts`:
    1. محاذاة أنواع الأعمدة في `CostCenterAllocationSplitTable` لتقبل `BIGINT` مع الكود بأمان تام.
    2. إزالة جميع كاستات `(this.db as any)` غير الآمنة والاعتماد على الاستعلامات المطبوعة بالكامل.
    3. إحكام التحقق الصارم من مجموع نسب التوزيع لتساوي 100% بدقة ($\pm 0.05\%$) وحظر حفظ نسب غير متطابقة.
  - `backend/src/modules/accounting/services/forex-revaluation.service.ts`:
    1. اكتشاف وتصحيح خطأ فادح في المخطط (Schema Bug): تصحيح الاستعلام بالبحث في `name_ar` بدلاً من العمود غير الموجود `name` في جدول `accounting_accounts` والذي كان محجوباً بكاست `(this.db as any)`، وإزالة الكاست نهائياً.
    2. تطبيق معيار الترقيم الموحد للوثائق على قيد تسوية فروق العملة بصيغة `FX-USD-YYMMDD-XXXX` عبر `formatDailyDocumentNumber('FX-USD', entryId, periodDate)`.
    3. التحقق من مطابقة معيار المحاسبة الدولي IAS 21 §23 بإعادة تقييم البنود النقدية فقط (`is_monetary = true`) والمعرفة بعملة التقييم `revaluation_currency`.
  - `backend/test/critical/phase10-governance-assets.spec.ts`:
    - إنشاء جناح اختبارات تدقيقي يغطي 6 سيناريوهات حتمية: حسابات القسط الثابت والمتناقص وسقف الخردة، قفل الأصول ومنع الإهلاك المزدوج، توزيع مراكز التكلفة وإحكام نسبة الـ 100%، إعادة تقييم العملات الأجنبية وتوليد قيد التسوية بالترقيم الموحد، منع حذف الأصول ذات السجلات وترقيتها إلى متقاعدة، وحوكمة وتدقيق أبعاد التكاليف.
* **فحص الجودة والسلامة البرمجية:**
...
* **تدقيق مسار الاستدعاء الفعلي وإصلاحات الفوترة السحابية (تدقيق لاحق):**
  - `tenant-subscription.controller.ts`: مسار `POST /sandbox-checkout/complete` كان بلا مصادقة ويقبل `tenantId`/`planId`/`amount` من جسم الطلب مباشرة → ترقية باقة مجانية لأي مستأجر دون دفع فعلي. أُصلح بإضافة `SessionAuthGuard`، واشتقاق `tenantId` حصراً من الجلسة الموثّقة، وحارس `assertSandboxGatewayUsable()` يعطّل المحاكاة كلياً عند تفعيل أي مفتاح بوابة دفع حقيقي (XPay/Paymob/Stripe).
  - `xpay.gateway.ts` / `paymob.gateway.ts` / `stripe.gateway.ts` (`verifyAndParseWebhook`): كان التحقق التشفيري "فشلاً آمناً معكوساً" (`isValid = true` افتراضياً عند غياب سر التحقق من البيئة)؛ Stripe لم يكن يتحقق من التوقيع إطلاقاً. أُصلح بجعل الفشل آمناً (`isValid = false` افتراضياً)، وتحقق Stripe الحقيقي بصيغة `t=...,v1=...` على الجسم الخام (`req.rawBody`، مُلتقط الآن في `main.ts`) مع حماية ضد إعادة التشغيل (نافذة 300 ثانية)، ومقارنات `timingSafeEqual` في الجميع.
  - راجع `ARCHITECTURE_INVARIANTS.md` القسم 3 (F11, F12) والقسم 5-ب ("منصة SaaS والفوترة السحابية") والقسم 8 (O11, O12) للتفصيل الكامل وما تبقى مفتوحاً (جناح اختبار مخصص لم يُكتب بعد).

## 158. تدقيق مسار الاستدعاء وإصلاحات المصادقة عبر المراحل 6-10 (Cross-Phase Controller/Guard Call-Path Audit)
* **حالة التدقيق:** فحص شامل لكل Controller ضمن المراحل 6 (الموارد البشرية)، 7 (الأقساط/التسعير/CRM)، 8 (اللوجستيات/الشحن)، 9 (القطاعيات)، 10 (الأصول/المحاسبة). **نظيف بالكامل:** المراحل 7، 9، 10 (حارس صفي واحد شامل في كل Controller، لا `@Controller` ثانٍ مخفٍ، لا استثناءات).
* **ثغرات مُصلحة (المرحلتان 6 و8):**
  - `mobile-attendance.controller.ts`: مساران إداريان (`branches/:id/geofence`, `employees/:id/pin`) بلا أي حارس مصادقة ولا تحقق توكن — الأول كان بلا شرط `tenant_id` في الاستعلام إطلاقاً (كتابة عابرة للمستأجرين بلا مصادقة). أُصلح بإضافة `SessionAuthGuard` + `PermissionsGuard` + `RequirePermissions('hrAttendance')`، واشتقاق `tenantId` عبر `requireTenantScope`.
  - `employee-portal.controller.ts` (بوابة الموظف): كان يقبل PIN افتراضي معروف (`'1234'` أو آخر 4 أرقام من الهاتف) بلا أي حد لمحاولات الدخول → انكشاف رواتب/سلف/عُهد الموظفين بالتخمين. أُصلح بإزالة البديل الافتراضي كلياً وإضافة `LoginAttemptLimiter` (`common/utils/login-attempt-limiter.ts`، قفل 15 دقيقة بعد 5 محاولات فاشلة).
  - `maritime-public-tracking.controller.ts`: بوابة عروض أسعار الناقلين العامة (`rfq/:id`, `rfq/:id/bid`) كانت تتحقق بالمفتاح الأساسي التسلسلي المشترك بين كل المستأجرين بلا أي رمز سرّي — يتيح تعداد وقراءة/حقن بيانات مناقصات أي مستأجر. أُصلح بعمود جديد `maritime_rfqs.public_quote_token` (هجرة 119) يُضمَّن في رابط دعوة الناقل ويُتحقق منه إلزامياً قبل أي عرض أو إرسال بيانات.
* **فحص الجودة والسلامة البرمجية:** `npx tsc --noEmit` (Backend + Frontend) نظيف بلا أخطاء؛ `npm run test:critical` (30 جناح) اجتياز 100%؛ هجرة 119 نُفِّذت بنجاح على قاعدة التطوير.
* **جولة إغلاق بنود مفتوحة (نفس اليوم):**
  - **O1** (إعادة تقييم العملات): أُضيف `isMonetary`/`revaluationCurrency` إلى `PUT /api/accounting/accounts/:id` — أول مسار كتابة فعلي لهذا العمود منذ هجرة 112.
  - **O3** (مسار استلام مخزون يتجاوز GRNI): تحقَّقتُ أن `POST /api/purchase-orders/:id/receive` **بلا أي مستدعٍ من الواجهة الأمامية إطلاقاً** (كود ميت خطير)، فحذفته بالكامل (Controller + Service + DTO + عميل API الأمامي) بدل توحيده — مسار الاستلام الوحيد الآن هو `convertToBill` ثم `POST /api/purchases/:id/receive-goods` المُحصَّن بالكامل (GRN/GRNI/مطابقة ثلاثية).
  - **O14** (بلا حد محاولات دخول): `LoginAttemptLimiter` نفسه مُفعَّل الآن أيضاً في `driver-portal` (`delivery-reps.service.ts:driverLogin`) و`mobile-punch` (`mobile-attendance.service.ts:employeeLogin`).
...
## 167. استعادة كلمة المرور الذاتية لمالك المنشأة (Self-Service Password Reset)
* **حالة الوحدة / المعيار:** 🟢 مكتملة (23 سبتمبر 2026) — الثوابت PWR-1..PWR-6 في `ARCHITECTURE_INVARIANTS.md` §2.11، ومحروسة بـ`password-reset.spec.ts` و`ForgotPasswordPage.spec.tsx` و`ResetPasswordPage.spec.tsx` داخل `npm run guards`.
* **الفجوة التي أُغلقت:** لم يكن في النظام أي مسار لاستعادة كلمة المرور إطلاقاً. رابط «نسيت كلمة المرور؟» في شاشة الدخول كان `href="#"` معطَّلاً بـ`preventDefault`، ولا مسار واحد في الباك إند — أي أن مالك منشأة ينسى كلمته كان يحتاج تدخّلاً يدوياً على قاعدة الإنتاج.
* **ما يستطيعه المستخدم الآن:**
  1. من شاشة الدخول ← «نسيت كلمة المرور؟» ← `/forgot-password`، يُدخل البريد المسجَّل لمالك المنشأة (وكود المنشأة اختيارياً حين يملك نفس البريد أكثر من منشأة).
  2. تصله رسالة بالهوية البصرية للنظام (عربي/إنجليزي) فيها اسم المستخدم ورابط تعيين كلمة مرور جديدة، صالح **30 دقيقة** ويعمل **مرة واحدة**. لو كان البريد مرتبطاً بأكثر من منشأة، تحمل الرسالة رابطاً مستقلاً لكل منشأة باسمها.
  3. الرابط يفتح `/reset-password`، تتحقق الصفحة من الرمز قبل عرض النموذج، ثم يضبط كلمة المرور الجديدة (مع تأكيدها).
  4. بعد النجاح: **كل الجلسات المفتوحة على هذا الحساب على كل الأجهزة تُنهى**، ويستطيع الدخول فوراً بالكلمة الجديدة.
* **قرارات النطاق:**
  - البريد مخزَّن على المنشأة (`tenants.owner_email`) لا على المستخدم، فالاستعادة بالبريد متاحة **لمالك المنشأة** (`admin` / `super_admin`) كما أن الدخول بالبريد متاح له وحده. بقية المستخدمين يعيد لهم مدير المنشأة كلمة المرور من شاشة المستخدمين، كما هو الحال اليوم.
  - الرابط مخفي في واجهة الديسكتوب (الإلكترون) لأن الاستعادة بالبريد مفهوم سحابي، بلا أي منطق ديسكتوب داخل مسارات المصادقة السحابية.
* **الأمان (تفصيله في §2.11):** الرمز لا يُخزَّن (تُخزَّن تجزئته فقط) · الرد واحد لبريد موجود وغير موجود فلا تُحصَر قائمة عملائنا · استهلاك واحد تحت `FOR UPDATE` · الرمز في الـfragment فلا يدخل سجلات البروكسي · حدود محاولات بعنوان العميل الحقيقي · صفّا تدقيق `AUTH_PASSWORD_RESET_REQUESTED` و`AUTH_PASSWORD_RESET_COMPLETED`.
* **الملفات:**
  - `backend/src/core/auth/utils/password-reset-token.ts` · `backend/src/modules/sessions/password-reset.service.ts` · `backend/src/modules/sessions/password-reset-mail.service.ts` · `backend/src/common/mail/platform-mail.service.ts` · `backend/src/modules/sessions/dto/password-reset.dto.ts` · `backend/src/modules/sessions/sessions.controller.ts`
  - هجرة `2040000000141_password_reset_tokens`
  - `frontend/src/features/auth/pages/ForgotPasswordPage.tsx` · `ResetPasswordPage.tsx` · `components/AuthScreenShell.tsx` · `hooks/useForgotPasswordForm.ts` · `hooks/useResetPasswordForm.ts`
* **التحقق:** الهجرة جُرِّبت `up → down → up` على قاعدة تطوير حقيقية، والمسار جُرِّب حياً من طرف لطرف (طلب ⇒ رمز مُصدَر · بريد مجهول ⇒ نفس الرد بلا رمز · تحقق · تأكيد ⇒ تغيير كلمة المرور وحذف الجلسات · إعادة استخدام الرمز ⇒ مرفوضة · الدخول بالكلمة الجديدة ⇒ ينجح). `npm run guards` أخضر بالكامل.
* **إعدادات البيئة:** `PASSWORD_RESET_TOKEN_TTL_MINUTES` (افتراضي 30، أقصى 120) · يعتمد على `APP_PUBLIC_URL` لبناء الرابط وعلى إعدادات `SMTP_*` / `MAIL_*` القائمة نفسها التي يستخدمها بريد النسخة التجريبية.

---

## 168. تقليص زمن فقد البيانات وأتمتة تمرين الاسترجاع (Backup RPO Reduction & Automated Restore Drill)
* **حالة الوحدة / المعيار:** 🟢 مكتملة (23 سبتمبر 2026) — الثوابت DEPLOY-7 و DEPLOY-8 في `ARCHITECTURE_INVARIANTS.md` §2.7، محروسة بـ`deploy-pipeline.spec.ts` داخل `npm run guards`، والحارسان جُرِّبا بكسر متعمَّد.
* **الفجوتان اللتان أُغلقتا:**
  1. **التواتر:** النسخ كان يومياً فقط (03:00)، فعطل الساعة الخامسة مساءً يضيّع يوم عمل كامل — فواتير ومستخلصات وحضور.
  2. **التحقق:** الاسترجاع كان مُجرَّباً ثلاث مرات فعلياً وموثّقاً في `docs/DISASTER_RECOVERY.md`، لكن **بقرار بشري كل 3 شهور**. بين التجربتين يمكن أن يفسد الملف أو تتغير كلمة السر أو يتوقف الرفع بلا أن يلاحظ أحد.
* **ما صار يعمل الآن:**
  - **نسخة كل ساعة** (`backup.sh hourly` — الدقيقة 30 من كل ساعة): `pg_dump` مضغوط، **مشفَّر قبل أن يغادر السيرفر** بنفس خوارزمية الملف المجمّع، بلا تجميع حزم العملاء (الجزء الثقيل). يُحفظ محلياً 3 أيام وعلى Google Drive 7 أيام، ويحذف نسخه القديمة بنفسه. **أقصى ما يضيع صار ساعة بدل 24.**
  - **تمرين استرجاع كل أحد 4:15** (`restore-drill.sh`): يأخذ أحدث نسخة موجودة فعلاً (ساعية أو يومية أو مجمّعة)، يفكّ تشفيرها، يسترجعها في قاعدة **منفصلة مؤقتة** داخل نفس الحاوية، ثم يقارنها بالإنتاج: عدد الجداول · الجداول الحرجة غير فارغة (`tenants`, `users`, `accounting_accounts`, `products`) · عدد الهجرات (نسخة أقدم من المخطط = فشل) · **عدد المفاتيح المرجعية** (نمط O65 الذي أسقط قيداً بصمت). ثم يحذف القاعدة المؤقتة ويرسل تنبيه تليجرام عند أي فشل.
  - **المراقبة:** `zsystems-watch.sh` صار ينبّه على انقطاع النسخة الساعية (3 ساعات) وعلى تمرين استرجاع فشل أو لم يعمل من 9 أيام — وكلاهما يُفحص فقط على سيرفر مُثبَّتة عليه فعلاً، فلا تنبيه على إعداد لم يُطلب بعد.
* **قرار صريح — لماذا النسخة الساعية لا تُرفع إلى Oracle Object Storage:** رابط الـPAR للكتابة فقط ولا يستطيع حذف القديم، فرفع 24 ملفاً يومياً يملأ حاوية Free Tier بلا حدّ. الحماية الجغرافية الكاملة تبقى على النسخة اليومية (محلي + Oracle + Google Drive)، والساعية تحمي من الأعطال والحذف الخاطئ وفساد القاعدة عبر (محلي + Google Drive).
* **السلامة:** تمرين الاسترجاع لا يكتب على قاعدة الإنتاج إطلاقاً — كل استعلاماته عليها `SELECT`، ولا يوجد في السكربت `DROP`/`TRUNCATE`/`DELETE`/`UPDATE` على أي قاعدة غير قاعدة التمرين، والقاعدة المؤقتة تُحذف حتى عند فشل التمرين (`trap … EXIT`). هذا كله مفروض بالحارس.
* **الملفات:** `deploy/scripts/zsystems-backup.sh` · `deploy/scripts/zsystems-restore-drill.sh` (جديد) · `deploy/scripts/zsystems-watch.sh` · `backend/test/critical/deploy-pipeline.spec.ts` · `docs/DISASTER_RECOVERY.md` §6 و§7.
* **التثبيت على السيرفر مطلوب** (السكربتات في المستودع، والـcron يدوي): ثلاثة أسطر cron موثّقة في `docs/DISASTER_RECOVERY.md` §6.
* **المتبقي بقرار المالك:** O73 — استرجاع نقطة-في-الزمن الحقيقي (أرشفة WAL) ينزل بالفقد إلى دقائق، لكنه يمسّ إعداد حاوية Postgres ويستهلك مساحة مستمرة.

## 169. المصادقة الثنائية — TOTP (Two-Factor Authentication)
* **حالة الوحدة / المعيار:** 🟢 مكتملة (23 سبتمبر 2026) — الثوابت MFA-1..MFA-6 في `ARCHITECTURE_INVARIANTS.md` §2.12، محروسة بـ`totp-mfa.spec.ts` + `session-auth.spec.ts` (MFA-1) + `LoginPageMfa.spec.tsx` داخل `npm run guards`.
* **الفجوة التي أُغلقت:** النظام كان يعتمد على كلمة المرور وحدها لتسجيل الدخول. سرقة كلمة المرور (تسريب، تخمين، إعادة استخدام عبر مواقع) كانت تكفي للدخول الكامل بما فيه حساب المنصة المركزية (`super_admin`). لا يوجد عامل ثانٍ يحمي الحساب حتى لو سُرقت كلمة المرور.
* **ما يستطيعه المستخدم الآن:**
  - تفعيل المصادقة الثنائية من **صفحة الحساب** (كارت مخصّص) عبر ربط تطبيق مصادقة (Google Authenticator / Authy / 1Password / أي تطبيق TOTP).
  - عند تسجيل الدخول بكلمة المرور، النظام يطلب رمز التحقق من التطبيق (6 أرقام، متجدد كل 30 ثانية) **قبل إنشاء الجلسة**.
  - رموز استرداد (10 رموز) تُعرض مرة واحدة عند التفعيل، تُستخدم عند فقدان الهاتف.
  - تجديد رموز الاسترداد من صفحة الحساب (يحتاج رمز TOTP صالح).
  - إيقاف المصادقة الثنائية (يحتاج كلمة المرور الحالية + رمز صالح معاً — جلسة مسروقة وحدها لا تكفي).
* **قرارات النطاق:**
  - **بلا حزمة خارجية:** محرك TOTP مكتوب يدوياً ومُتحقَّق بمتجهات RFC 6238 الرسمية. أقل سطح هجوم في مسار المصادقة.
  - **بلا QR افتراضياً:** رابط `otpauth://` يفتح تطبيق المصادقة مباشرة على الهاتف + السر معروض في مجموعات للإدخال اليدوي. إضافة حزمة QR متاحة بقرار المالك.
  - **الفرض الإجباري مقفول:** `MFA_REQUIRED_FOR_PLATFORM=false` افتراضياً. فرضه فوراً يقفل المالك خارج النظام قبل تسجيل التطبيق. يُفعَّل يدوياً بعد ربط الحساب.
* **الأمان:**
  - السر مشفَّر AES-256-GCM بمفتاح في البيئة (`MFA_ENCRYPTION_KEY` أو `SESSION_SECRET`) — لا مفتاح افتراضي في السحابة (فشل مغلق 500).
  - كلمة المرور الصحيحة لحساب مفعّل لا تُنشئ جلسة — تحدٍّ HMAC موقَّع عمره 5 دقائق فقط.
  - مقارنة الرمز بـ`timingSafeEqual` (ثابتة الزمن) — لا `===`.
  - منع إعادة استعمال الرمز عبر `last_used_step`.
  - رموز الاسترداد مخزَّنة مجزَّأة SHA-256 ولا مسار يعيدها بعد لحظة العرض الأولى.
* **الملفات:**
  - `backend/src/core/auth/utils/totp.ts` — محرك TOTP (RFC 6238)
  - `backend/src/core/auth/utils/mfa-secret-cipher.ts` — تشفير السر + رموز الاسترداد
  - `backend/src/modules/sessions/mfa.service.ts` — خدمة التفعيل/الإيقاف/التجديد
  - `backend/src/modules/sessions/dto/mfa.dto.ts` — عقود البيانات
  - `backend/src/modules/sessions/sessions.controller.ts` — المسارات (`mfa/status`, `mfa/setup`, `mfa/confirm`, `mfa/disable`, `mfa/recovery-codes`, `login/mfa`)
  - `backend/src/core/auth/services/session.service.ts` — بوابة الدخول (`hasConfirmedMfa` → `completeMfaLogin`)
  - `backend/src/database/migrations/2040000000142_user_mfa.ts` — الهجرة
  - `frontend/src/shared/hooks/useTwoFactor.ts` — هوك الواجهة
  - `frontend/src/shared/components/TwoFactorCard.tsx` — كارت التفعيل في صفحة الحساب
  - `frontend/src/features/auth/pages/LoginPage.tsx` — خطوة الرمز في شاشة الدخول
* **التحقق:**
  - `backend/test/critical/totp-mfa.spec.ts` — 7 جنايح: Base32، متجهات RFC 6238، نافذة القبول، توليد الأسرار، الرابط، التشفير/فك التشفير، رموز الاسترداد.
  - `backend/test/critical/session-auth.spec.ts` (MFA-1) — كلمة المرور الصحيحة لحساب مفعّل لا تُنشئ جلسة.
  - `frontend LoginPageMfa.spec.tsx` — 6 اختبارات واجهة لخطوة الرمز.
  - حُرّاس الواجهة الكاملة: 63 ملف، 288 اختبار، كلها خضراء.
  - حُرّاس الباك إند: كلها خضراء.
  - حُرّاس المعمارية: 9 حُرّاس، كلها خضراء.
* **إعدادات البيئة:**
  - `MFA_ENCRYPTION_KEY` (اختياري) — مفتاح تشفير مخصّص لأسرار TOTP. إن لم يُحدَّد يُشتق من `SESSION_SECRET`.
  - `MFA_REQUIRED_FOR_PLATFORM` (اختياري، `false` افتراضياً) — فرض المصادقة الثنائية على حسابات المنصة المركزية.
  - `MFA_ISSUER_NAME` (اختياري، `Z Systems Pro` افتراضياً) — اسم المُصدِر في تطبيق المصادقة.

## 170. الفترات المحاسبية الشهرية وإقفال الشهور (Monthly Fiscal Periods & Closing — O6)
* **حالة الوحدة / المعيار:** 🟢 مكتملة (23 سبتمبر 2026) — الثوابت FP-1..FP-6 في `ARCHITECTURE_INVARIANTS.md` §2.13، محروسة بـ`fiscal-periods.spec.ts` داخل `npm run guards`.
* **الفجوة التي أُغلقت (البند O6):** النظام كان يعتمد على تاريخ إقفال شامل (`lock_date_all`) أو إقفال السنة المالية بالكامل فقط. لم يكن هناك مفهوم معتمد للفترات الشهرية (12 فترة للسنة المالية)، مما كان يمنع المحاسبين والمديرين الماليين من إقفال الشهور السابقة وتجميد الحسابات والإقرارات الضريبية شهرياً بشهر، مع ترك الباب مفتوحاً لإضافة أو تعديل قيود في شهور سابقة مُعتمدة.
* **ما يستطيعه المستخدم الآن:**
  - عرض الفترات الشهرية الـ 12 لكل سنة مالية عبر زر "الفترات الشهرية" في شاشة السنوات المالية `/accounting/fiscal-years`.
  - توليد وتقسيم السنة تلقائياً إلى 12 فترة شهرية (من أول يوم إلى آخر يوم في كل شهر، مع دعم دقيق للسنوات الكبيسة 29 فبراير، والسنوات غير المنتظمة، والتوليد الكسول للسنوات القديمة).
  - إقفال الفترة الشهرية بنقرة واحدة مع إضافة ملاحظات الإقفال.
  - الحظر الصارم للإقفال غير الزمني (لا يمكن إقفال الشهر N إلا إذا كان الشهر السابق N-1 مقفلاً).
  - الحظر الصارم للإقفال في حال وجود قيود يومية معلقة (مسودات `draft`) خلال نطاق الشهر، وإلزام المحاسب بترحيلها أو حذفها أولاً.
  - حظر ترحيل أي قيد يومي (يدوي عبر شاشة القيود أو آلي من المبيعات والمشتريات والخزينة والمخزون) يقع تاريخه ضمن فترة مقفلة بكود `ACCOUNTING_PERIOD_LOCKED` (400).
  - إمكانية إعادة فتح فترة شهرية مقفلة للمدير المالي، بشرط إلزامي لتوثيق سبب إعادة الفتح للتدقيق، والالتزام بالترتيب العكسي (لا يُعاد فتح الشهر N طالما الشهر اللاحق N+1 ما زال مقفلاً).
* **الملفات:**
  - `backend/src/database/migrations/2040000000143_accounting_fiscal_periods.ts` — الهجرة وقيد التفرد والفهارس
  - `backend/src/database/database.types.ts` — تعريف جدول `AccountingFiscalPeriodTable`
  - `backend/src/modules/accounting/dto/fiscal-period.dto.ts` — عقود البيانات DTOs
  - `backend/src/modules/accounting/services/fiscal-year.service.ts` — محرك تقسيم الفترات والإقفال وإعادة الفتح
  - `backend/src/modules/accounting/accounting-posting.service.ts` — حظر الترحيل التلقائي في الفترات المقفلة
  - `backend/src/modules/accounting/accounting.service.ts` — حظر الترحيل اليدوي في الفترات المقفلة
  - `backend/src/modules/accounting/accounting.controller.ts` — مسارات API
  - `frontend/src/features/accounting/types/fiscal-years.types.ts` — أنواع الفرونت إند
  - `frontend/src/features/accounting/api/fiscal-years.api.ts` — استدعاءات API
  - `frontend/src/features/accounting/components/fiscal-years/FiscalPeriodsModal.tsx` — المودال المتكامل لإدارة الفترات
  - `frontend/src/features/accounting/pages/AccountingFiscalYearsPage.tsx` — زر الوصول وعرض الفترات
* **التحقق:**
  - `backend/test/critical/fiscal-periods.spec.ts` — اختبارات المحرك: التقسيم المنتظم، السنوات الكبيسة، الفترات المخصصة، الترتيب الزمني، حظر المسودات، إعادة الفتح العكسية، وحظر القيود.
  - فحص الأنواع (`tsc -b`) وعقود API والمعمارية (`npm run guards`) بنجاح 100%.


---

## 171. أحجام قابلة للضبط لاختبارات الحمل ومعايرتها على مواصفات الإنتاج (Load Profiles & Client-Address Simulation)
* **حالة الوحدة / المعيار:** 🟢 مكتملة (24 سبتمبر 2026) — محروسة في `performance-hot-paths.spec.ts` ضمن `npm run guards`، والحارس مجرَّب بكسر متعمَّد.
* **ما تغيّر:** منظومة k6 كانت بأحجام ثابتة مكتوبة داخل كل سيناريو (50 مستخدماً)، وتعديلها يعني تعديل ملفات — فعملياً لا أحد يعدّلها. الآن الحجم يأتي من البيئة عبر أربعة ملفات جاهزة:
  - `default` (50) للتشغيل السريع بعد أي تغيير · `heavy` (150) لحمل ثقيل مع فرض عتبات الـSLA · `extreme` (300، **بلا عتبات**) لقياس حدّ السيرفر · `soak` (40 لعشر دقائق) لكشف تسرّب الذاكرة.
  - أو ضبط يدوي: `PEAK_VUS` و`RAMP_SECONDS` و`HOLD_SECONDS`.
  - المشغّلات الثلاثة (`run.sh` · `run.bat` · `runner.cjs`) متطابقة، وحارس يمنع تباعدها.
* **لماذا `extreme` بلا عتبات:** k6 يخرج بكود فشل عند خرق أي عتبة. عند 300 مستخدم متزامن على نواتين ستُخرق عتبة الـp95 حتماً، فتبدو النتيجة «فشلاً» بينما هي الرقم المطلوب قياسه. الملف يطفئها ويطبع الأرقام.
* **`SPOOF_CLIENT_IPS` — محاكاة عملاء متعددين:** حدود المحاولات مفتاحها عنوان العميل، واختبار يعمل من داخل السيرفر يبدو عميلاً **واحداً**، فبعد الطلب الثلاثين يرد 429 على كل شيء ويقيس الاختبار الحارس لا قاعدة البيانات. المفتاح يعطي كل VU عنواناً في `X-Real-IP` فيصير «50 مشترياً متزامناً» خمسين عميلاً فعلاً. ليس التفافاً على الحماية — `resolveClientIp` تثق بالترويسة فقط حين يكون القرين داخلياً — لكنه يبقى **خلف مفتاح صريح** حتى لا يُخفي حدّاً مكسوراً بالغلط.
* **عطل حقيقي أُصلح:** `auth-login-burst.js` كان يرسل `X-Forwarded-For` لتحديد عنوان المهاجم، **والتطبيق لا يقرأ هذه الترويسة إطلاقاً** (يقرأ `X-Real-IP` حصراً). صادف أن النتيجة كانت صحيحة لأن كل الطلبات وقعت في دلو واحد، لكن أول محاولة لمحاكاة عدة مهاجمين كانت ستفشل بصمت. الحارس يمنع رجوعها.
...
## 173. الحصانة الذاتية لأرقام الباركود الطويلة ومنع تشويه الصيغ العلمية في ملفات الإكسل (Barcode Scientific Notation Auto-Protection)
* **حالة الوحدة / المعيار:** 🟢 مكتملة ومحمية (24 سبتمبر 2026).
* **المشكلة المعالجة:** عند استيراد ملفات الإكسل التي تحتوي على أرقام باركود أو هويات وطنية أو أرقام هواتف طويلة (مثل الباركود الدولي EAN-13 أو GS1 ذي الـ 13 رقماً)، يقوم برنامج Microsoft Excel تلقائياً بتحويل طريقة عرض الأرقام الطويلة إلى صيغة أسية علمية (`Scientific Notation` مثل `6.22401E+12`). وكان قارئ الإكسل في المتصفح يأخذ النص المعروض بصرياً (`cell.w`) فتنزل كافة الأصناف بنفس الباركود المشوه، مما يؤدي إلى ابتلاع الأصناف وتحديث أول صنف مكرراً آلاف المرات بدلاً من إضافة الأصناف الجديدة.
* **الحصانة المطبقة:**
  1. **الواجهة الأمامية (`browser.ts`):**
     - عند قراءة ملف الإكسل (`parseImportFile`)، يتم فحص خلايا الشيت مسبقاً، واستعادة الرقم الصحيح الكامل بدقة تامة من القيمة الخام `cell.v` عبر `BigInt` وتعيين `cell.w` بالرقم السليم لمنع أي بتر للأرقام أو تحولها لصيغة علمية.
     - دعم تمدد وتطبيع أي سلاسل نصية أسية قد تمر من ملفات CSV أو أجهزة أخرى.
     - عند تصدير قوالب وملفات الإكسل (`downloadExcelFile`)، تعيين تنسيق الخلايا النصية والرقمية الطويلة إلى تنسيق النص الصريح (`@`) لمنع إكسل من تحويلها لصيغة علمية عند فتحها وتعديلها من قبل المستخدم.
  2. **الخلفية (`settings-import.service.ts`):**
     - إضافة دالة `normalizeBarcodeText` لتطبيع الأرقام العربية والإنجليزية وفك أي صيغة أسية علمية تلقائياً وتحويلها للرقم الصحيح في كل من استيراد بطاقات الأصناف (`importProducts`) واستيراد بضاعة أول المدة (`importOpeningStock`).
* **الملفات:**
  - `frontend/src/lib/browser.ts` — قارئ ومصدر ملفات الإكسل وحماية الباركود من الصيغ الأسية.
  - `backend/src/modules/settings/services/settings-import.service.ts` — تطبيع الباركود وفك الصيغ الأسية في مسارات الاستيراد.

---

## 174. حوكمة صرف أجرة المناديب وحماية نقدية الدرج في مبيعات الدليفري (Delivery Freelance Rep Payout Governance)
* **حالة الوحدة / المعيار:** 🟢 مكتملة ومحمية (24 سبتمبر 2026).
* **المشكلة المعالجة:** كان النظام في مبيعات الدليفري للعملاء الآجلين (Credit) أو السداد الجزئي يقوم خطأً باعتبار الفاتورة الآجلة مستوجبة لصرف نقدي من الدرج (`cash_out`) للمندوب الحر ويخصم الأجرة من النقدية المتوقعة في شفت الكاشير (`cashier_shifts.expected_cash`)، مع ظهور رسالة مضللة في نافذة نجاح البيع تفيد بصرف الأجرة نقداً من الدرج، بينما العميل في الحقيقة هو من يحاسب المندوب عند الاستلام أو أن الأجرة مسجلة على حساب العميل ولم يدخل الدرج أي نقدية.
* **الحوكمة المطبقة:**
  1. **الخلفية المحاسبية والتشغيلية (`sales-write.service.ts`):**
     - إلغاء شرط `effectivePaymentType === 'credit'` نهائياً من مسار الصرف النقدي.
     - قصر تسجيل حركة الخروج النقدية (`cash_out`) وتخفيض النقدية المتوقعة للدرج حصرياً على حالة **السداد الإلكتروني المسبق الكامل** (`prepaid_online` أو قنوات السداد الإلكتروني كـ InstaPay والمحافظ الرقمية بالكامل `paidAmount >= total`) وعندما يكون المندوب حراً (`freelance_courier`)، حيث استلم المحل الأجرة إلكترونياً ويأخذ المندوب أجرته نقداً قبل التحرك لعدم تحصيل أي شيء من العميل.
     - منع الصرف التلقائي نهائياً في: المبيعات الآجلة (`credit`)، الدفع عند الاستلام (`cod`)، الخالص من المندوب (`prepaid_by_rep`)، أو أي طلبات غير مسددة بالكامل.
  2. **الواجهة الأمامية لنقطة البيع (`PosSaleSuccessDialog.tsx`):**
     - إزالة بانر الصرف النقدي نهائياً من الفواتير الآجلة، وعرض أجرة التوصيل كبند مسجل على ذمة العميل: `أجرة التوصيل: XX.XX (على حساب العميل)`.
     - قصر ظهور رسالة صرف الأجرة من الدرج على الطلبات المسددة أونلاين بالكامل فقط للمناديب المستقلين.
* **الملفات:**
  - `backend/src/modules/sales/services/sales-write.service.ts` — ضبط شروط الصرف النقدي للمناديب وقصرها على السداد الأونلاين الكامل.
  - `frontend/src/features/pos/components/pos-workspace/PosSaleSuccessDialog.tsx` — تصحيح رسائل ومؤشرات الفواتير الآجلة والمسددة إلكترونياً.
* **التحقق:**
  - `src/features/pos/components/pos-workspace/PosSaleSuccessDialog.spec.tsx` (7 اختبارات ناجحة بنسبة 100%).
  - نجاح الفحص النوعي للفرونت إند والباك إند (`tsc --noEmit`).
