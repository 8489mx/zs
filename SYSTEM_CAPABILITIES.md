...
* **مسارات الكود والوحدات المقسمة:**
  ...
  21. **تأسيس منشأة المنصة المركزية المستقلة `zs` وعزل صلاحيات السوبر أدمن الصارم (Dedicated Platform Root Tenant & Super Admin Isolation):**
      - **منشأة المنصة المركزية المستقلة (`zs`):** تأسيس منشأة المنصة المستقلة بالمعرف الرسمي `zs` في جدول `tenants` بحالة نشطة دائمة، ونقل وتعيين حساب المطور ومسؤول المنصة `zs` بداخلها برتبة `super_admin` عبر المايجريشن رقم 68 (`2040000000068_establish_zs_platform_tenant_and_superadmin_isolation.ts`).
