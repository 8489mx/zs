...
| **رابط مستقل لكل متجر على سب دومين (`almhnds.zsystemai.com`)** | 🟡 | 90% | `store-public-url.engine.ts`, `frontend/src/lib/store-public-url.ts`, `root-router.tsx`, `PublicStorefrontPage.tsx`, `deploy/nginx/zsystemai-subdomains.conf` | كل متجر يفتح على `https://<slug>.<root>` بنفس قاعدة البيانات (الاسم بيتقري من الدومين بدل المسار). رابط المتجر في الإعدادات وصفحة الطلبات ومشاركة المنتج وروابط QR الطاولات ورابط تتبع الطلب ومعاينة واتساب ورد بوت الذكاء الاصطناعي كلها من محرك واحد (SF-10). الأسماء المحجوزة (`app`، `www`، `api`، ...) مرفوضة. الروابط القديمة `/st/` و`/store/` و`/shop/` بتتحول تلقائياً. **الحالة:** الكود جاهز ومجرّب محلياً؛ مطفأ في الإنتاج لحد ضبط `STOREFRONT_ROOT_DOMAIN` و`VITE_STOREFRONT_ROOT_DOMAIN` والـDNS والـnginx (O68). من غيرهم المتاجر على `/st/<slug>` كما كانت. الديسكتوب مش متأثر |
