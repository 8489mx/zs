...
* **مسارات الكود الأساسية:**
  ...
  * **الباك إند ومصادر البيانات الأساسية (Single Source of Truth):**
    ...
    * `backend/src/modules/hr/hr.service.ts` (`upsertEmployee` لتعديل الحالة والهاتف).
...
* **الميزات والقدرات المفعلة:**
  ...
  2. **التبويبات الموحدة في شاشة إدارة المستخدمين (`/settings/users`):**
     ...
     - **موظفو الخدمة الذاتية والبصمة (Staff):** عرض كافة موظفي المنشأة، أرقام الهواتف، رموز الدخول السريع للبصمة الذكية والبوابة، زر تعيين الـ PIN السريع بنقرة واحدة، تبديل حالة الحساب (نشط / موقوف)، ونسخ روابط `/portal` و `/punch` وزر انتقال للملف الوظيفي في HR.
  3. **زر الإدارة السريع في ترويسة مركز البوابات (`/hub`):** إضافة زر "إدارة الحسابات والوصول" في الشريط العلوي لدليل البوابات، لتمكين المسؤول من الوصول الفوري لتعديل بيانات أي سائق أو موظف يواجه مشكلة في تسجيل الدخول.
  4. **زر ونافذة تعيين الـ PIN في بروفايل الموظف في HR (`EmployeeProfilePage`):** تمكين مسؤولي شؤون العاملين من تعيين أو إعادة تعيين رمز الدخول السريع للموظف مباشرة من بطاقة التشغيل بالملف الشخصي.
  5. **تصميم مؤسسي نظيف وخالٍ تماماً من الإيموجيز (0 Emojis):** اعتماد كامل على أيقونات SVG القياسية من `@/shared/components/icons/AppIcons`، ألوان المنظومة المعتمدة (`#170e5e`، رمادي محايد، كروت بيضاء متجاوبة)، وتنسيق RTL احترافي.
