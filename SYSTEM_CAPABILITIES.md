...
| **مكتب طلبات الهاتف والتوصيل السريع وكاشف المتصل (Phone Order Desk & Caller ID Integration)** | 🟢 | 100% | `PosPhoneOrderDialog.tsx`, `PosCallerIdFloatingAlert.tsx`, `pos-caller-id-service.ts`, `partners.service.ts`, `partners.controller.ts` | بيئة متكاملة لطلبات الهاتف والتوصيل السريع: بحث فوري برقم الهاتف، استرجاع فوري لملف العميل ونقاط الولاء والعناوين السابقة، سجل آخر الطلبات مع ميزة تكرار الطلب في السلة بضغطة واحدة (1-Click Reorder)، تسجيل العميل الجديد فورياً، ربط أجهزة كاشف المتصل (Caller ID Box عبر Web Serial API / USB) مع تنبيهات رنين مرئية وصوتية وتشغيل اختصار لوحة المفاتيح Alt+P / F10. |
* **حالة الوحدة العامة:** 🟢 مكتمل 100%
...
* **المسارات في الكود:**
  ...
  - `frontend/src/features/contracting/components/ImportMasterBoqModal.tsx`, `ImportMasterBoqExcelModal.tsx`, و `ContractingMasterBoqPage.tsx`:
    ...
    4. **معالجة النصوص وفلاتر التخصصات المرنة:** القضاء التام على كسر وبتر النصوص الطويلة عبر شرائح مرنة متجاوبة (`flex-wrap: wrap`) تظهر المسميات الهندسية كاملة مع بادجات الأعداد.
    5. **عمود الإجراءات وحوكمة الحذف والاستبعاد:** زر تعديل السعر والمواصفات وزر حذف نهائي للبنود المخصصة مع زر استبعاد / إخفاء وإعادة تفعيل للبنود القياسية العامة وفلترة بحالة البند (النشطة / الكل / المستبعدة).
* **فحص الجودة والسلامة البرمجية:**
  - نجاح تنفيذ Migration `2040000000094_contracting_master_boq_comprehensive_expansion` بنسبة 100%.
  - فحص TypeScript للفرونت إند: **Code 0 (Zero Errors)**.
  - فحص TypeScript للباك إند: **Code 0 (Zero Errors)**.
