...
>
> **وملف حماية الأداء:** [`PERFORMANCE_CONSTITUTION.md`](./PERFORMANCE_CONSTITUTION.md) — الثوابت PERF-1..PERF-11
...
| **بيع السيارات المتنقلة ومخزون المندوب (Van Sales & Mobile Warehouse)** | 🟢 | 100% | `van-sales.service.ts`, `location-stock-ledger.ts` | تحميل سيارة المندوب من المستودع، البيع الميداني، مرتجعات العملاء، وتصفية الرحلة بإعادة غير المُباع.<br/>**تحصين سبتمبر 2026 (المرحلة 3):** كانت الوحدة تكتب `product_location_stock` بعبارات `qty + n` / `qty - n` مباشرة **ولا تمس `products.stock_qty` ولا `stock_movements` إطلاقاً** — فكان الرصيد العام ينفصل عن مجموع أرصدة المواقع في كل عملية تحميل أو بيع أو مرتجع، ولم تكن أي حركة سيارة تظهر في دفتر المخزون المستمر. حُوِّلت كل حركات المخزون (7 مواضع) للمرور عبر `applyStockDelta` بدالة موحدة `moveVanStock` تكتب حركة رسمية بأرصدة قبل/بعد حقيقية، مع التمييز الصحيح: التحميل والتصفية نقل بين مواقع (لا يمس الرصيد العام)، والبيع والمرتجع يغيّران الرصيد العام. كما استُعيد ترتيب الأقفال القانوني (`products` قبل `product_location_stock`). |
