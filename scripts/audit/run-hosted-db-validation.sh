...
if [[ "${DATABASE_SSL:-true}" != "true" ]]; then
  echo "[audit] FAIL: DATABASE_SSL must be true for hosted validation."
  exit 1
fi
...

{
  ...
  if [[ "${DATABASE_SSL_REJECT_UNAUTHORIZED:-true}" != "true" ]]; then
    echo "- Warning: DATABASE_SSL_REJECT_UNAUTHORIZED is not true; accepted to match runtime-specific pooler/TLS behavior."
  fi
  echo ""
