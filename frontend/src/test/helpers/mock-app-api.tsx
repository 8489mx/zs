...
const summary = {
  ...
  returns: { count: 1, total: 10, salesTotal: 10, purchasesTotal: 0 },
...
const sales = [{ id: 'sale-1', createdAt: today, date: today, docNo: 'S-1', invoiceNumber: 'S-1', customerId: 'cust-1', customerName: 'عميل الآجل', total: 100, paid: 50, due: 50, paymentType: 'credit', status: 'posted', items: [{ id: 'sale-item-1', productId: 'prod-1', name: 'مياه معدنية', unitName: 'قطعة', qty: 2, price: 10, total: 20 }] }];
const purchases = [{ id: 'pur-1', createdAt: today, date: today, docNo: 'P-1', invoiceNumber: 'P-1', supplierId: 'sup-1', supplierName: 'مورد رئيسي', branchName: 'الفرع الرئيسي', locationName: 'المخزن الرئيسي', total: 50, paid: 50, due: 0, paymentType: 'cash', status: 'posted', items: [{ id: 'pur-item-1', productId: 'prod-1', name: 'مياه معدنية', unitName: 'قطعة', qty: 5, cost: 5, total: 25 }] }];
const returnsData = [{ id: 'ret-1', createdAt: today, docNo: 'RT-1', type: 'sale', partyName: 'عميل الآجل', productName: 'مياه معدنية', qty: 1, total: 10, note: 'مرتجع تجريبي', items: [] }];
...
function apiPayload(pathname: string) {
  ...
  if (pathname === '/api/returns') return { returns: returnsData, pagination: { page: 1, pageSize: 20, totalItems: 1, totalPages: 1 }, summary: { totalReturns: 1, totalAmount: 10 } };
...
export function installGlobalAppFetchMock() {
...
export async function renderAppAt(path: string) {
  ...
  return render(<AppProviders><AppRouter /></AppProviders>);
}
