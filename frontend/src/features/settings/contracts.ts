...
export function buildSettingsUpdatePayload(currentSettings: AppSettings | undefined, values: SettingsFormOutput) {
  ...
  const settings = {
    ...
    purchasesModuleEnabled: values.purchasesModuleEnabled !== false,
    inventoryModuleEnabled: values.inventoryModuleEnabled !== false,
    hrModuleEnabled: values.hrModuleEnabled !== false,
    businessIndustry: values.businessIndustry || currentSettings?.businessIndustry || 'general',
    ...
    taxDeclarationModuleEnabled: values.taxDeclarationModuleEnabled !== false,
    deliveryFleetModuleEnabled: values.deliveryFleetModuleEnabled !== false,
    loyaltyEnabled: values.loyaltyEnabled !== false,
    loyaltyPointsPer100Egp: Math.max(0, Number(values.loyaltyPointsPer100Egp ?? 10)),
    loyaltyPointRedeemValue: Math.max(0.01, Number(values.loyaltyPointRedeemValue ?? 0.1)),
    loyaltyMinRedeemPoints: Math.max(0, Number(values.loyaltyMinRedeemPoints ?? 50)),
    loyaltyMaxDiscountPercentage: Math.max(1, Math.min(100, Number(values.loyaltyMaxDiscountPercentage ?? 50))),
    printShowLoyaltyPoints: values.printShowLoyaltyPoints !== false,
    ...
    posMaxDiscountThresholdEnabled: values.posMaxDiscountThresholdEnabled === true,
    posMaxDiscountThresholdType: values.posMaxDiscountThresholdType === 'fixed' ? 'fixed' : 'percentage',
    posMaxDiscountThresholdValue: Math.max(0, Number(values.posMaxDiscountThresholdValue ?? 15)),
