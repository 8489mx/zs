...
export function SettingsReadinessSection({
  ...
  return (
    <div className="two-column-grid settings-diagnostics-grid">
      <QueryCard className="settings-admin-card" title="جاهزية الاختبار" actions={<div className="actions compact-actions"><Button variant="secondary" onClick={() => downloadSummaryCsv('uat-readiness.csv', uatSummary)}>تصدير Excel</Button><Button variant="secondary" onClick={() => printSummaryList('جاهزية الاختبار', uatSummary)}>طباعة</Button></div>} isLoading={uatQuery.isLoading} isError={uatQuery.isError} error={uatQuery.error}>
        <SummaryList data={uatSummary} />
      </QueryCard>
      <QueryCard className="settings-admin-card" title="ملخص الدعم" actions={<div className="actions compact-actions"><Button variant="secondary" onClick={() => downloadSummaryCsv('support-snapshot.csv', supportData)}>تصدير Excel</Button><Button variant="secondary" onClick={onCopySupportSnapshot}>نسخ</Button><Button variant="secondary" onClick={() => printSummaryList('ملخص الدعم', supportData)}>طباعة</Button></div>} isLoading={supportQuery.isLoading} isError={supportQuery.isError} error={supportQuery.error}>
