...
export function CostSnapshotModal({
  ...
  return (
    <StandardDialog
      ...
      subtitle={`حفظ لقطة مرجعية ثابتة غير قابلة للتعديل لمقايسة وتكاليف المشروع: ${projectName || ''}`}
      maxWidth="700px"
      footer={
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }} dir="rtl">
        ...
        <div>
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: '24px', color: '#64748b' }}>جارٍ التحميل...</div>
          ) : snapshots.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '24px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px dashed #cbd5e1', color: '#94a3b8' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                    ...
                    <th style={{ padding: '8px 12px', fontSize: 'var(--font-table-head)', color: '#475569' }}>إجمالي القيمة</th>
                    <th style={{ padding: '8px 12px', fontSize: 'var(--font-table-head)', color: '#475569' }}>التكلفة التقديرية</th>
                  ...
                  {snapshots.map((s) => (
                    <tr key={s.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '8px 12px', fontSize: 'var(--font-body)', fontWeight: 600, color: '#170e5e' }}>
                        {Number(s.totalContractValue).toLocaleString()} ر.س
                      </td>
                      <td style={{ padding: '8px 12px', fontSize: 'var(--font-body)', color: '#475569' }}>
                        {Number(s.totalBudgetCost).toLocaleString()} ر.س
                      </td>
