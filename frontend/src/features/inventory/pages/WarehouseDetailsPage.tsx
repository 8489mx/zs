  ...
  return (
    <main className="document-prototype-column">
      <PageHeader 
        ...
        actions={<span className="nav-pill" onClick={() => navigate('/inventory/warehouses')}>العودة للمخازن</span>}
      />
      ...
      {selectedCategoryId === null ? (
        ...
        <FormSection title={`الأصناف - ${selectedCategoryId === 'all' ? 'كل الأقسام' : categoriesQuery.data?.find(c => c.id === selectedCategoryId)?.name}`} actions={<button type="button" className="secondary-button" onClick={() => setSelectedCategoryId(null)}>العودة للأقسام</button>}>
