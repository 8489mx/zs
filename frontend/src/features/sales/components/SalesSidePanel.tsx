...
export function SalesSidePanel({
  ...
  return (
    <div className="sales-side-stack">
      <Card title="أعلى العملاء في النتائج الحالية" description="ملخص سريع للعملاء الأكثر شراءً داخل الفلاتر الحالية." actions={<div className="actions compact-actions"><Button variant="secondary" onClick={() => void onExportTopCustomers()} disabled={!topCustomers.length}>تصدير CSV</Button><Button variant="secondary" onClick={() => void onPrintTopCustomers()} disabled={!topCustomers.length || !canPrint}>طباعة</Button><span className="nav-pill">ملخص سريع</span></div>} className="workspace-panel sales-insight-card">
        <div className="list-stack">
          ...
          )) : <div className="muted">لا توجد نتائج كافية لحساب العملاء الأعلى.</div>}
        </div>
      </Card>
