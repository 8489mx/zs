    ...
    <DialogShell open={open} onClose={handleCancel} width="min(980px, 100%)" zIndex={70}>
      <FormSection title={`تعديل ${sale.docNo || sale.id}`} actions={<span className="nav-pill">{t('sales.759fdc')}</span>} className="dialog-card dialog-card-wide">
        ...
        <div className="form-grid">
          ...
          <Field label={t('sales.2249f4')}><input type="text" className="secure-password-field" inputMode="numeric" value={managerPin} onChange={(e) => setManagerPin(e.target.value)} disabled={isBusy} placeholder={t('sales.f7fa23')} autoComplete="off" data-lpignore="true" data-1p-ignore="true" data-form-type="other" autoCorrect="off" autoCapitalize="off" spellCheck={false} /></Field>
        </div>
