      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
        <table className="table" style={{ width: '100%', margin: 0, fontSize: '13px' }}>
          <thead style={{ background: '#f8fafc' }}>
            <tr>
              <th style={{ padding: '8px 4px', whiteSpace: 'nowrap' }}>التاريخ والوقت</th>
              <th style={{ padding: '8px 4px', whiteSpace: 'nowrap' }}>المبلغ المورد</th>
              <th style={{ padding: '8px 4px', whiteSpace: 'nowrap' }}>مستلم المبلغ</th>
              <th style={{ padding: '8px 4px' }}>البيان</th>
            </tr>
          </thead>
          <tbody>
            {settlementsQuery.isLoading && <tr><td colSpan={4} style={{ padding: '8px', textAlign: 'center', color: '#64748b' }}>جاري التحميل...</td></tr>}
            {settlementsQuery.data?.length === 0 && <tr><td colSpan={4} style={{ padding: '8px', textAlign: 'center', color: '#64748b' }}>لا يوجد توريدات سابقة</td></tr>}
            {settlementsQuery.data?.map((settlement: any) => (
              <tr key={settlement.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                <td style={{ padding: '8px 4px', direction: 'ltr', textAlign: 'right', whiteSpace: 'nowrap' }}>{new Date(settlement.createdAt).toLocaleString('ar-EG')}</td>
                ...
                <td style={{ padding: '8px 4px', color: '#64748b', fontSize: '12px' }}>{settlement.note}</td>
              </tr>
            ))}
          </tbody>
        </table>
