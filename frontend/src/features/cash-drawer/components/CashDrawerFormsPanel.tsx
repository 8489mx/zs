...
export function CashDrawerFormsPanel(props: CashDrawerFormsPanelProps) {
  return (
    <div className="three-column-grid panel-grid">
      <Card title="فتح وردية جديدة" description={SINGLE_STORE_MODE ? 'سجل رصيد الفتح ثم ابدأ الوردية على المخزن الأساسي مباشرة.' : 'سجل رصيد الفتح واربط الوردية بالفرع والموقع عند الحاجة.'} actions={<span className="nav-pill">فتح الوردية</span>}>
        <form className="form-grid" onSubmit={props.openForm.handleSubmit((values) => props.openMutation.mutate(values))}>
          ...
          <Field label="ملاحظة الافتتاح"><textarea rows={3} {...props.openForm.register('note')} disabled={props.openMutation.isPending} /></Field>
      ...
      <Card title="تسجيل حركة درج" description="إضافة حركة إيداع أو صرف على وردية مفتوحة فقط مع سبب واضح لكل حركة." actions={<span className="nav-pill">حركة نقدية</span>}>
        <form className="form-grid" onSubmit={props.onMovementSubmit}>
          ...
          <Field label="سبب الحركة"><textarea rows={3} placeholder="اكتب السبب بوضوح" {...props.movementForm.register('note')} disabled={props.movementMutation.isPending} /></Field>
      ...
      <Card title="إغلاق وردية" description="أغلق الوردية بالمبلغ المعدود فعليًا لتسجيل الفرق النهائي." actions={<span className="nav-pill">إغلاق</span>}>
        <form className="form-grid" onSubmit={props.onCloseSubmit}>
          ...
          <Field label="ملاحظة الإغلاق"><textarea rows={3} placeholder={Math.abs(props.closeVariancePreview) >= 0.01 ? 'اشرح سبب الفرق قبل الإغلاق' : 'اختياري عند عدم وجود فرق'} {...props.closeForm.register('note')} disabled={props.closeMutation.isPending} /></Field>
