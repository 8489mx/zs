...
export function CashDrawerReviewDialog(props: CashDrawerReviewDialogProps) {
  ...
  const comparison = useMemo(() => {
    const rows: ComparisonRow[] = [
      { key: 'cash', label: 'النقدي', systemAmount: expected.expectedCash, declaredAmount: declared.cash, amountDiff: toMoney(declared.cash - expected.expectedCash), systemCount: null, declaredCount: null, countDiff: null },
      { key: 'card', label: 'فيزا', systemAmount: expected.expectedCard, declaredAmount: declared.card, amountDiff: toMoney(declared.card - expected.expectedCard), systemCount: toCount(shift?.cardOperationCount || 0), declaredCount: toCount(shift?.declaredCardCount || 0), countDiff: toCount(shift?.declaredCardCount || 0) - toCount(shift?.cardOperationCount || 0) },
    ...
    if (expected.expectedCredit > 0) rows.push({ key: 'credit', label: 'آجل', systemAmount: expected.expectedCredit, declaredAmount: 0, amountDiff: toMoney(-expected.expectedCredit), systemCount: null, declaredCount: null, countDiff: null });
  ...
  return (
    <DialogShell open={props.open} onClose={props.onClose} width="min(1080px, 96vw)" ariaLabel="مراجعة إغلاق الوردية">
      <div className="page-stack cash-drawer-review-dialog">
        <Card title="مراجعة إغلاق الوردية" description={shift ? `الوردية: ${shift.docNo || shift.id}` : ''} className="dialog-card">
          {shift ? (
            <div className="page-stack">
              ...
              <div className="cash-drawer-review-block">
                ...
                <div className="table-wrap">
                  <table className="cash-drawer-review-table">
                    <thead><tr><th>البند</th><th>مبلغ النظام</th><th>مبلغ إقرار الكاشير</th><th>فرق المبلغ</th><th>عدد عمليات النظام</th><th>عدد عمليات الكاشير</th><th>فرق العدد</th></tr></thead>
                    <tbody>
                      {comparison.rows.map((row) => (
                        <tr key={row.key}>
                          <td><strong>{row.label}</strong></td><td>{formatMoney(row.systemAmount)}</td><td>{formatMoney(row.declaredAmount)}</td>
                          <td className={`cash-drawer-review-diff-${differenceTone(row.amountDiff)}`}>{differenceLabel(row.amountDiff)}</td>
                          <td>{toDisplayCount(row.systemCount)}</td><td>{toDisplayCount(row.declaredCount)}</td><td>{row.countDiff == null ? '—' : formatSignedCount(row.countDiff)}</td>
                        </tr>
                      ))}
                      <tr className="cash-drawer-review-total-row">
                        <td><strong>الإجمالي</strong></td><td><strong>{formatMoney(comparison.systemAmountTotal)}</strong></td><td><strong>{formatMoney(comparison.declaredAmountTotal)}</strong></td>
                        ...
                        <td><strong>{formatCount(comparison.systemOpsTotal)}</strong></td><td><strong>{formatCount(comparison.declaredOpsTotal)}</strong></td><td><strong>{formatSignedCount(comparison.opsDiffTotal)}</strong></td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              {!hasValidBlindMetadata ? <div className="warning-box">لا توجد بيانات إقرار كاشير منظمة لهذه الوردية.</div> : null}
              {shift.closeNote ? <div className="cash-drawer-review-block"><h4>ملاحظة الكاشير</h4><p className="muted" style={{ margin: 0 }}>{shift.closeNote}</p></div> : null}
              {!hasValidBlindMetadata && shift.closeNoteRaw ? <div className="cash-drawer-review-block"><h4>ملاحظة الكاشير</h4><p className="muted" style={{ margin: 0 }}>{shift.closeNoteRaw}</p></div> : null}

              <Field label="ملاحظة المدير (اختياري)">
                <textarea rows={2} value={props.managerNote} onChange={(event) => props.onManagerNoteChange(event.target.value)} placeholder="اكتب ملاحظة قصيرة للمراجعة" disabled={props.isPending} />
              </Field>

              {shift.managerReviewNote ? <div className="muted small">آخر ملاحظة مدير: <strong>{shift.managerReviewNote}</strong>{shift.managerReviewedByName ? ` — بواسطة ${shift.managerReviewedByName}` : ''}{shift.managerReviewedAt ? ` — ${formatDate(shift.managerReviewedAt)}` : ''}</div> : null}
              <MutationFeedback isError={props.isError} isSuccess={false} error={props.error} errorFallback="تعذر اعتماد إغلاق الوردية" />
              {!reviewMatched ? <p className="muted small" style={{ margin: 0 }}>يوجد فرق مسجل، تأكد من المراجعة قبل الاعتماد.</p> : null}

              <div className="actions compact-actions">
                <Button variant="secondary" onClick={props.onClose} disabled={props.isPending}>إغلاق</Button>
                <Button variant="primary" onClick={props.onApprove} disabled={props.isPending}>{props.isPending ? 'جاري اعتماد الإغلاق...' : 'اعتماد الإغلاق'}</Button>
              </div>
            </div>
          ) : null}
        </Card>
