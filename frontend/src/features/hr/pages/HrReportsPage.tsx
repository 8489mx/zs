  ...
  return (
    <div className="page-stack page-shell" dir="rtl">
      ...
      <QueryFeedback isLoading={loading} isError={isError} error={error} isEmpty={!hasAnyData} loadingText="جارٍ تحميل التقارير..." errorTitle="تعذر تحميل تقارير الموارد البشرية" emptyTitle="لا توجد بيانات كافية لعرض التقرير.">
        ...
        {showSection('loans') ? <FormSection className={reportSectionCardClassName} title="تقرير السلف" actions={<Button variant="secondary" onClick={() => navigate('/hr/loans')}>فتح السلف</Button>}><div className="stats-grid"><div className="stat-card"><span>إجمالي السلف</span><strong>{loansReport.total}</strong></div><div className="stat-card"><span>سلف مفتوحة</span><strong>{loansReport.open}</strong></div><div className="stat-card"><span>أقساط مستحقة</span><strong>{loansReport.dueCount}</strong></div><div className="stat-card"><span>مستحق هذا الشهر</span><strong>{money(loansReport.dueAmount)}</strong></div><div className="stat-card"><span>إجمالي المتبقي</span><strong>{money(loansReport.remainingAmount)}</strong></div></div></FormSection> : null}
        ...
        <FormSection title="دليل الاستخدام السريع">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', fontSize: 13 }} className="muted">
            <div><strong>1. راجع التنبيهات:</strong> العناصر التي تحتاج إجراء تظهر أولًا.</div>
            <div><strong>2. افهم السبب:</strong> بيانات ناقصة، إجازة، سلفة، أو عهدة.</div>
            <div><strong>3. افتح الصفحة المناسبة:</strong> التنبيه يوجهك للمكان الصحيح.</div>
            <div><strong>4. اعتمد المرتبات:</strong> التقرير يساعدك قبل الاعتماد النهائي.</div>
          </div>
        </FormSection>
