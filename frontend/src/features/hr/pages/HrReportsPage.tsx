...
export function HrReportsPage() {
  ...
  return (
    <div className="page-stack page-shell" dir="rtl">
      ...
      <FormSection title="الفترة والفلاتر" description="تغيير الفترة أو القسم يحدّث المؤشرات والنتائج الظاهرة.">
        <div className="form-grid" style={{ gridTemplateColumns: 'repeat(6, 1fr)' }}>
          <label className="field"><span>بحث</span><input style={{ width: '100%' }} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث بالاسم أو الكود" /></label>
          <label className="field"><span>من</span><input style={{ width: '100%' }} type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label>
          <label className="field"><span>إلى</span><input style={{ width: '100%' }} type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label>
          <label className="field"><span>شهر المرتبات</span><input style={{ width: '100%' }} type="month" value={month} onChange={(event) => setMonth(event.target.value)} /></label>
          <label className="field"><span>القسم</span><select style={{ width: '100%' }} value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)}><option value="all">كل الأقسام</option>{departmentOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
          <label className="field" style={{ gridColumn: 'auto' }}><span>نوع التقرير</span><select style={{ width: '100%' }} value={reportType} onChange={(event) => setReportType(event.target.value as ReportType)}>{reportTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        </div>
      </FormSection>
      ...
      <QueryFeedback isLoading={loading} isError={isError} error={error} isEmpty={!hasAnyData} loadingText="جارٍ تحميل التقارير..." errorTitle="تعذر تحميل تقارير الموارد البشرية" emptyTitle="لا توجد بيانات كافية لعرض التقرير.">
        <FormSection className={reportSectionCardClassName} title="لوحة المؤشرات المختصرة" description="أهم أرقام HR في الفترة الحالية."><div className="stats-grid"><button className="stat-card" type="button" onClick={() => setReportType('employees')} style={{ textAlign: 'right' }}><span>إجمالي الموظفين</span><strong>{employeesReport.total}</strong></button><button className="stat-card" type="button" onClick={() => setReportType('employees')} style={{ textAlign: 'right' }}><span>ملفات ناقصة</span><strong>{employeesReport.missingBasics}</strong></button><button className="stat-card" type="button" onClick={() => setReportType('attendance')} style={{ textAlign: 'right' }}><span>سجلات الحضور</span><strong>{attendanceReport.total}</strong></button><button className="stat-card" type="button" onClick={() => setReportType('leaves')} style={{ textAlign: 'right' }}><span>طلبات الإجازة</span><strong>{leavesReport.total}</strong></button><button className="stat-card" type="button" onClick={() => setReportType('loans')} style={{ textAlign: 'right' }}><span>سلف مفتوحة</span><strong>{loansReport.open}</strong></button><button className="stat-card" type="button" onClick={() => setReportType('payroll')} style={{ textAlign: 'right' }}><span>صافي المرتبات</span><strong>{payrollReport.totalNet}</strong></button><button className="stat-card" type="button" onClick={() => setReportType('assets')} style={{ textAlign: 'right' }}><span>عُهد تحتاج مراجعة</span><strong>{assetsReport.needsReview}</strong></button><button className="stat-card" type="button" onClick={() => setReportType('alerts')} style={{ textAlign: 'right' }}><span>تنبيهات</span><strong>{alerts.length}</strong></button></div></FormSection>
        {showSection('employees') ? <FormSection className={reportSectionCardClassName} title="تقرير الموظفين" actions={<Button variant="secondary" onClick={() => navigate('/hr/employees')}>فتح الموظفين</Button>}><div className="stats-grid" style={{ gridTemplateColumns: 'repeat(6, 1fr)', marginBottom: 12 }}><div className="stat-card"><span>إجمالي النتائج</span><strong>{employeesReport.total}</strong></div><div className="stat-card"><span>نشط</span><strong>{employeesReport.active}</strong></div><div className="stat-card"><span>غير نشط</span><strong>{employeesReport.inactive}</strong></div><div className="stat-card"><span>بدون رقم قومي</span><strong>{employeesReport.missingNationalId}</strong></div><div className="stat-card"><span>بدون قسم/مسمى</span><strong>{employeesReport.missingDepartmentOrTitle}</strong></div><div className="stat-card"><span>دوام ناقص</span><strong>{employeesReport.missingWorkSchedule}</strong></div></div><DataTable density="compact" rowKey={(row) => String(row.id)} rows={filteredEmployees.slice(0, 40)} onRowClick={(row) => navigate(`/hr/employees/${row.id}`)} columns={[{ key: 'employeeNo', header: 'كود الموظف', cell: (row) => text(row.employeeNo) }, { key: 'name', header: 'اسم الموظف', cell: employeeName }, { key: 'department', header: 'القسم', cell: (row) => text(row.departmentName) }, { key: 'jobTitle', header: 'المسمى الوظيفي', cell: (row) => text(row.jobTitleName) }, { key: 'nationalId', header: 'الرقم القومي', cell: (row) => normalize(row.nationalId) ? 'موجود' : 'غير مسجل' }, { key: 'status', header: 'الحالة', cell: (row) => isActiveEmployee(row) ? 'نشط' : 'غير نشط' }]} /></FormSection> : null}
        {showSection('attendance') ? <FormSection className={reportSectionCardClassName} title="تقرير الحضور" actions={<Button variant="secondary" onClick={() => navigate('/hr/attendance')}>فتح الحضور</Button>}><div className="stats-grid" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}><div className="stat-card"><span>إجمالي سجلات الفترة</span><strong>{attendanceReport.total}</strong></div><div className="stat-card"><span>حاضر</span><strong>{attendanceReport.present}</strong></div><div className="stat-card"><span>غائب</span><strong>{attendanceReport.absent}</strong></div><div className="stat-card"><span>متأخر</span><strong>{attendanceReport.late}</strong></div><div className="stat-card"><span>غير مسجل/يحتاج مراجعة</span><strong>{attendanceReport.needsReview}</strong></div></div></FormSection> : null}
        {showSection('leaves') ? <FormSection className={reportSectionCardClassName} title="تقرير الإجازات" actions={<Button variant="secondary" onClick={() => navigate('/hr/leaves')}>فتح الإجازات</Button>}><div className="stats-grid" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}><div className="stat-card"><span>إجمالي الطلبات</span><strong>{leavesReport.total}</strong></div><div className="stat-card"><span>قيد المراجعة</span><strong>{leavesReport.pending}</strong></div><div className="stat-card"><span>معتمدة</span><strong>{leavesReport.approved}</strong></div><div className="stat-card"><span>مرفوضة</span><strong>{leavesReport.rejected}</strong></div><div className="stat-card"><span>غير مدفوعة/أيام</span><strong>{leavesReport.unpaid}</strong></div></div></FormSection> : null}
        {showSection('loans') ? <FormSection className={reportSectionCardClassName} title="تقرير السلف" actions={<Button variant="secondary" onClick={() => navigate('/hr/loans')}>فتح السلف</Button>}><div className="stats-grid" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}><div className="stat-card"><span>إجمالي السلف</span><strong>{loansReport.total}</strong></div><div className="stat-card"><span>سلف مفتوحة</span><strong>{loansReport.open}</strong></div><div className="stat-card"><span>أقساط مستحقة</span><strong>{loansReport.dueCount}</strong></div><div className="stat-card"><span>مستحق هذا الشهر</span><strong>{money(loansReport.dueAmount)}</strong></div><div className="stat-card"><span>إجمالي المتبقي</span><strong>{money(loansReport.remainingAmount)}</strong></div></div></FormSection> : null}
        {showSection('payroll') ? <FormSection className={reportSectionCardClassName} title="تقرير المرتبات" actions={<Button variant="secondary" onClick={() => navigate('/hr/payroll')}>فتح المرتبات</Button>}><div className="stats-grid"><div className="stat-card"><span>عدد المسيرات</span><strong>{payrollReport.runs}</strong></div><div className="stat-card"><span>حالة مسير الشهر</span><strong>{payrollReport.selectedRunStatus}</strong></div><div className="stat-card"><span>موظفون في المسير</span><strong>{payrollReport.employeesInRun}</strong></div><div className="stat-card"><span>إجمالي الأساسي</span><strong>{payrollReport.totalBase}</strong></div><div className="stat-card"><span>إجمالي الخصومات</span><strong>{payrollReport.totalDeduction}</strong></div><div className="stat-card"><span>إجمالي السلف/الأقساط</span><strong>{payrollReport.totalLoan}</strong></div><div className="stat-card"><span>صافي المرتبات</span><strong>{payrollReport.totalNet}</strong></div><div className="stat-card"><span>يحتاج مراجعة</span><strong>{payrollReport.needsReview}</strong></div></div></FormSection> : null}
        {showSection('assets') ? <FormSection className={reportSectionCardClassName} title="تقرير العُهد" actions={<Button variant="secondary" onClick={() => navigate('/hr/assets')}>فتح العُهد</Button>}><div className="stats-grid"><div className="stat-card"><span>إجمالي العُهد</span><strong>{assetsReport.total}</strong></div><div className="stat-card"><span>مسلّمة</span><strong>{assetsReport.assigned}</strong></div><div className="stat-card"><span>مرتجعة</span><strong>{assetsReport.returned}</strong></div><div className="stat-card"><span>تالفة</span><strong>{assetsReport.damaged}</strong></div><div className="stat-card"><span>مفقودة</span><strong>{assetsReport.lost}</strong></div><div className="stat-card"><span>مفتوحة</span><strong>{assetsReport.open}</strong></div><div className="stat-card"><span>تحتاج مراجعة</span><strong>{assetsReport.needsReview}</strong></div></div></FormSection> : null}
