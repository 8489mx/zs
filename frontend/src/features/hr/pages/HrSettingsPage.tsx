...
function shouldShowSection(activeSection: SettingsSection, section: SettingsSection) {
...
export function HrSettingsPage() {
  ...
  const setupChecklist = [
    {
      title: 'الأقسام',
      status: departments.length ? `${departments.length} قسم جاهز للاستخدام.` : 'لم يتم إنشاء أقسام بعد. ابدأ هنا قبل إضافة الموظفين.',
      ok: departments.length > 0,
      section: 'organization' as SettingsSection,
    },
    {
      title: 'المسميات الوظيفية',
      status: jobTitles.length ? `${jobTitles.length} مسمى وظيفي جاهز.` : 'لم يتم إنشاء مسميات وظيفية بعد.',
      ok: jobTitles.length > 0,
      section: 'organization' as SettingsSection,
    },
    {
      title: 'أنواع الإجازات',
      status: leaveTypes.length ? `${leaveTypes.length} نوع إجازة جاهز.` : 'أنواع الإجازات مطلوبة قبل تشغيل طلبات الإجازة بشكل صحيح.',
      ok: leaveTypes.length > 0,
      section: 'leaves' as SettingsSection,
    },
    {
      title: 'الموظفون',
      status: departments.length && jobTitles.length ? 'يمكنك الآن إضافة موظف ببيانات منظمة.' : 'استكمل الأقسام والمسميات أولًا لتقليل الملفات الناقصة.',
      ok: departments.length > 0 && jobTitles.length > 0,
      section: 'organization' as SettingsSection,
  ...
  return (
    <div className="page-stack page-shell" dir="rtl">
      ...
      <Card title="ترتيب الإعداد الصحيح" description="هذه هي نقطة البداية قبل إضافة الموظفين وتشغيل الحضور والمرتبات.">
        <div className="form-grid">
          {setupChecklist.map((item) => (
            <div key={item.title} className="field" style={{ alignItems: 'flex-start' }}>
      ...
      >
        {shouldShowSection(activeSection, 'organization') ? (
        ...
        {shouldShowSection(activeSection, 'leaves') ? (
          <Card title="الإجازات" description="إدارة أنواع الإجازات المعتمدة. هذا يؤثر على صفحة الإجازات وعلى مراجعة المرتبات إذا كانت الإجازة غير مدفوعة.">
            ...
            <div className="actions compact-actions" style={{ marginTop: 12 }}>
              ...
              <Button type="button" variant="secondary" onClick={() => navigate('/hr/leaves')}>فتح صفحة الإجازات</Button>
            </div>
        ...
        {shouldShowSection(activeSection, 'documents') ? (
        ...
        {shouldShowSection(activeSection, 'attendance') ? (
          <Card title="الحضور والانصراف" description="إعدادات القواعد التشغيلية للحضور.">
            <p className="muted" style={{ margin: 0 }}>
              إعدادات الحضور المتقدمة غير متاحة حاليًا من البيانات الحالية. يمكن ضبط مواعيد الدوام لكل موظف من صفحة إضافة/تعديل الموظف.
            </p>
            <div className="actions compact-actions" style={{ marginTop: 12 }}>
        ...
        {shouldShowSection(activeSection, 'payroll') ? (
