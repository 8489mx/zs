  ...
  return (
    <div className="page-stack page-shell customers-page" dir="rtl">
      <main className="document-prototype-column" style={{ paddingBottom: '100px' }}>
        <PageHeader title="العملاء" description="ابدأ بالسجل والبحث ثم راجع العميل المحدد أو أضف عميلًا جديدًا من نفس الشاشة." badge={<span className="nav-pill">{controller.summary?.totalCustomers || 0} عميل</span>} actions={<div className="actions compact-actions"><Button variant="secondary" onClick={controller.resetCustomersView}>إعادة الضبط</Button><Button variant="secondary" onClick={controller.exportCustomersCsv} disabled={!controller.summary?.totalCustomers}>تصدير Excel</Button><Button variant="secondary" onClick={() => void controller.copyCustomersSummary()} disabled={!controller.summary?.totalCustomers}>نسخ الملخص</Button><Button variant="secondary" onClick={controller.printCustomersRegister} disabled={!controller.rows.length || !controller.canPrint}>طباعة السجل</Button></div>} />
      <StatsGrid items={stats} />


      <CustomersRegisterCard {...controller} />

      <div ref={editCustomerSectionRef}>
        <FormSection title={controller.selectedCustomer ? `تعديل: ${controller.selectedCustomer.name}` : 'تعديل عميل'} description="اختر عميلًا من السجل لتعديل بياناته أو حذفه." actions={<span className="nav-pill">تعديل وحذف</span>}>
          <CustomerEditorCard customer={controller.selectedCustomer || undefined} onSaved={() => controller.setSelectedCustomer(null)} />
          {controller.selectedCustomer ? <div className="actions section-actions"><Button variant="danger" onClick={() => controller.setCustomerToDelete(controller.selectedCustomer)} disabled={!controller.canDelete}>حذف العميل</Button></div> : null}
        </FormSection>
      </div>

      <FormSection title="إضافة عميل" description="إدخال عميل جديد بنفس قواعد التحقق الموحدة." actions={<span className="nav-pill">إضافة</span>}>
        <CustomerForm />
      </FormSection>
      ...
      <ActionConfirmDialog open={controller.bulkDeleteOpen} title="تأكيد حذف العملاء المحددين" description={controller.selectedCustomers.length ? `سيتم محاولة حذف ${controller.selectedCustomers.length} عميلًا دفعة واحدة. أي عميل مرتبط بحركات قائمة سيرفضه الخادم وسيظهر السبب بعد المحاولة.` : 'لا يوجد عملاء محددون.'} confirmLabel="نعم، حذف المحدد" isBusy={controller.bulkDeleteMutation.isPending} onCancel={() => controller.setBulkDeleteOpen(false)} onConfirm={async () => { if (!controller.selectedIds.length) return; await controller.bulkDeleteMutation.mutateAsync(controller.selectedIds); }} />
      </main>
    </div>
  );
}
