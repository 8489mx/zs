...
export function SupplierBalanceScheduleCard({ supplier, disabled = false }: SupplierBalanceScheduleCardProps) {
  ...
  const settleMutation = useMutation({
    mutationFn: ({ row, amount }: { row: SupplierPaymentScheduleItem; amount?: number }) => supplierBalanceScheduleApi.settle(row.id, {
      amount,
    ...
    >
      ...
      {canSchedule ? (
        <div className="form-grid" style={{ marginTop: 12 }}>
          ...
          <div className="field"><span>إنشاء الجدول</span><Button type="button" onClick={() => createMutation.mutate()} disabled={createMutation.isPending}>{rows.length ? 'إعادة جدولة المستحقات' : 'جدولة المستحقات'}</Button></div>
        </div>
      ) : (
        <div className="surface-note" style={{ marginTop: 12 }}>لا توجد مستحقات موجبة على هذا المورد يمكن جدولتها حاليًا.</div>
      )}
              ...
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.id}>
                    ...
                    <td><div className="actions compact-actions"><input type="number" min="0.01" step="0.01" value={amountValue} onChange={(event) => setSettleAmounts((current) => ({ ...current, [row.id]: event.target.value }))} placeholder={String(row.remainingAmount || '')} disabled={isSettled || disabled} style={{ maxWidth: 110 }} /><Button type="button" variant="secondary" disabled={isSettled || disabled || settleMutation.isPending} onClick={() => settleMutation.mutate({ row, amount: amountValue ? Number(amountValue) : undefined })}>دفع</Button></div></td>
                  </tr>
                );
              })}
            </tbody>
