...
const categories = [{ id: 'cat-1', name: 'مشروبات' }];
const products = [{
...
const dashboardOverview = {
  ...
  lowStock: [],
  topToday: [{ productId: 'prod-1', name: 'مياه معدنية', qty: 10, total: 100 }],
...
const sales = [{ id: 'sale-1', createdAt: today, invoiceNumber: 'S-1', customerId: 'cust-1', customerName: 'عميل الآجل', total: 100, paid: 50, due: 50, paymentType: 'credit', items: [] }];
const purchases = [{ id: 'pur-1', createdAt: today, invoiceNumber: 'P-1', supplierId: 'sup-1', supplierName: 'مورد رئيسي', total: 50, paid: 50, due: 0, items: [] }];
const returnsData = [{ id: 'ret-1', createdAt: today, type: 'sale', partyName: 'عميل الآجل', total: 0, items: [] }];
const services = [{ id: 'svc-1', name: 'صيانة', price: 25, cost: 5, category: 'خدمات', isActive: true, notes: '' }];
const ledgerCustomer = {
  customer: customers[0],
  entries: [{ id: 'cle-1', date: today, type: 'sale', amount: 100, balance: 150, note: 'فاتورة بيع' }],
  summary: { debit: 100, credit: 50, balance: 150 },
  pagination: { page: 1, pageSize: 10, totalItems: 1, totalPages: 1 },
};
const ledgerSupplier = {
  supplier: suppliers[0],
  entries: [{ id: 'sle-1', date: today, type: 'purchase', amount: 50, balance: 320, note: 'فاتورة شراء' }],
  summary: { debit: 50, credit: 0, balance: 320 },
  pagination: { page: 1, pageSize: 10, totalItems: 1, totalPages: 1 },
};
...
function byPath(pathname: string) {
  ...
  if (pathname === '/api/returns') return { returns: returnsData, pagination: { page: 1, pageSize: 20, totalItems: 1, totalPages: 1 }, summary: { totalReturns: 1, totalAmount: 0 } };
  if (pathname === '/api/reports/customer-balances') return { customers };
  if (pathname.match(/^\/api\/reports\/customers\/[^/]+\/ledger$/)) return ledgerCustomer;
  if (pathname.match(/^\/api\/reports\/suppliers\/[^/]+\/ledger$/)) return ledgerSupplier;
  if (pathname === '/api/reports/inventory') return { inventory: { rows: [], summary: { totalItems: 1, lowStockItems: 0, outOfStockItems: 0, inventoryCost: 100, inventorySaleValue: 200 } }, pagination: { page: 1, pageSize: 20, totalItems: 0, totalPages: 1 } };
  ...
  if (pathname === '/api/services') return { services, pagination: { page: 1, pageSize: 20, totalItems: 1, totalPages: 1 }, summary: { totalServices: 1, activeServices: 1 } };
...
export async function installMockApi(page: Page) {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    return json(route, byPath(url.pathname));
  });

  return {
    async expectNoClientErrors() {
