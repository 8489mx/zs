...
exports.migration = {
    async up(db) {
        await db.schema
            ...
            .addCheckConstraint('tenants_status_valid', (0, kysely_1.sql) `status in ('trial', 'active', 'expired', 'suspended')`)
            .execute();
        await db.schema
        ...
        await (0, kysely_1.sql) `ALTER TABLE users ADD COLUMN IF NOT EXISTS tenant_id TEXT NOT NULL DEFAULT ''`.execute(db);
        await (0, kysely_1.sql) `ALTER TABLE users ADD COLUMN IF NOT EXISTS account_id TEXT NOT NULL DEFAULT ''`.execute(db);
    ...
    async down(db) {
        ...
        await (0, kysely_1.sql) `ALTER TABLE users DROP COLUMN IF EXISTS account_id`.execute(db);
        await (0, kysely_1.sql) `ALTER TABLE users DROP COLUMN IF EXISTS tenant_id`.execute(db);
...
//# sourceMappingURL=20260527000100_saas_trial_foundation.js.map