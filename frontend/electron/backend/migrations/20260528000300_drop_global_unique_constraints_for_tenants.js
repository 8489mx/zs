    ...
    async up(db) {
        if (await tableExists(db, 'settings')) {
            if (!(await columnExists(db, 'settings', 'tenant_id')))
                await (0, kysely_1.sql) `ALTER TABLE settings ADD COLUMN IF NOT EXISTS tenant_id TEXT NOT NULL DEFAULT 'default'`.execute(db).catch(() => undefined);
            if (!(await columnExists(db, 'settings', 'account_id')))
                await (0, kysely_1.sql) `ALTER TABLE settings ADD COLUMN IF NOT EXISTS account_id TEXT NOT NULL DEFAULT 'default'`.execute(db).catch(() => undefined);
        ...
        if (await tableExists(db, 'product_units')) {
            if (!(await columnExists(db, 'product_units', 'tenant_id')))
                await (0, kysely_1.sql) `ALTER TABLE product_units ADD COLUMN IF NOT EXISTS tenant_id TEXT NOT NULL DEFAULT 'default'`.execute(db).catch(() => undefined);
            if (!(await columnExists(db, 'product_units', 'account_id')))
                await (0, kysely_1.sql) `ALTER TABLE product_units ADD COLUMN IF NOT EXISTS account_id TEXT NOT NULL DEFAULT 'default'`.execute(db).catch(() => undefined);
