...
exports.migration = {
    ...
    down: async (db) => {
        await (0, kysely_1.sql) `ALTER TABLE products ADD CONSTRAINT chk_products_stock_qty_non_negative CHECK (stock_qty >= 0)`.execute(db);
        await (0, kysely_1.sql) `ALTER TABLE product_location_stock ADD CONSTRAINT chk_product_location_stock_qty_non_negative CHECK (qty >= 0)`.execute(db);
    },
};
//# sourceMappingURL=1710000014000-allow-negative-stock-ledger.js.map