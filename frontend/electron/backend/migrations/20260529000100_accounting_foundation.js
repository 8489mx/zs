...
exports.migration = {
    ...
    `.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_accounting_accounts_code ON accounting_accounts(code)`.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_accounting_accounts_parent_id ON accounting_accounts(parent_id)`.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_journal_entries_entry_date ON journal_entries(entry_date)`.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_journal_entries_status ON journal_entries(status)`.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_journal_entries_source ON journal_entries(source_type, source_id)`.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_journal_entry_lines_journal_entry_id ON journal_entry_lines(journal_entry_id)`.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_journal_entry_lines_account_id ON journal_entry_lines(account_id)`.execute(db);
        await (0, kysely_1.sql) `CREATE INDEX IF NOT EXISTS idx_journal_entry_lines_partner ON journal_entry_lines(partner_type, partner_id)`.execute(db);
    ...
    async down(db) {
        await (0, kysely_1.sql) `DROP TABLE IF EXISTS accounting_settings`.execute(db);
        await (0, kysely_1.sql) `DROP TABLE IF EXISTS journal_entry_lines`.execute(db);
        await (0, kysely_1.sql) `DROP TABLE IF EXISTS journal_entries`.execute(db);
        await (0, kysely_1.sql) `DROP TABLE IF EXISTS accounting_accounts`.execute(db);
    },
};
//# sourceMappingURL=20260529000100_accounting_foundation.js.map