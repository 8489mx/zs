...
async function run(): Promise<void> {
  ...
  assert.match(String(inserted[1]?.details), /scoped-details/);
  assert.match(String(inserted[1]?.details), /"tenantId":"tenant-a"/);
  assert.match(String(inserted[1]?.details), /"accountId":"account-a"/);
