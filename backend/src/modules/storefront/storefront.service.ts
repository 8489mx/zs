...
export class StorefrontService {
  ...
  async convertToSale(id: number, actor: AuthContext) {
    const { tenantId } = requireTenantScope(actor);
    ...
    // Prepare Sale lines
    const items = order.items as Array<{ productId: number; quantity: number; unitPrice: number; name: string }>;
    ...
    // Call SalesService to create formal sale invoice
    ...
    // Update order with saleId and status 'delivered'
    await this.db
      .updateTable('online_orders')
      .set({
    ...
    return { ok: true, saleId, sale: saleResult };
  }
