...
const BACKUP_TABLES: BackupTableName[] = [
  '_phase1_bootstrap', 'branches', 'stock_locations', 'users', 'user_branches', 'settings', 'product_categories', 'suppliers', 'customers', 'products', 'product_units', 'product_offers', 'product_customer_prices', 'sales', 'sale_items', 'sale_payments', 'held_sales', 'held_sale_items', 'purchases', 'purchase_items', 'return_documents', 'return_items', 'customer_payments', 'customer_ledger', 'supplier_payments', 'supplier_ledger', 'treasury_transactions', 'expenses', 'cashier_shifts', 'stock_movements', 'stock_transfers', 'stock_transfer_items', 'stock_count_sessions', 'stock_count_items', 'damaged_stock_records', 'sessions', 'audit_logs'
];
...
function normalizeRowForInsert(row: Record<string, unknown>, scope?: { tenantId: string; accountId: string }): Record<string, unknown> {
  const normalized: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(row)) normalized[key] = key.endsWith('_json') && typeof value !== 'string' && value != null ? JSON.stringify(value) : value;
  if (scope) { normalized.tenant_id = scope.tenantId; normalized.account_id = scope.accountId; }
  return normalized;
}
...
export class SettingsBackupService {
  ...
  private assertRestoreConfirmation(payload: unknown): void {
    ...
    if (confirmation !== RESTORE_CONFIRMATION_TEXT) throw new AppError(`Type ${RESTORE_CONFIRMATION_TEXT} to confirm backup restore`, 'BACKUP_RESTORE_CONFIRMATION_REQUIRED', 400);
  }
  private normalizeEnvelope(payload: unknown): BackupEnvelope {
    if (!isObjectRecord(payload)) throw new AppError('Backup payload must be an object', 'BACKUP_INVALID', 400);
    const meta = isObjectRecord(payload.meta) ? { version: String(payload.meta.version || '1.0.0'), exportedAt: String(payload.meta.exportedAt || payload.meta.exported_at || ''), source: String(payload.meta.source || 'manual'), tenantId: String(payload.meta.tenantId || ''), accountId: String(payload.meta.accountId || '') } : { version: '1.0.0', exportedAt: '', source: 'manual' };
    const tablesRaw = isObjectRecord(payload.tables) ? payload.tables : {};
    const tables: Partial<Record<BackupTableName, unknown[]>> = {};
    for (const table of BACKUP_TABLES) tables[table] = Array.isArray(tablesRaw[table]) ? tablesRaw[table] as unknown[] : [];
    return { meta, tables };
  }
  ...
  private async tableExists(table: string): Promise<boolean> {
    const result = await sql<{ exists: boolean }>`select exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = ${table}) as exists`.execute(this.db);
    return Boolean(result.rows[0]?.exists);
  }
  private async tableHasColumn(table: string, column: string): Promise<boolean> {
    const result = await sql<{ exists: boolean }>`select exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = ${table} and column_name = ${column}) as exists`.execute(this.db);
    return Boolean(result.rows[0]?.exists);
  }
  private async getSettingValue<T>(key: string, fallback: T, actor?: AuthContext | null): Promise<T> {
    if (!actor) return fallback;
    const scope = this.scope(actor);
    const row = await this.db.selectFrom('settings').select(['value']).where('key', '=', key).where(sql<boolean>`tenant_id = ${scope.tenantId}`).executeTakeFirst();
    return this.parseSettingValue<T>(row?.value, fallback);
  }
  private async saveSettingValue(key: string, value: unknown, actor: AuthContext): Promise<void> {
    const scope = this.scope(actor);
  ...
  private async getBackupConfigState(actor?: AuthContext | null): Promise<BackupConfigState> {
    const folderPath = this.normalizeFolderPath(await this.getSettingValue<string>(BACKUP_FOLDER_SETTING_KEY, DEFAULT_BACKUP_FOLDER, actor));
    const automationRaw = await this.getSettingValue<Record<string, unknown>>(BACKUP_AUTOMATION_SETTING_KEY, {}, actor);
    const legacyAutoBackup = await this.getSettingValue<string>('autoBackup', 'on', actor);
    return { folderPath, automation: { enabled: typeof automationRaw.enabled === 'boolean' ? automationRaw.enabled : legacyAutoBackup !== 'off', frequency: this.normalizeFrequency(automationRaw.frequency), time: this.normalizeTime(automationRaw.time), weeklyDay: this.normalizeWeeklyDay(automationRaw.weeklyDay), lastSuccessAt: String(automationRaw.lastSuccessAt || '').trim() || undefined, lastAttemptAt: String(automationRaw.lastAttemptAt || '').trim() || undefined, lastAttemptStatus: String(automationRaw.lastAttemptStatus || '').trim() === 'failed' ? 'failed' : String(automationRaw.lastAttemptStatus || '').trim() === 'success' ? 'success' : '', lastError: String(automationRaw.lastError || '').trim() || undefined, lastScheduledFor: String(automationRaw.lastScheduledFor || '').trim() || undefined, lastSavedPath: String(automationRaw.lastSavedPath || '').trim() || undefined } };
  }
  private async saveBackupConfigState(state: BackupConfigState, actor: AuthContext): Promise<void> {
    await this.saveSettingValue(BACKUP_FOLDER_SETTING_KEY, this.normalizeFolderPath(state.folderPath), actor);
    await this.saveSettingValue(BACKUP_AUTOMATION_SETTING_KEY, { enabled: state.automation.enabled, frequency: this.normalizeFrequency(state.automation.frequency), time: this.normalizeTime(state.automation.time), weeklyDay: this.normalizeWeeklyDay(state.automation.weeklyDay), lastSuccessAt: state.automation.lastSuccessAt || null, lastAttemptAt: state.automation.lastAttemptAt || null, lastAttemptStatus: state.automation.lastAttemptStatus || '', lastError: state.automation.lastError || '', lastScheduledFor: state.automation.lastScheduledFor || null, lastSavedPath: state.automation.lastSavedPath || null }, actor);
    await this.saveSettingValue('autoBackup', state.automation.enabled ? 'on' : 'off', actor);
  }
  ...
  private buildBackupFileName(now: Date): string { const pad = (value: number) => String(value).padStart(2, '0'); return `zs-backup-${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}-${pad(now.getHours())}-${pad(now.getMinutes())}-${pad(now.getSeconds())}.json`; }
  ...
  private async saveBackupToFolder(folderPath: string, source: string, actor: AuthContext): Promise<{ filePath: string; fileName: string }> {
    const resolvedFolder = this.normalizeFolderPath(folderPath); await this.ensureWritableFolder(resolvedFolder); const backupPayload = await this.exportBackup(actor); const now = new Date(); const fileName = this.buildBackupFileName(now); const filePath = path.join(resolvedFolder, fileName); const scope = this.scope(actor); await fs.writeFile(filePath, JSON.stringify(backupPayload, null, 2), 'utf8');
    await sql`insert into backup_snapshots (label, source, payload_json, tenant_id, account_id) values (${`file-${now.toISOString()}`}, ${source}, ${JSON.stringify(backupPayload)}::jsonb, ${scope.tenantId}, ${scope.accountId})`.execute(this.db).catch(() => undefined);
    return { filePath, fileName };
  }

  async runAutoBackupIfDue(actor?: AuthContext | null): Promise<Record<string, unknown>> {
    if (!actor) return { ok: true, ran: false, reason: 'no-actor' };
    const state = await this.getBackupConfigState(actor); if (!state.automation.enabled) return { ok: true, ran: false, reason: 'disabled', config: state };
    const now = new Date(); const scheduledAt = this.getLastScheduledDate(now, state.automation); const scheduledIso = scheduledAt.toISOString();
    if (String(state.automation.lastScheduledFor || '').trim() === scheduledIso) return { ok: true, ran: false, reason: 'already-attempted', config: state };
    const lastSuccessAt = state.automation.lastSuccessAt ? new Date(state.automation.lastSuccessAt) : null;
    if (lastSuccessAt && lastSuccessAt.getTime() >= scheduledAt.getTime()) { state.automation.lastScheduledFor = scheduledIso; await this.saveBackupConfigState(state, actor); return { ok: true, ran: false, reason: 'already-succeeded', config: state }; }
    state.automation.lastAttemptAt = now.toISOString(); state.automation.lastScheduledFor = scheduledIso;
    try { const result = await this.saveBackupToFolder(state.folderPath, 'auto-scheduled', actor); state.automation.lastAttemptStatus = 'success'; state.automation.lastSuccessAt = now.toISOString(); state.automation.lastError = ''; state.automation.lastSavedPath = result.filePath; await this.saveBackupConfigState(state, actor); await this.audit.log('نسخ احتياطي تلقائي', `تم إنشاء نسخة احتياطية تلقائية في ${result.filePath}`, actor).catch(() => undefined); return { ok: true, ran: true, success: true, filePath: result.filePath, config: state }; } catch (error) { state.automation.lastAttemptStatus = 'failed'; state.automation.lastError = error instanceof Error ? error.message : 'تعذر تنفيذ النسخ التلقائي'; await this.saveBackupConfigState(state, actor); return { ok: false, ran: true, success: false, error: state.automation.lastError, config: state }; }
  }
  async getBackupConfig(actor?: AuthContext | null): Promise<Record<string, unknown>> { if (actor) this.assertAdmin(actor); await this.runAutoBackupIfDue(actor); const state = await this.getBackupConfigState(actor); return { ok: true, defaultFolderPath: DEFAULT_BACKUP_FOLDER, folderPath: state.folderPath || DEFAULT_BACKUP_FOLDER, automation: state.automation, scope: actor ? this.scope(actor) : null }; }
  async saveBackupConfig(payload: unknown, actor: AuthContext): Promise<Record<string, unknown>> { this.assertAdmin(actor); const input = isObjectRecord(payload) ? payload : {}; const current = await this.getBackupConfigState(actor); const automationInput = isObjectRecord(input.automation) ? input.automation : {}; current.folderPath = this.normalizeFolderPath(input.folderPath ?? current.folderPath ?? DEFAULT_BACKUP_FOLDER); current.automation.enabled = typeof automationInput.enabled === 'boolean' ? automationInput.enabled : typeof input.autoBackupEnabled === 'boolean' ? Boolean(input.autoBackupEnabled) : current.automation.enabled; current.automation.frequency = this.normalizeFrequency(automationInput.frequency ?? input.frequency ?? current.automation.frequency); current.automation.time = this.normalizeTime(automationInput.time ?? input.time ?? current.automation.time); current.automation.weeklyDay = this.normalizeWeeklyDay(automationInput.weeklyDay ?? input.weeklyDay ?? current.automation.weeklyDay); await this.saveBackupConfigState(current, actor); await this.audit.log('تعديل إعدادات النسخ الاحتياطي', `تم تعديل إعدادات النسخ الاحتياطي بواسطة ${actor.username}`, actor).catch(() => undefined); return this.getBackupConfig(actor); }
  async testBackupFolder(payload: unknown, actor: AuthContext): Promise<Record<string, unknown>> { this.assertAdmin(actor); const input = isObjectRecord(payload) ? payload : {}; const folderPath = this.normalizeFolderPath(input.folderPath); try { await this.ensureWritableFolder(folderPath); return { ok: true, success: true, folderPath, message: `تم اختبار المسار بنجاح: ${folderPath}` }; } catch (error) { return { ok: false, success: false, folderPath, message: error instanceof Error ? error.message : 'تعذر الوصول إلى المجلد المحدد' }; } }
  async saveBackupToConfiguredFolder(actor: AuthContext): Promise<Record<string, unknown>> { this.assertAdmin(actor); const state = await this.getBackupConfigState(actor); const result = await this.saveBackupToFolder(state.folderPath, 'manual-save-to-folder', actor); state.automation.lastAttemptAt = new Date().toISOString(); state.automation.lastAttemptStatus = 'success'; state.automation.lastError = ''; state.automation.lastSuccessAt = state.automation.lastAttemptAt; state.automation.lastSavedPath = result.filePath; await this.saveBackupConfigState(state, actor); await this.audit.log('نسخ احتياطي إلى مجلد', `تم حفظ نسخة احتياطية في ${result.filePath} بواسطة ${actor.username}`, actor).catch(() => undefined); return { ok: true, folderPath: state.folderPath, fileName: result.fileName, filePath: result.filePath, message: `تم حفظ النسخة في: ${result.filePath}` }; }

  async exportBackup(actor?: AuthContext | null): Promise<Record<string, unknown>> {
    if (actor) this.assertAdmin(actor);
    const scope = actor ? this.scope(actor) : null;
    const tables = {} as Partial<Record<BackupTableName, unknown[]>>;
    for (const table of BACKUP_TABLES) {
      const exists = await this.tableExists(String(table)); if (!exists) { tables[table] = []; continue; }
      const hasTenant = await this.tableHasColumn(String(table), 'tenant_id');
      tables[table] = hasTenant && scope
        ? await (this.db as any).selectFrom(table).selectAll().where(sql<boolean>`tenant_id = ${scope.tenantId}`).orderBy('id asc').execute().catch(async () => await (this.db as any).selectFrom(table).selectAll().where(sql<boolean>`tenant_id = ${scope.tenantId}`).execute())
        : await (this.db as any).selectFrom(table).selectAll().orderBy('id asc').execute().catch(async () => await (this.db as any).selectFrom(table).selectAll().execute());
    }
    return { meta: { version: '1.2.0', exportedAt: new Date().toISOString(), source: 'manual-export', ...(scope ? { tenantId: scope.tenantId, accountId: scope.accountId } : {}) }, tables };
  }
  async listSnapshots(actor?: AuthContext | null): Promise<Record<string, unknown>> {
    if (actor) this.assertAdmin(actor); const scope = actor ? this.scope(actor) : null;
    const result = scope ? await sql<{ id: number; created_at: string; label: string; source: string; payload_json: unknown }>`select id, created_at, label, source, payload_json from backup_snapshots where tenant_id = ${scope.tenantId} order by created_at desc, id desc limit 20`.execute(this.db) : await sql<{ id: number; created_at: string; label: string; source: string; payload_json: unknown }>`select id, created_at, label, source, payload_json from backup_snapshots order by created_at desc, id desc limit 20`.execute(this.db);
    return { snapshots: result.rows.map((row) => ({ id: String(row.id), createdAt: row.created_at, reason: row.label || row.source || '', payload: row.payload_json })), scope };
  }
  ...
  async restoreBackup(payload: unknown, actor: AuthContext, dryRun = false): Promise<Record<string, unknown>> {
    if (!dryRun) { this.assertCanRestoreBackup(actor); this.assertRestoreConfirmation(payload); }
    const scope = this.scope(actor); const envelope = this.normalizeEnvelope(payload); const verification = await this.verifyBackup(payload);
    if (dryRun) return { ok: true, dryRun: true, summary: verification.summary, scope };
    await this.db.transaction().execute(async (trx) => {
      for (const table of CLEAR_ORDER) { if (!(await this.tableExists(String(table))) || !(await this.tableHasColumn(String(table), 'tenant_id'))) continue; await sql`delete from ${sql.table(table)} where tenant_id = ${scope.tenantId}`.execute(trx); }
      for (const table of BACKUP_TABLES) {
        if (!(await this.tableExists(String(table)))) continue; const rows = Array.isArray(envelope.tables[table]) ? envelope.tables[table]! : []; if (!rows.length) continue; const hasTenant = await this.tableHasColumn(String(table), 'tenant_id');
        for (const row of rows) { if (!isObjectRecord(row)) continue; const record = hasTenant ? normalizeRowForInsert(row, scope) : normalizeRowForInsert(row); await (trx as any).insertInto(table).values(record as any).execute(); }
