...
export class PurchasesService {
  ...
  private async fetchMappedPurchases(): Promise<Array<Record<string, unknown>>> {
  ...
  async createPurchase(payload: UpsertPurchaseDto, auth: AuthContext): Promise<Record<string, unknown>> {
    ...
    const purchase = (await this.fetchMappedPurchases()).find((entry) => Number(entry.id) === purchaseId) || null;
