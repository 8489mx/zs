...
export class CatalogProductService {
  ...
  private hasPermission(actor: AuthContext | undefined, permission: string): boolean { return actor?.role === 'super_admin' || Boolean(actor?.permissions?.includes(permission)); }
  ...
  async listProducts(query: Record<string, unknown>, actor?: AuthContext): Promise<Record<string, unknown>> {
    ...
    const products = (await this.db.selectFrom('products').select(['id','name','barcode','category_id','supplier_id','cost_price','retail_price','wholesale_price','stock_qty','min_stock_qty','notes']).where('is_active', '=', true).orderBy('id desc').execute()) as ProductRow[];
    const units = await this.db.selectFrom('product_units').select(['id','product_id','name','multiplier','barcode','is_base_unit','is_sale_unit_default','is_purchase_unit_default']).orderBy('product_id asc').orderBy('is_base_unit desc').orderBy('id asc').execute();
    const offers = await this.db.selectFrom('product_offers').select(['id','product_id','offer_type','value','start_date','end_date']).where('is_active', '=', true).orderBy('id desc').execute();
    const customerPrices = await this.db.selectFrom('product_customer_prices').select(['id','product_id','customer_id','price']).orderBy('id desc').execute();
    ...
    for (const unit of units) {
      const key = String(unit.product_id); if (!unitsByProduct.has(key)) unitsByProduct.set(key, []);
      unitsByProduct.get(key)!.push({ id: String(unit.id), name: unit.name, multiplier: Number(unit.multiplier || 1), barcode: unit.barcode || '', isBaseUnit: Boolean(unit.is_base_unit), isSaleUnit: Boolean(unit.is_sale_unit_default), isPurchaseUnit: Boolean(unit.is_purchase_unit_default) });
    }
    const offersByProduct = new Map<string, Record<string, unknown>[]>();
    for (const offer of offers) {
      const key = String(offer.product_id); if (!offersByProduct.has(key)) offersByProduct.set(key, []);
      offersByProduct.get(key)!.push({ id: String(offer.id), type: offer.offer_type, value: Number(offer.value || 0), from: offer.start_date ? String(offer.start_date) : '', to: offer.end_date ? String(offer.end_date) : '' });
    }
    const pricesByProduct = new Map<string, Record<string, unknown>[]>();
    for (const cp of customerPrices) {
      const key = String(cp.product_id); if (!pricesByProduct.has(key)) pricesByProduct.set(key, []);
      pricesByProduct.get(key)!.push({ id: String(cp.id), customerId: String(cp.customer_id), price: Number(cp.price || 0) });
    }
    let rows = products.map((p) => { const mapped: Record<string, unknown> = { id: String(p.id), name: p.name || '', barcode: p.barcode || '', categoryId: p.category_id ? String(p.category_id) : '', supplierId: p.supplier_id ? String(p.supplier_id) : '', costPrice: Number(p.cost_price || 0), retailPrice: Number(p.retail_price || 0), wholesalePrice: Number(p.wholesale_price || 0), stock: Number(p.stock_qty || 0), minStock: Number(p.min_stock_qty || 0), notes: p.notes || '', units: unitsByProduct.get(String(p.id)) || [{ id: `base-${p.id}`, name: 'قطعة', multiplier: 1, barcode: p.barcode || '', isBaseUnit: true, isSaleUnit: true, isPurchaseUnit: true }], offers: offersByProduct.get(String(p.id)) || [], customerPrices: pricesByProduct.get(String(p.id)) || [] }; if (!canViewCost) delete mapped.costPrice; return mapped; });
    if (view !== 'all') rows = rows.filter((row) => { const stock = Number(row.stock || 0); const minStock = Number(row.minStock || 0); if (view === 'low') return stock <= minStock; if (view === 'out') return stock <= 0; if (view === 'offers') return Array.isArray(row.offers) && row.offers.length > 0; if (view === 'special') return Array.isArray(row.customerPrices) && row.customerPrices.length > 0; return true; });
    if (q) rows = rows.filter((row) => { const unitValues = Array.isArray(row.units) ? row.units.flatMap((unit) => [String((unit as Record<string, unknown>).name || ''), String((unit as Record<string, unknown>).barcode || '')]) : []; const haystack = [String(row.name || ''), String(row.barcode || ''), categoriesById[String(row.categoryId || '')] || '', suppliersById[String(row.supplierId || '')] || '', String(row.notes || ''), ...unitValues].join(' ').toLowerCase(); return haystack.includes(q); });
    const total = rows.length; const totalPages = Math.max(1, Math.ceil(total / pageSize)); const safePage = Math.min(page, totalPages); const start = (safePage - 1) * pageSize; const pagedRows = rows.slice(start, start + pageSize);
    return { products: pagedRows, pagination: { page: safePage, pageSize, totalItems: total, totalPages }, summary: { totalProducts: total, lowStockCount: rows.filter((row) => Number(row.stock || 0) <= Number(row.minStock || 0)).length, outOfStockCount: rows.filter((row) => Number(row.stock || 0) <= 0).length, inventoryCost: canViewCost ? rows.reduce((sum, row) => sum + (Number(row.stock || 0) * Number(row.costPrice || 0)), 0) : null, inventorySaleValue: rows.reduce((sum, row) => sum + (Number(row.stock || 0) * Number(row.retailPrice || 0)), 0), activeOffersCount: rows.reduce((sum, row) => sum + Number((row.offers as unknown[] | undefined)?.length || 0), 0), customerPriceCount: rows.reduce((sum, row) => sum + Number((row.customerPrices as unknown[] | undefined)?.length || 0), 0) } };
  }

  private normalizeProductPayload(payload: UpsertProductDto): NormalizedUpsertProduct {
    const units = payload.units?.length ? payload.units : [{ name: 'قطعة', multiplier: 1, barcode: payload.barcode || '', isBaseUnit: true, isSaleUnit: true, isPurchaseUnit: true }];
    const normalizedUnits = units.map((unit, index) => ({ name: String(unit.name || '').trim() || (index === 0 ? 'قطعة' : 'وحدة'), multiplier: Number(unit.multiplier || 1), barcode: String(unit.barcode || '').trim(), isBaseUnit: Boolean(unit.isBaseUnit) || Number(unit.multiplier || 1) === 1 || index === 0, isSaleUnit: Boolean(unit.isSaleUnit), isPurchaseUnit: Boolean(unit.isPurchaseUnit) }));
    if (!normalizedUnits.some((unit) => unit.isBaseUnit)) { normalizedUnits[0].isBaseUnit = true; normalizedUnits[0].multiplier = 1; }
    ...
    const seenNames = new Set<string>(); const seenBarcodes = new Set<string>();
    for (const unit of normalizedUnits) {
      if (!(Number(unit.multiplier || 0) > 0)) throw new AppError('Unit multiplier must be greater than zero', 'INVALID_UNITS', 400);
      const nameKey = unit.name.toLowerCase(); if (seenNames.has(nameKey)) throw new AppError('Unit names must be unique per product', 'INVALID_UNITS', 400); seenNames.add(nameKey);
      if (unit.barcode) { if (seenBarcodes.has(unit.barcode)) throw new AppError('Unit barcodes must be unique per product', 'INVALID_UNITS', 400); seenBarcodes.add(unit.barcode); }
    }
    const offers: NormalizedProductOffer[] = (payload.offers || []).map((offer): NormalizedProductOffer => ({ type: offer.type === 'fixed' ? 'fixed' : 'percent', value: Number(offer.value || 0), from: offer.from || null, to: offer.to || null })).filter((offer) => offer.value > 0);
    const customerPrices = (payload.customerPrices || []).map((entry) => ({ customerId: Number(entry.customerId || 0), price: Number(entry.price || 0) })).filter((entry) => entry.customerId > 0);
    return { name: String(payload.name || '').trim(), barcode: String(payload.barcode || '').trim(), categoryId: payload.categoryId ? Number(payload.categoryId) : null, supplierId: payload.supplierId ? Number(payload.supplierId) : null, costPrice: Number(payload.costPrice || 0), retailPrice: Number(payload.retailPrice || 0), wholesalePrice: Number(payload.wholesalePrice || 0), minStock: Number(payload.minStock || 0), notes: String(payload.notes || '').trim(), units: normalizedUnits, offers, customerPrices, ...(payload.stock !== undefined && payload.stock !== null ? { stock: Number(payload.stock || 0) } : {}) };
  }

  private async ensureProductIdentityAvailable(payload: NormalizedUpsertProduct, productId?: number): Promise<void> {
    if (payload.name) { let query = this.db.selectFrom('products').select('id').where(sql`LOWER(name)`, '=', payload.name.toLowerCase()).where('is_active', '=', true); if (productId) query = query.where('id', '!=', productId); const existing = await query.executeTakeFirst(); if (existing) throw new AppError('Product already exists', 'PRODUCT_EXISTS', 400); }
