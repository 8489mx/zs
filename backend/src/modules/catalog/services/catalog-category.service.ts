...
export class CatalogCategoryService {
  ...
  async listCategories(actor: AuthContext): Promise<Record<string, unknown>> {
    const categories = await this.db
      ...
      .where('tenant_id', '=', this.tenantId(actor))
      .orderBy('id asc')
      .execute();
  ...
  async createCategory(payload: UpsertCategoryDto, actor: AuthContext): Promise<Record<string, unknown>> {
    ...
    const duplicate = await this.db.selectFrom('product_categories').select('id').where(sql`LOWER(name)`, '=', name.toLowerCase()).where('is_active', '=', true).where('tenant_id', '=', this.tenantId(actor)).executeTakeFirst();
    if (duplicate) throw new AppError('Category already exists', 'CATEGORY_EXISTS', 400);
    await this.db.insertInto('product_categories').values({ name, is_active: true, tenant_id: this.tenantId(actor), account_id: this.accountId(actor) }).execute();
  ...
  async updateCategory(id: number, payload: UpsertCategoryDto, actor: AuthContext): Promise<Record<string, unknown>> {
    const name = String(payload.name || '').trim();
    if (!name) throw new AppError('Category name is required', 'CATEGORY_NAME_REQUIRED', 400);
    const existing = await this.db.selectFrom('product_categories').select(['id']).where('id', '=', id).where('is_active', '=', true).where('tenant_id', '=', this.tenantId(actor)).executeTakeFirst();
    if (!existing) throw new AppError('Category not found', 'CATEGORY_NOT_FOUND', 404);
    const duplicate = await this.db.selectFrom('product_categories').select('id').where(sql`LOWER(name)`, '=', name.toLowerCase()).where('id', '!=', id).where('is_active', '=', true).where('tenant_id', '=', this.tenantId(actor)).executeTakeFirst();
    if (duplicate) throw new AppError('Category already exists', 'CATEGORY_EXISTS', 400);
    await this.db.updateTable('product_categories').set({ name, updated_at: sql`NOW()` }).where('id', '=', id).where('tenant_id', '=', this.tenantId(actor)).execute();
  ...
  async deleteCategory(id: number, actor: AuthContext): Promise<Record<string, unknown>> {
    const inUse = await this.db.selectFrom('products').select((eb) => eb.fn.countAll<number>().as('count')).where('category_id', '=', id).where('is_active', '=', true).where('tenant_id', '=', this.tenantId(actor)).executeTakeFirstOrThrow();
    if (Number(inUse.count || 0) > 0) throw new AppError('Category is used by products', 'CATEGORY_IN_USE', 400);
    await this.db.updateTable('product_categories').set({ is_active: false, updated_at: sql`NOW()` }).where('id', '=', id).where('tenant_id', '=', this.tenantId(actor)).execute();
