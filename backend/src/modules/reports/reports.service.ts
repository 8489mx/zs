...
import { TrendPoint, buildLastNDays, dateKey, filterScope, normalizeProduct, paginate, parseRange } from './helpers/reports-range.helper';
...
export class ReportsService {
  ...
  async dashboardOverview(query: ReportRangeQueryDto): Promise<Record<string, unknown>> {
    ...
    const todaySalesRows = filterScope(await this.db
      .selectFrom('sales')
      .select(['id', 'total', 'branch_id', 'location_id', 'created_at'])
      .where('status', '=', 'posted')
      .execute(), query).filter((row) => dateKey(row.created_at) === todayKey);

    const todayPurchasesRows = filterScope(await this.db
      .selectFrom('purchases')
      .select(['id', 'total', 'branch_id', 'location_id', 'created_at'])
      .where('status', '=', 'posted')
      .execute(), query).filter((row) => dateKey(row.created_at) === todayKey);
    ...
    const salesTrend: TrendPoint[] = dayKeys.map((key) => ({
      key,
      value: Number(todaySalesRows.filter((row) => dateKey(row.created_at) === key).reduce((sum, row) => sum + Number(row.total || 0), 0).toFixed(2)),
    }));
    const purchasesTrend: TrendPoint[] = dayKeys.map((key) => ({
      key,
      value: Number(todayPurchasesRows.filter((row) => dateKey(row.created_at) === key).reduce((sum, row) => sum + Number(row.total || 0), 0).toFixed(2)),
    }));
  ...
  async inventoryReport(query: ReportRangeQueryDto): Promise<Record<string, unknown>> {
    ...
    const paged = paginate(items, query, 20);
    return {
      items: paged.rows,
  ...
  async customerLedger(customerId: number, query: ReportRangeQueryDto): Promise<Record<string, unknown>> {
    ...
    const paged = paginate(rows, query, 25);
    return {
  ...
  async treasuryTransactions(query: ReportRangeQueryDto): Promise<Record<string, unknown>> {
    ...
    const paged = paginate(mapped, query, 25);
    return {
      treasury: paged.rows,
      pagination: paged.pagination,
      summary: {
  ...
  async auditLogs(query: ReportRangeQueryDto, auth: AuthContext): Promise<Record<string, unknown>> {
    ...
    const paged = paginate(mapped, query, 50);
    return {
