...
| **O74** | **خط أساس الكتابة تحت الحِمل لم يُقَس بعد** — جولة 24 سبتمبر قاست القراءة والدخول والصحة فقط؛ نصف الكتابة ارتدّ من تحقّق المخزون قبل أن يفتح معاملة، فلا رقم موثوقاً حتى الآن لـ`p95` إنشاء الطلب ولا إثبات عملي لخلوّ الترتيب القانوني للأقفال من الجمود تحت تزاحم حقيقي | **الأداة جاهزة:** السيناريوهات المعادة كتابتها تلغي كل طلب فيبقى الحمل مستمراً. المطلوب جولة واحدة على `storefront-checkout.js` بملف `heavy` وقراءة `storefront_order_success_rate` و`storefront_deadlocks_detected` و`storefront_cancel_success_rate` |
