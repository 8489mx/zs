...
- `frontend npm run lint` — **22,451 ملاحظة** (20,201 خطأ). مرشّح سقّاطة أو تخفيف قواعد، بند مستقل.
- `frontend npm run test:run` — **12 اختباراً فاشلاً في 6 ملفات** (كانت 18 في 9). الستة الخاصة بانحدار
  الصلاحيات (O29) أُصلحت وصارت خضراء؛ الباقي انحراف واجهة (نصوص وبنية شاشات تغيّرت ولم تُحدَّث أجنحتها):
  `app-ui-coverage` ×4 · `PortalsHubPage` ×3 · `ProductOfferDialog` ×2 · `app-route-smoke` ×1 ·
  `system-status-banner` ×1 · `DashboardPage.manager-actions` ×1. **لا يُوصَّل كبوابة قبل تصفيرها.**
