...
| **O75** | **الفواتير الـ2,281 على `hesham` بلا قيود، ولا تُرحَّل بأثر رجعي تلقائياً** — `AccountingRecoveryService` يعيد محاولة ما هو مسجَّل في `accounting_posting_failures` فقط، والفواتير القديمة سبقت الجدول فلا صفوف لها. وهي **بيانات تجريبية** على منشأة اختبار، لكن الآلية نفسها ستلزم لو تكرر الأمر على منشأة حقيقية | **قرار مؤجَّل.** الترحيل بأثر رجعي ممكن (القيد مشتقٌّ بالكامل من الفاتورة) لكنه يصطدم بالفترات المقفلة: قيد سبتمبر لا يُرحَّل بعد إقفال سبتمبر. يحتاج قرار المالك على تاريخ الترحيل |
