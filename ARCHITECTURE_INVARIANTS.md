...
| 31 | **روابط المتاجر على سب دومين** (22 سبتمبر 2026) | ✅ في الكود: SF-10، F37/F38، محرك `store-public-url.engine.ts` ومرآته، راوتر مضيف المتجر، رفض الأسماء المحجوزة في المسارات الثلاثة، حارس `validateEnv`، ملف `deploy/nginx/zsystemai-subdomains.conf`، متغير `VITE_STOREFRONT_ROOT_DOMAIN` في النشر. الميزة **مطفأة** حتى يُضبط المتغيران والـDNS والـnginx (O68). جُرّبت فعلياً في المتصفح على `almhnds.localhost` |
