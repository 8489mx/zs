...
| **O67** | **روابط رجوع الدفع لـTap وStripe تشير لدومين غير موجود.** `storefront-payment.service.ts` يبني `https://${custom_domain || slug + '.z-systems.cloud'}/storefront/orders?...` — الدومين ليس لنا (لا يحل في DNS)، والمسار `/storefront/...` من منطقة التاجر التي تحتاج تسجيل دخول لا صفحة المتجر، فالعميل بعد الدفع بالبطاقة يرجع لصفحة لا تفتح. لم يُصلح مع SF-10 لأنه مسار مالي يحتاج اختباراً مع البوابة | العميل الذي يدفع بالبطاقة يضيع بعد الدفع (الطلب نفسه يتأكد بالـWebhook) |
